...
function addLowerThird(L, { label, sub, start = 0.4, end = 4.5, accent = '#F5A623', bg = '#0B0F14' }) {
  if (!label) return;
  const wide = L.W > L.H;
  const barH = Math.round(L.H * (wide ? 0.072 : 0.052));
  const y0 = Math.round(L.H * (sub ? 0.745 : 0.785));
...
function addFigureCard(L, { value, label, start = 0.3, end = 4, accent = '#F5A623', side = 'right' }) {
  if (!value) return;
  const wide = L.W > L.H;
  const cardW = Math.round(L.W * (wide ? 0.33 : 0.76));
  const cardH = Math.round(L.H * (wide ? 0.175 : 0.12));
  const x0 = side === 'right' ? L.W - cardW - Math.round(L.W * 0.055) : Math.round(L.W * 0.055);
  const y0 = Math.round(L.H * (wide ? 0.14 : 0.20));

  L.box(x0, y0, cardW, cardH, '#0B0F14', start, end, { alpha: '2A', layer: 3, fadeIn: 0.3, fadeOut: 0.3 });
  L.box(x0, y0, cardW, Math.max(4, Math.round(L.H * 0.007)), accent, start, end, { layer: 4, fadeIn: 0.3, fadeOut: 0.3 });

  const vSize = Math.round(cardH * (String(value).length > 9 ? 0.34 : 0.44));
  const nmV = L.style('FigV' + side, {
    font: FAMILY.display, size: vSize, primary: '#FFFFFF', bold: false,
    outline: '#000000', outlineW: 2, shadow: 0, align: 7,
    marginL: x0 + Math.round(cardW * 0.06), marginR: 10,
    marginV: y0 + Math.round(cardH * 0.16),
  });
  L.add(nmV, start, end, esc(value), { layer: 5, override: '{\\fad(280,280)}' });

  if (label) {
    const lSize = Math.round(cardH * 0.2);
    const nmL = L.style('FigL' + side, {
      font: FAMILY.semibold, size: lSize, primary: accent, bold: true,
      outline: '#000000', outlineW: 1, shadow: 0, align: 7,
      marginL: x0 + Math.round(cardW * 0.06), marginR: 10,
      marginV: y0 + Math.round(cardH * 0.68),
      spacing: 1.5,
    });
