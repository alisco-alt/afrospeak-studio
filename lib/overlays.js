...
function addHeadline(L, { text, start = 0.25, end = 4, position = 'top', sizeRatio = 0.055, upper = true, accent = '#F5A623', vertical = false, maxLines = 3, still = false }) {
  if (!text) return;
  ...
  L.add(name, start, end, body, {
    layer: 6,
    /* `still` : rendu destiné à UNE frame (miniature) — le fondu d'entrée
     * rendrait le texte invisible à t=0 (constaté : miniatures sans titre). */
    override: still ? '' : `{\\fad(220,260)\\fscx104\\fscy104\\t(0,220,\\fscx100\\fscy100)}`,
  });
}
 ...
 * Deux modes :
 *  · PAR PLAN (défaut historique) : `steps` segments remplis sur `duration` ;
 *  · GLOBAL (`offset` + `total` fournis) : la barre représente la vidéo
 *    ENTIÈRE — le segment k s'allume quand (offset + t) franchit k×total/n.
 *    C'est la barre « Stories » des gros canaux : elle tire l'œil vers la
 *    fin au lieu de repartir à zéro toutes les deux secondes. Les segments
 *    à cheval sur le plan sont posés partiellement et se continuent sur le
 *    plan suivant (continuité parfaite au montage). */
function addProgressBar(L, { duration, accent = '#F5A623', steps = 90, offset = null, total = null }) {
  const h = Math.max(4, Math.round(L.H * 0.006));
  const y = L.H - h;
  L.box(0, y, L.W, h, '#FFFFFF', 0, duration, { alpha: 'D0', layer: 9 });
  if (total > 0 && offset != null) {
    const n = Math.min(steps, Math.max(20, Math.round(total)));
    const seg = total / n;
    for (let i = 0; i < n; i++) {
      const gs = i * seg - offset;               // début du segment, en temps LOCAL au plan
      const ge = (i + 1) * seg + 0.05 - offset;  // (+0,05 s anti-micro-trou)
      if (ge <= 0 || gs >= duration) continue;   // segment hors de ce plan
      L.box(0, y, Math.round(L.W * ((i + 1) / n)), h, accent,
        Math.max(0, gs), Math.min(duration, ge), { alpha: '10', layer: 10 });
    }
    return;
  }
