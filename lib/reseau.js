...
/* Résolveurs de secours — DÉSACTIVÉS PAR DÉFAUT.
 *
 * Mesure sur la machine de l'utilisateur (WSL2) : `1.1.1.1`, `8.8.8.8` et
 * `9.9.9.9` sont TOUS injoignables, alors que le proxy DNS de l'hôte
 * (`10.255.255.254`) répond correctement. Basculer vers un résolveur
 * public y transformait un DNS lent — mais fonctionnel — en DNS mort :
 * `EAI_AGAIN` sur 7 hôtes sur 8, `git` incapable de résoudre github.com.
 *
 * Sous WSL2, le DNS transite par la passerelle NAT de l'hôte Windows.
 * Un pare-feu, un antivirus ou un VPN bloque très souvent le trafic DNS
 * sortant DIRECT, tout en laissant passer celui du proxy. Le résolveur
 * système est donc la voie la plus sûre, même quand il est lent.
 *
 * On ne bascule donc plus d'autorité. Le vrai remède à la lenteur est
 * `options timeout:1 attempts:2` dans /etc/resolv.conf (voir
 * REPARER-DNS.md) et le préchauffage du cache ci-dessous.
 * DNS_SERVEURS='1.1.1.1,8.8.8.8' réactive la bascule si l'utilisateur a
 * vérifié que ces serveurs répondent chez lui. */
const DNS_PUBLICS = (process.env.DNS_SERVEURS || '')
...
async function verifierResolveur(log = () => {}) {
  ...
  if (echec) {
    /* Résolution impossible. On TENTE la bascule, mais on doit pouvoir
     * REVENIR EN ARRIÈRE : un résolveur public n'est pas forcément
     * joignable.
     *
     * Leçon apprise à mes dépens sur la machine de l'utilisateur : après
     * avoir remplacé le résolveur de l'hôte WSL2 par 1.1.1.1, TOUT est
     * tombé (`EAI_AGAIN` sur 7 hôtes sur 8, `git` ne résolvait plus
     * github.com). Le DNS était lent — 5 s — mais il RÉPONDAIT ; le
     * remplaçant, lui, était filtré par le pare-feu. Un résolveur lent
     * qui répond vaut infiniment mieux qu'un résolveur rapide
     * inaccessible.
     *
     * On restaure donc systématiquement en cas d'échec. */
    ...
    try {
      ...
      // Les résolveurs publics sont eux aussi injoignables : on remet
      // ceux d'origine plutôt que de laisser la machine sans DNS.
      try { dns.setServers(avant); } catch (e3) {}
      log('résolveurs publics injoignables (pare-feu ou VPN ?) — '
        + 'résolveur système conservé. Voir REPARER-DNS.md');
      return false;
    }
  }
...
/* TTL porté de 5 à 30 minutes. Sur un résolveur sain, 5 min suffisaient.
 * Sur un résolveur qui met 5 s à répondre — cas WSL2 mesuré — chaque
 * expiration coûte 5 s à la production suivante. Les adresses des
 * banques d'images ne changent pas en une demi-heure. */
const CACHE_TTL_MS = Number(process.env.DNS_CACHE_TTL_MS) || 1800000;   // 30 min
 ...
 * Sur un résolveur lent (5 s par nom, mesuré sous WSL2), une production
 * de 25 plans paie ce coût des dizaines de fois : ~17 min consommées
 * uniquement en résolution, budget média épuisé avant tout
 * téléchargement.
 *
 * On résout donc EN PARALLÈLE, une seule fois au démarrage, les domaines
 * que le studio interroge en boucle. Coût total : le temps du plus lent
 * (~5 s) au lieu de 5 s × N. Le cache système et le cache applicatif
 * sont alors chauds pour toute la session.
 *
 * Les échecs sont ignorés : l'objectif est d'amortir, pas de garantir.
 */
const DOMAINES_CHAUDS = [
  'openrouter.ai', 'api.groq.com',
  'commons.wikimedia.org', 'upload.wikimedia.org',
  'archive.org', 'api.pexels.com', 'pixabay.com',
  'api.openverse.org', 'image.pollinations.ai',
  'duckduckgo.com', 'www.bing.com', 'html.duckduckgo.com',
  'www.youtube.com', 'i.ytimg.com',
];

async function prechauffer(log = () => {}) {
  if (process.env.DNS_PRECHAUFFAGE === '0') return 0;
  installerCacheGlobal();
  const t0 = Date.now();
  const r = await Promise.allSettled(DOMAINES_CHAUDS.map(h => resoudre(h)));
  const ok = r.filter(x => x.status === 'fulfilled' && x.value).length;
  const ms = Date.now() - t0;
  if (ok) log(`DNS préchauffé : ${ok}/${DOMAINES_CHAUDS.length} domaines en ${(ms / 1000).toFixed(1)}s`);
  else log(`DNS injoignable au préchauffage (${(ms / 1000).toFixed(1)}s) — voir REPARER-DNS.md`);
  return ok;
}

/* ── LE CACHE DOIT SERVIR AUX REQUÊTES HTTP, PAS SEULEMENT AU SCRIPT ──
 *
 * Défaut mesuré sur les relevés de l'utilisateur :
 *   openrouter.ai   DNS 1010 ms │ TCP 51 ms │ TLS 138 ms │ fetch() 1285 ms
 *
 * Si la résolution était mise à profit, `fetch()` coûterait ~190 ms
 * (TCP + TLS + HTTP). Il en coûte 1285 : il a REPAYÉ intégralement la
 * résolution. Même constat sur les 8 domaines.
 *
 * La raison : `fetch()` de Node résout lui-même, via `dns.lookup`, et
 * ignore complètement notre `_dnsCache`. Le préchauffage ne servait donc
 * qu'à remplir un cache que personne ne consultait — sur 200 requêtes,
 * c'est 200 s de résolution repayées.
 *
 * `fetch()` n'accepte pas d'agent HTTP personnalisé. On intercepte donc
 * `dns.lookup` lui-même : toute résolution du process — fetch, https,
 * net — passe par notre cache. C'est le seul point de passage commun.
 *
 * On respecte scrupuleusement le contrat de `dns.lookup` : signatures
 * à 2 et 3 arguments, option `all`, et repli sur l'implémentation
 * d'origine dès que le cache ne peut pas répondre. En cas de doute, on
 * délègue — jamais d'invention d'adresse.
 * DNS_CACHE_GLOBAL=0 désactive cette interception.
 */
let _cacheInstalle = false;
function installerCacheGlobal() {
  if (_cacheInstalle || process.env.DNS_CACHE_GLOBAL === '0') return false;
  _cacheInstalle = true;
  const origine = dns.lookup.bind(dns);

  dns.lookup = function (hote, options, callback) {
    // Normalisation des signatures : lookup(h, cb) ou lookup(h, opts, cb)
    if (typeof options === 'function') { callback = options; options = {}; }
    const opts = (typeof options === 'number') ? { family: options } : (options || {});

    const hit = _dnsCache.get(hote);
    if (hit && Date.now() - hit.at < CACHE_TTL_MS && hit.adresses && hit.adresses.length) {
      const fam = hit.famille || 4;
      // `all: true` attend un tableau ; sinon (adresse, famille).
      if (opts.all) {
        const liste = hit.adresses.map(a => ({ address: a, family: fam }));
        return process.nextTick(() => callback(null, liste));
      }
      return process.nextTick(() => callback(null, hit.adresses[0], fam));
    }

    // Pas en cache : résolution normale, puis alimentation du cache.
    return origine(hote, opts, (err, adresse, famille) => {
      if (!err && adresse) {
        try {
          const liste = Array.isArray(adresse)
            ? adresse.map(x => x.address)
            : [adresse];
          const fam = Array.isArray(adresse)
            ? (adresse[0] && adresse[0].family) || 4
            : famille || 4;
          if (liste.length && liste[0]) {
            _dnsCache.set(hote, { adresses: liste, famille: fam, at: Date.now() });
          }
        } catch (e) { /* le cache ne doit jamais casser une résolution */ }
      }
      callback(err, adresse, famille);
    });
  };
  return true;
}
...
module.exports = {
  ...
  verifierResolveur, prechauffer, installerCacheGlobal, DOMAINES_CHAUDS,
  _dnsCache,
};
