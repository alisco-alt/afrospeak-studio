...
async function concatWithTransitions(clips, ctx) {
  ...
  /* Le lot était plafonné à 6 quelle que soit la machine. Or moins de
   * passes = moins de ré-encodages successifs : chaque niveau de l'arbre
   * réencode tout le montage, et les pertes s'accumulent. Sur 32 Go, un
   * lot de 24 plans réduit un montage de 30 plans de 3 passes à 2.
   *
   * Le coût mémoire d'un xfade est proportionnel au nombre d'entrées
   * décodées SIMULTANÉMENT : on budgète ~80 Mo par entrée en 1080×1920,
   * en réservant 40 % de la RAM au système et au reste du pipeline. */
...
function xfadeName(kind) {
  const ok = ['fade', 'dissolve', 'slideleft', 'slideright', 'slideup', 'slidedown',
    'wipeleft', 'wipeup', 'wipedown', 'zoomin', 'smoothleft', 'smoothright',
    'circleopen', 'circleclose', 'fadeblack', 'pixelize',
    'radial', 'squeezeh', 'squeezev',
    'vertopen', 'vertclose', 'horzopen', 'horzclose',
    'diagll', 'diaglr', 'diagul', 'diagur',
...
async function buildAudio(shots, ctx, totalDuration) {
  ...
  filters.push(`${voiceLabel}highpass=f=85,equalizer=f=200:t=q:w=1:g=-2,equalizer=f=3300:t=q:w=2:g=2.5,acompressor=threshold=0.09:ratio=3.2:attack=12:release=220,loudnorm=I=-14:TP=-1.5:LRA=11[voicefx]`);
