...
function findLogo() {
  ...
  /* ── FILIGRANE AFROSPEAK : LOGO COMPLET ──
   * Le watermark doit identifier la chaîne, pas seulement afficher la
   * pastille « plume ». On privilégie donc le logo fourni avec AFRO SPEAK,
   * son nom et sa signature « PARLONS VRAI · PENSONS AFRICAIN ».
   *
   * Le renderer le réduit avec une largeur proportionnelle au format puis
   * l'ancre en haut à droite, comme un logo de chaîne professionnel. La
   * pastille reste un repli explicite si le logo complet est absent.
   */
  const candidates = [
...
/* Le symbole Afrique est réservé au vertical : il reste lisible sur téléphone
 * et n'entre pas en concurrence avec le logo complet horizontal. */
function findLogoVertical() {
  if (process.env.LOGO_VERTICAL_PATH && fs.existsSync(process.env.LOGO_VERTICAL_PATH)) {
    return process.env.LOGO_VERTICAL_PATH;
  }
  for (const c of [
    path.join(DIRS.assets, 'logo-mark.png'),
    path.join(DIRS.root, 'public', 'logo-mark.png'),
...
function logoForFormat(W, H) {
  return H > W ? LOGO_VERTICAL_PATH : LOGO_PATH;
}
...
async function renderShot(shot, ctx, onProgress) {
    ...
    if (isVideo) {
      ...
      if (mode === 'blur') {
        ...
        if (ctx.kenburns && style.zoom > 0.02) {
          const z = 1 + Math.min(0.10, Math.max(0.04, (style.zoom || 0.05) * vv.zoomMul));
  ...
  const useLogo = ctx.watermark && activeLogoPath && ctx.logoOverlay !== false;
  ...
  if (useLogo) {
    ...
    const defaultLogoRatio = /logo-mark/i.test(String(activeLogoPath || ''))
      ? 0.13
      : 0.15;
    const minLogoRatio = /logo-mark/i.test(String(activeLogoPath || '')) ? 0.13 : 0.15;
    ...
    const styleOpacity = Number(ctx.style && ctx.style.logoOpacity);
    const envOpacity = Number(process.env.LOGO_OPACITY);
    /* Par défaut, aucune transparence artificielle : les couleurs du PNG
     * fourni par la chaîne restent celles du logo. LOGO_OPACITY peut encore
     * servir à un habillage spécial, mais n'est plus imposé par le preset. */
    const opacity = Number.isFinite(envOpacity) && envOpacity >= 0
      ? Math.max(0, Math.min(1, envOpacity))
      : (Number.isFinite(styleOpacity) ? Math.max(0, Math.min(1, styleOpacity)) : 1);
    const idx = inputs.filter(a => a === '-i').length;
    inputs.push('-i', activeLogoPath);
    /* Le vertical est centré dans la bande haute pour ne pas se confondre
     * avec les boutons latéraux des plateformes. Le paysage reste en haut à
     * droite. LOGO_POS conserve une surcharge explicite pour un habillage
     * particulier ; il n'y a jamais deux positions simultanées. */
    const logoPos = process.env.LOGO_POS
      || (H > W ? 'top-center' : ((ctx.style && ctx.style.logoPos) || 'top-right'));
    /* Le logo descend SOUS la barre Shorts/TikTok/Reels : même Y pour
     * les deux ancrages, marge haute pilotée par LOGO_TOP_RATIO
     * (10 % de la hauteur en vertical, 6 % en paysage) afin de
     * rester sous les barres et boutons des plateformes sociales. */
    const logoTopRatio = Number(process.env.LOGO_TOP_RATIO)
      || Number(ctx.style && ctx.style.logoTopRatio)
      || (H > W ? 0.10 : 0.06);
    let logoX, logoY;
    if (logoPos === 'top-center') {
      logoX = Math.round((W - logoW) / 2);
      logoY = Math.round(H * logoTopRatio);
    } else {
...
function kenBurns(shot, ctx) {
  ...
  const dur = shot.duration;
  let amt = ctx.kenburns ? style.zoom : 0.008;
...
function transitionFor(group, i, round, style) {
  ...
  // La coupe reste une respiration éditoriale, mais elle ne doit plus être
  // le comportement dominant d'un diaporama. Un plan sur trois peut couper
  // net ; les autres reçoivent une transition directionnelle courte.
  if (i % 3 === 2) return 'cut';
...
async function renderMotionShot(shot, ctx, onProgress) {
  ...
  try {
    ...
    if (startAt > 0.02 || Math.abs(contentDuration - timelineDuration) > 0.05) {
      ...
      await ffmpeg([
        '-i', file,
        '-vf', `tpad=start_mode=add:start_duration=${startAt.toFixed(3)}:color=${bg},trim=duration=${timelineDuration.toFixed(3)},setpts=PTS-STARTPTS,format=yuv420p`,
        '-an',
    ...
    const fallback = path.join(workDir, `motion_fallback_${shot.index}.mp4`);
    ...
    return { file: fallback, duration: dur, fallback: true };
  }
}
