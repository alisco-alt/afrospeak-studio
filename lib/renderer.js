...
const LOW_MEM = (() => {
  ...
  return require('os').totalmem() / 1e6 < 1400;   // < 1,4 Go -> mode économe
})();
...
function encoderArgs(q, { master = false } = {}) {
  if (!LOW_MEM) {
    return [
      '-c:v', 'libx264', '-preset', master ? q.preset : (q.preset === 'slow' ? 'medium' : q.preset),
      '-crf', String(master ? q.crf : Math.max(15, q.crf - 3)),
      '-maxrate', master ? '8M' : '6M', '-bufsize', master ? '4M' : '3M',
      '-refs', master ? '2' : '1', '-rc-lookahead', '10',
    ];
  }
  return [
...
function kenBurns(shot, ctx) {
  ...
  // Le tampon de zoompan croît avec le carré du facteur : 1.5x coûte 2,25×
  // la mémoire d'une image pleine. 1.25x suffit à un zoom de 5-10 % net.
  const OVER = 1.25;
...
module.exports = { renderShot, concatWithTransitions, buildAudio, mux, thumbnail, kenBurns,
  assFilter, autoCrop, blurPad, pickFitMode, encoderArgs, LOW_MEM };
