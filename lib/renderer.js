...
/* ══════════ LOGO DE LA CHAÎNE ══════════
 * La référence 168917.png est cherchée dans les emplacements de distribution.
 * Si elle manque, aucun ancien logo ni filigrane de substitution n'est ajouté.
 */
function findLogo() {
  /* Référence de chaîne unique. Si la pièce jointe n'est pas présente, on
   * préfère un rendu sans logo plutôt que de substituer silencieusement un
   * ancien fichier ou un logo de secours. */
  const candidates = [];
  /* Un chemin externe reste accepté uniquement s'il pointe vers la pièce
   * fournie sous son nom exclusif : aucun ancien logo ne peut être injecté
   * par LOGO_PATH. */
  if (process.env.LOGO_PATH && path.basename(process.env.LOGO_PATH) === '168917.png') {
    candidates.push(process.env.LOGO_PATH);
  }
  candidates.push(
    path.join(DIRS.assets, '168917.png'),
    path.join(DIRS.root, 'public', '168917.png'),
    path.join(DIRS.root, '168917.png'),
  );
  return candidates.find(c => fs.existsSync(c)) || null;
}
const LOGO_PATH = findLogo();

/* La même référence est utilisée dans tous les formats : aucun médaillon
 * concurrent n'est sélectionné pour le vertical. */
function findLogoVertical() {
  return findLogo();
}
const LOGO_VERTICAL_PATH = findLogoVertical();

function logoForFormat(W, H) {
  return findLogo();
}

/* La miniature ne doit pas réintroduire un ancien logo. */
...
async function renderShot(shot, ctx, onProgress) {
  ...
  if (!shot.motion && fs.existsSync(out)) {
    try {
    ...
    try {
      ...
      if (mres && mres.file && fs.existsSync(mres.file)
        ...
        assetMotion = {
          ...
          fallback: !!mres.fallback,
        };
        shot.credit = shot.credit || '';
      }
    } catch (e) {
      shot._motionFailed = shot.motion.type;
      log.warn(`motion ${shot.motion.type} échoué (${String(e.message).slice(0, 90)}) — visuel documentaire explicite utilisé`);
    }
  }

  const asset = assetMotion || shot.asset;
  const assetExiste = !!(asset && asset.file && fs.existsSync(asset.file));
  const isVideo = assetExiste && asset.info && !asset.info.isImage && asset.info.duration > 0.6;
  ...
  if (assetExiste) {
    if (isVideo) {
      ...
      /* Un B-roll ne démarre plus à 12 % de sa durée — ce décalage
       * arbitraire pouvait couper exactement l'action recherchée. Le
       * collecteur peut fournir `sourceStart`/`mediaOffset` en secondes
       * lorsqu'un extrait a été repéré ; sinon on commence au début, choix
       * déterministe et vérifiable. */
      const requestedStart = Number(shot.sourceStart ?? shot.mediaOffset ?? 0);
      const start = Number.isFinite(requestedStart)
        ? Math.max(0, Math.min(requestedStart, Math.max(0, srcDur - Math.min(dur, srcDur))))
        : 0;
      inputs.push('-ss', start.toFixed(3), '-i', asset.file);
      chain.push(`fps=${fps}`);
      ...
      const srcAR = ((asset.info && asset.info.width) || 16) / ((asset.info && asset.info.height) || 9);
      const dstAR = W / H;
      const mode = pickFitMode(ctx.fitMode, srcAR, dstAR);
      if (mode === 'blur') {
        chain.push(blurPad(W, H));
        if ((ctx.kenburns || format === 'vertical') && style.zoom > 0.02) {
          const z = 1 + (format === 'vertical' ? 0.10
            : Math.min(0.10, Math.max(0.04, (style.zoom || 0.05) * vv.zoomMul)));
          const d = dur.toFixed(3);
          // Mouvement appliqué au compositing final : l'image centrale
          // reste intacte, tandis que le fond flou accompagne le drift.
          chain.push(`scale=${even(W * z)}:${even(H * z)}:flags=lanczos`,
            `crop=${W}:${H}:x='(iw-${W})*t/${d}':y='(ih-${H})/2'`);
        }
      } else {
        ...
        if ((ctx.kenburns || format === 'vertical') && style.zoom > 0.02) {
          ...
          // Même un B-roll vidéo reçoit un mouvement mesuré : 1.0 →
          // 1.04–1.10 selon le plan. Le sujet reste dans le cadre, mais
          // l'image ne devient jamais une simple boucle statique.
          const z = 1 + (format === 'vertical' ? 0.10
            : Math.min(0.10, Math.max(0.04, (style.zoom || 0.05) * vv.zoomMul)));
  ...
  if (Math.abs(shield.speed - 1.0) > 0.005) {
    ...
    chain.push(`trim=duration=${dur.toFixed(3)}`, 'setpts=PTS-STARTPTS');
      ...
      const srcAR = ((asset.info && asset.info.width) || 4) / ((asset.info && asset.info.height) || 3);
      ...
      if (mode === 'blur') {
        ...
        if ((ctx.kenburns || format === 'vertical') && style.zoom > 0.02) {
          ...
          const amt = ctx.format === 'vertical' ? 0.10 : style.zoom * 0.6 * vv.zoomMul;
  ...
  /* Une carte motion possède déjà son propre chiffre / ses propres labels.
   * Ne pas lui superposer l'ancien FigureCard : les deux ASS se lisent
   * ensemble et donnent l'impression d'un double slide (parfois avec deux
   * valeurs différentes). Si le motion a échoué et renvoyé un fond de
   * secours, on conserve au contraire la carte classique. */
  const motionOwnsFigure = !!(assetMotion && !assetMotion.fallback && shot.motion
    && /^(dataSlide|comparison|barChart|line|counter|statTile)$/i.test(String(shot.motion.type)));
  if (shot.figure && shot.figure.value && !motionOwnsFigure) {
    ov.addFigureCard(L, {
      plaques,
   ...
   * Mode GLOBAL quand la timeline est connue (shot.start / shot.total,
   * figés par le pipeline) : la barre se remplit sur TOUTE la vidéo —
   * effet « Stories », tiraillement vers la fin — au lieu de repartir
   * à zéro à chaque plan. Repli : comportement par plan (steps: 1).
   * En global, la barre est posée dès le plan 0 (elle montre ~0 %). */
  if (style.progressBar !== false && (shot.index > 0 || shot.start != null)) {
    const globale = shot.start != null && shot.total > 0;
    ov.addProgressBar(L, {
      duration: dur,
      accent: ch.primary,
      steps: globale ? (Number(process.env.PROGRESS_STEPS) || 90) : 1,
      offset: globale ? shot.start : null,
      total: globale ? shot.total : null,
    });
  }
  ...
  if (!motionOwnsFigure && shot.kind === 'data' && shot.figure && shot.figure.value && shot.figure.source) {
    ov.addDataSource(L, {
  ...
  if (estGenere) {
    ...
    const logoEnHaut = ctx.logoPath || findLogo();
  ...
  // Un seul logo est incrusté par plan : le médaillon en 9:16, le bloc
  // horizontal en 16:9. Le texte de secours n'est jamais ajouté en plus
  // lorsque le PNG existe — cela évite l'impression de deux logos joints.
  const activeLogoPath = logoForFormat(W, H);
  /* Le logo n'est plus posé plan par plan : une seule composition finale
   * dans mux() garantit sa présence pendant concaténation, sous-titres,
   * étalonnage et fondus, sans double affichage. */
  const useLogo = false;
  /* Pas de filigrane textuel de substitution : l'incrustation vidéo est
   * exclusivement 168917.png. Tant que cette référence n'est pas disponible,
   * le rendu reste propre et sans second logo concurrent. */
  ...
  let logoFilter = null;
  if (useLogo) {
    const styleLogoRatio = Number(ctx.style && ctx.style.logoWidthRatio);
    const envLogoWidth = Number(process.env.LOGO_WIDTH);
    /* Les deux fichiers gardent exactement leur dessin et leurs couleurs.
     * On leur donne seulement une taille lisible : le médaillon doit rester
     * identifiable sur un téléphone et le bloc horizontal doit laisser lire
     * AFRO SPEAK et sa signature. */
    const defaultLogoRatio = 0.15;
    const minLogoRatio = 0.15;
    const styleRatio = Number.isFinite(styleLogoRatio) && styleLogoRatio > 0
      ? styleLogoRatio : defaultLogoRatio;
    const logoW = even(envLogoWidth > 0
      ? envLogoWidth
      : Math.round(W * Math.max(minLogoRatio, styleRatio)));
    const styleMarginRatio = Number(ctx.style && ctx.style.logoMarginRatio);
    const marginRatio = Number.isFinite(styleMarginRatio) && styleMarginRatio > 0
      ? styleMarginRatio : 0.026;
    const margin = Math.round(H * marginRatio);
    /* Opacité FFmpeg strictement pleine : l'alpha natif du PNG est conservé,
     * mais aucun filtre ni variable d'environnement ne peut le réduire. */
    const opacity = 1.0;
    const idx = inputs.filter(a => a === '-i').length;
    inputs.push('-i', activeLogoPath);
    /* Safe zone constante : le vertical est toujours en haut à droite, à
     * distance des bords et de la barre d'interface des plateformes. */
    const logoPos = H > W ? 'top-right' : 'top-right';
    ...
    const logoX = W - logoW - safeX;
    const logoY = safeY;
    logoFilter = { idx, w: logoW, x: logoX, y: logoY, opacity, logoPos };
  }
  ...
  if (!plaqueFiltres.length && !logoFilter) {
    ...
    if (logoFilter) {
      /* Le PNG possède déjà son alpha propre. Ne pas multiplier son canal
     * alpha artificiellement réduit ternissait les couleurs et faisait
     * croire à un logo effacé. `format=rgba` conserve le dessin
     * original, puis overlay pose une seule occurrence du logo. */
      parties.push(`[${logoFilter.idx}:v]scale=${logoFilter.w}:-1:flags=lanczos,format=rgba[lg]`);
  ...
  await ffmpeg([
    ...
    /* Un graphe de plan qui ne produit absolument aucun octet pendant
     * deux minutes est bloqué, même si la qualité exige de laisser vivre
     * les encodages simplement lents. On borne ici le watchdog spécifique
     * aux plans ; PLAN_WATCHDOG_MS reste disponible pour une machine plus
     * lente, sans réouvrir le délai de 180 s qui a doublé les reprises. */
    inactivityTimeoutMs: Number(process.env.PLAN_WATCHDOG_MS) || 120000,
    maxExecutionMs: ctx._planMaxMs
      || Number(process.env.RENDU_PLAN_MAX_MS || process.env.PLAN_MAX_MS)
...
function pickFitMode(pref, srcAR, dstAR) {
  ...
  /* En 9:16, les bandes/miniatures floutées font perdre le cadre plein
   * demandé pour les Shorts. On remplit donc toujours le cadre par crop ;
   * le mode `blur` reste disponible uniquement sur demande explicite.
   * Pour les formats larges, le blur conserve encore les sources très
   * éloignées sans réduire leur sujet à une vignette centrale. */
  if (dstAR < 0.75) return 'crop';
...
/* ── PROBE CROPDETAIL SERRÉ ──
 * `-vframes` borne bien la sortie (donc le décodage s'arrête après N
 * images) : le coût unitaire est déjà faible. On resserre quand même à
 * 6 images (une décision de bandes noires n'a pas besoin de 18) et on
 * encadre la sonde d'un budget propre — sur une source réseau lente,
 * c'est elle qui pouvait bloquer le rendu du plan. Les sources
 * re-décodées après reprise passent par `cropCache` : zéro second
 * probe pour un même fichier. */
async function autoCrop(file, info) {
  ...
  try {
    ...
    const { stderr } = await ffmpeg([
      '-ss', ss.toFixed(2), '-i', file, '-vframes', '6',
      '-vf', 'cropdetect=limit=26:round=4:reset=0',
      '-f', 'null', '-',
    ], { label: 'cropdetect', loglevel: 'info', maxExecutionMs: 30000, threads: 1 });
...
function kenBurns(shot, ctx) {
  ...
  const dur = shot.duration;
  let amt = (ctx.kenburns || ctx.format === 'vertical') ? (ctx.format === 'vertical' ? 0.10 : style.zoom) : 0.008;
  /* ── PUNCH D'ACCROCHE (plan 0) ────────────────────────────────────
   * Les 0-1 s décident de tout : les gros canaux « tapent » le premier
   * plan avec un zoom plus ample que le reste de la vidéo (pattern
   * interrupt d'ouverture). ×1,6 plafonné à 14 % — présent, jamais
   * caricatural. style.hookPunch (viral : true) ; HOOK_PUNCH=0 désactive. */
  if (shot.index === 0 && style.hookPunch !== false
      && process.env.HOOK_PUNCH !== '0' && amt > 0.02) {
    amt = Math.min(ctx.format === 'vertical' ? 0.10 : 0.14, amt * 1.6);
  }
...
async function concatWithTransitions(clips, ctx) {
  ...
  let level = await Promise.all(clips.map(async (c) => {
    ...
    return {
      ...c,
      duration: reelle > 0.04 ? reelle : c.duration,
    };
  }));
  let round = 0;
...
/* Choisit une transition parce qu'elle raconte quelque chose, pas parce
 * qu'elle est la suivante dans un tableau cyclique. Les grosses chaînes
 * utilisent la coupe comme respiration normale, réservent le fondu à un
 * changement d'idée et donnent un traitement distinct à une révélation.
 * `TRANSITIONS_CONTENU=0` permet de retrouver l'ancien cycle pour comparer
 * deux exports. */
function transitionFor(group, i, round, style) {
  const cycle = style.transitions || ['cut'];
  const previous = group[i - 1] || {};
  const current = group[i] || {};
  if (process.env.TRANSITIONS_CONTENU === '0') return cycle[(i + round) % cycle.length];

  const chapterChanged = previous.sectionIndex != null && current.sectionIndex != null
    && previous.sectionIndex !== current.sectionIndex;
  const headingChanged = previous.sectionHeading && current.sectionHeading
    && previous.sectionHeading !== current.sectionHeading;
  const isReveal = /^(dataSlide|comparison|barChart|chart|figure)$/i.test(String(current.motionType || ''))
    || /^(data|map)$/i.test(String(current.kind || ''));
  const isQuote = /quote|citation/i.test(String(current.kind || ''));

  /* Deux transitions animées à la suite font « diaporama ». Un plan sur
   * deux reste donc une coupe franche, sauf changement de chapitre ou
   * révélation qui mérite explicitement une ponctuation visuelle. */
  if (chapterChanged || headingChanged) return 'fade';
  if (isReveal) return cycle.includes('circleopen') ? 'circleopen' : 'zoomin';
  if (isQuote) return cycle.includes('dissolve') ? 'dissolve' : 'fadefast';
  /* Le vertical premium garde une liaison animée entre deux plans. Les
   * coupes franches restent disponibles pour les presets documentaires, mais
   * ne sont jamais imposées par défaut au style Shorts. */
  if (!style.noHardCuts && i % 3 === 2) return 'cut';
  const candidate = cycle[(i + round) % cycle.length];
  const directional = [
    'slideleft', 'slideright', 'slideup', 'slidedown',
    'smoothleft', 'smoothright', 'smoothup', 'smoothdown',
    'wipeleft', 'wiperight', 'wipeup', 'wipedown',
  ];
  if (directional.includes(candidate)) return candidate;
  if (candidate !== 'cut' && candidate !== 'fade') return candidate;
  return directional[(i + round) % directional.length];
}
...
async function xfadeGroup(group, out, ctx, round) {
  ...
  for (let i = 1; i < group.length; i++) {
    const kind = transitionFor(group, i, round, style);
...
async function buildAudio(shots, ctx, totalDuration) {
  ...
  /* ── MASTER AUDIO : loudness plateforme (EBU R128) ────────────────
   * La voix est normalisée à -16 LUFS, mais le MIX final (voix + musique
   * duckée + SFX) sortait à un niveau variable selon la nappe et la durée.
   * Or YouTube / Shorts / TikTok recalent tout vers -14 LUFS : un master
   * trop bas est re-boosté par la plateforme (perte de punch), trop haut
   * est bridé. On normalise donc le bus final à la cible plateformes.
   * `MIX_LOUDNORM=0` rétablit l'ancien comportement. */
  const masterNorm = filtreMasterLoudness();
  filters.push(`${finalLabel}apad,atrim=0:${totalDuration.toFixed(3)},afade=t=out:st=${Math.max(0, totalDuration - 1.2).toFixed(2)}:d=1.2,${masterNorm}aformat=sample_fmts=fltp:sample_rates=48000:channel_layouts=stereo[aout]`);
...
async function mux(videoFile, audioFile, assFile, ctx, totalDuration, onProgress) {
  ...
  /* LOGO OBLIGATOIRE — dernière opération vidéo. Le PNG fourni est composé
   * après la LUT, les sous-titres, la barre de progression et le fondu : il
   * reste donc visible sur chaque image, à 100 % d'opacité native. */
  const masterLogo = findLogo();
  if (!masterLogo) {
    const err = new Error('Rendu bloqué : assets/168917.png est obligatoire pour le master.');
    err.code = 'LOGO_MISSING';
    throw err;
  }
  const styleLogoRatio = Number(ctx.style && ctx.style.logoWidthRatio);
  const logoRatio = Number.isFinite(styleLogoRatio) && styleLogoRatio > 0
    ? Math.max(0.15, styleLogoRatio) : 0.15;
  const logoW = even(Math.round(W * logoRatio));
  const safeX = H > W ? Math.max(80, Math.round(W * 0.074)) : Math.max(80, Math.round(W * 0.026));
  const safeY = H > W ? Math.max(100, Math.round(H * 0.052)) : Math.max(80, Math.round(H * 0.06));
  const logoX = W - logoW - safeX;
  const logoY = safeY;
  const logoFilter = `[0:v]${vf.join(',')}[base];`
    + `[2:v]scale=${logoW}:-1:flags=lanczos,format=rgba[lg];`
    + `[base][lg]overlay=${logoX}:${logoY}:format=auto[v]`;

  await ffmpeg([
    '-i', videoFile, '-i', audioFile,
    '-loop', '1', '-i', masterLogo,
    '-filter_complex', logoFilter,
    '-map', '[v]', '-map', '1:a',
  ...
  ctx.logoApplied = true;
  ctx.logoPath = masterLogo;
...
async function thumbnail(project, shots, ctx) {
  ...
  /* Aucun ancien filigrane texte dans la miniature : lorsqu'il existe,
   * seule la référence 168917.png est superposée, à opacité pleine. */
  const p = L.write('thumb');
  if (p) chain.push(assFilter(p));
  let thumbArgs;
  const thumbnailLogo = findLogo();
  if (thumbnailLogo) {
    const logoW = even(Math.round(tw * 0.15));
    const safeX = Math.max(36, Math.round(tw * 0.026));
    const safeY = Math.max(36, Math.round(th * 0.06));
    const x = tw - logoW - safeX;
    const y = safeY;
    inputs.push('-i', thumbnailLogo);
    thumbArgs = [
      ...inputs,
      '-filter_complex',
      `[0:v]${chain.join(',')}[base];`
        + `[1:v]scale=${logoW}:-1:flags=lanczos,format=rgba[lg];`
        + `[base][lg]overlay=${x}:${y}:format=auto[v]`,
      '-map', '[v]', '-frames:v', '1', '-q:v', '2', out,
    ];
  } else {
    thumbArgs = [...inputs, '-vf', chain.join(','), '-frames:v', '1', '-q:v', '2', out];
  }
  await ffmpeg(thumbArgs, { label: 'miniature' });
  return out;
}
...
async function renderMotionShot(shot, ctx, onProgress) {
  ...
  const sourceParams = shot.motion.params || {};
  const startAt = Math.max(0, Number(sourceParams.startAt) || 0);
  const timelineDuration = Math.max(
    Number(sourceParams.timelineDuration) || 0,
    startAt + (Number(sourceParams.duration) || 0),
  ) || shot.duration || 3;
  const contentDuration = Math.max(0.35, Number(sourceParams.duration) || timelineDuration - startAt);
  const delayedPath = path.join(workDir, `motion_${shot.index}_${sha1(JSON.stringify({
    type: shot.motion.type, params: sourceParams, W, H, fps,
  })).slice(0, 10)}.mp4`);

  try {
    // Le moteur graphique reçoit uniquement la durée utile. Le préfixe est
    // ajouté ensuite : le premier pixel de la slide tombe donc sur le mot
    // horodaté, et non au début arbitraire du plan.
    const file = await mg.generateMotionClip(shot.motion.type, {
      ...sourceParams,
      duration: contentDuration,
      startAt: 0,
      timelineDuration: contentDuration,
    }, ctx);
    if (!file || !fs.existsSync(file)) throw new Error('motion: fichier non généré');

    let output = file;
    if (startAt > 0.02 || Math.abs(contentDuration - timelineDuration) > 0.05) {
      const tmp = delayedPath + '.tmp';
      const bg = String((ch && ch.bg) || '#0B0F14').replace('#', '0x');
      /* tpad était la source des `motion-cue exited 234` observés sur les
       * slides : selon la version de FFmpeg, le mode start + trim refusait
       * une durée fractionnaire. Un préfixe lavfi concaténé est déterministe
       * et permet de vérifier un vrai conteneur vidéo à la sortie. */
      await ffmpeg([
        '-f', 'lavfi', '-t', startAt.toFixed(3),
        '-i', `color=c=${bg}:s=${W}x${H}:r=${fps || 30}`,
        '-i', file,
        '-filter_complex',
        `[0:v]format=yuv420p[prefix];[1:v]fps=${fps || 30},scale=${W}:${H}:force_original_aspect_ratio=decrease,pad=${W}:${H}:(ow-iw)/2:(oh-ih)/2,format=yuv420p[body];[prefix][body]concat=n=2:v=1:a=0,trim=duration=${timelineDuration.toFixed(3)},setpts=PTS-STARTPTS,format=yuv420p[v]`,
        '-map', '[v]', '-an',
        ...encoderArgs(QUALITY[ctx.quality] || QUALITY.high),
        '-r', String(fps || 30), '-t', timelineDuration.toFixed(3), tmp,
      ], { label: 'motion-cue', totalDuration: timelineDuration, onChild: ctx.onChild });
      fs.renameSync(tmp, delayedPath);
      output = delayedPath;
    }
    const info = await mediaInfo(output);
    if (!info || !info.hasVideo || Number(info.width) !== Number(W)
        || Number(info.height) !== Number(H)
        || Number(info.duration || 0) < timelineDuration - 0.20) {
      throw new Error(`conteneur motion invalide (${info && info.width}x${info && info.height}, ${info && info.duration}s)`);
    }
    return {
      file: output,
      duration: info.duration || timelineDuration,
      info,
    };
  } catch (e) {
    const err = new Error(`motion ${shot.motion.type} invalide : ${String(e.message).slice(0, 140)}`);
    err.code = 'MOTION_RENDER_FAILED';
    err.motionType = shot.motion.type;
    log.warn(err.message);
    /* Aucun fond coloré ne doit maquiller l'échec. renderShot conserve le
     * média réel du plan, marque `_motionFailed`, puis qualityGate invalide
     * explicitement le rendu au lieu d'annoncer 100/100. */
    throw err;
  }
}
...
/** Filtre de loudness appliqué au MASTER final (exporté pour les tests).
 * Cible plateformes : -14 LUFS intégrés, true peak -1,5 dBTP, LRA 11.
 * `MIX_LOUDNORM=0` → chaîne vide (comportement historique). */
function filtreMasterLoudness() {
  return process.env.MIX_LOUDNORM === '0' ? '' : 'loudnorm=I=-14:TP=-1.5:LRA=11,';
}

module.exports = { renderShot, renderMotionShot, concatWithTransitions, buildAudio, mux, thumbnail, kenBurns, filtreMasterLoudness,
  transitionFor, assFilter, autoCrop, blurPad, pickFitMode, encoderArgs, LOW_MEM, extendTail,
  findLogo, findLogoVertical, logoForFormat, LOGO_PATH, LOGO_VERTICAL_PATH,
  splitScreen, pictureInPicture };
