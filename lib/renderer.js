...
/* ── PROTECTION COPYRIGHT (Content ID Shield) ──
 * Modifie le fingerprint visuel des clips pour éviter la détection
 * Content ID (droits d'auteur, ban, démonétisation). Les modifications
 * sont volontairement subtites (imperceptibles à l'œil) mais suffisantes
 * pour casser les hash de frames utilisés par YouTube/TikTok/Instagram.
 *
 * Transformations appliquées par plan (déterministe, cyclique) :
 *  - Hue shift : ±3-8° → change le spectre couleur (très efficace)
 *  - Rotation : ±0.3-0.8° → change la géométrie du frame
 *  - Noise grain : niveau 2-4 → perturbe les pixels
 *  - Mirror (optionnel) : flip horizontal sur certains plans
 *  - Speed : 0.98-1.02x → change le timing fingerprint
 *
 * Contrôles via env :
 *  COPYRIGHT_SHIELD=1  (activer/désactiver, défaut 1)
 *  SHIELD_HUE=8        (amplitude max du hue shift en degrés)
 *  SHIELD_ROTATE=0.8   (amplitude max de rotation en degrés)
 *  SHIELD_NOISE=3      (niveau de bruit, 0-10)
 *  SHIELD_MIRROR=0     (1 = flip sur 1 plan sur 3)
 *  SHIELD_SPEED=0.02   (amplitude max du speed change, 0=off)
 */
...
function copyrightShield(shotIndex) {
  const enabled = (Number(process.env.COPYRIGHT_SHIELD) ?? 1) !== 0;
  if (!enabled) return { filters: [], speed: 1.0 };

  const i = shotIndex % 8;
  const maxHue = Number(process.env.SHIELD_HUE) ?? 8;
  const maxRot = Number(process.env.SHIELD_ROTATE) ?? 0.8;
  const noiseLevel = Number(process.env.SHIELD_NOISE) ?? 3;
  const mirrorEnabled = (Number(process.env.SHIELD_MIRROR) ?? 0) !== 0;
  const maxSpeed = Number(process.env.SHIELD_SPEED) ?? 0.02;
...
async function renderShot(shot, ctx, onProgress) {
  ...
  if (Math.abs(shield.speed - 1.0) > 0.005) {
     ...
     * Ça donne de l'information visuelle même sans photo. */
    const narration = (shot.narration || shot.text || '').slice(0, 120);
    if (narration) {
      L.addText(narration, {
        x: '8%', y: '30%', w: '84%', h: '40%',
        font: 'WixMadefor Display',
        size: Math.round(H * 0.055),
        color: ch.primary || '#F5A623',
        align: 'center', bold: true,
        shadow: '0x000000@0.8,2,2',
        fade: { in: 0.4, out: 0.4 },
      });
      // Petit label en bas
      L.addText('AfroSpeak Studio', {
        x: '8%', y: '85%', w: '84%',
        font: 'WixMadefor Text',
        size: Math.round(H * 0.028),
        color: ch.muted || '#888888',
        align: 'center',
        fade: { in: 0.6, out: 0.3 },
      });
    }
...
function kenBurns(shot, ctx) {
  ...
  const OVER = Number(process.env.KENBURNS_OVERSAMPLE)
    || (LOW_MEM ? 1.5
      : memGo >= 16 ? (amt >= 0.09 ? 6 : 5)
        : memGo >= 8 ? (amt >= 0.09 ? 5 : 4)
          : (amt >= 0.09 ? 4 : 3));
