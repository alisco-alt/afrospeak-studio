...
async function renderShot(shot, ctx, onProgress) {
  ...
  await ffmpeg([
    ...
    maxExecutionMs: Number(process.env.PLAN_MAX_MS)
      || Math.max(45000, Math.min(180000, Math.round(dur * 20000))),
    progressStallMs: Number(process.env.PLAN_STALL_MS) || 45000,
  });
  return out;
}
...
function kenBurns(shot, ctx) {
  ...
  switch (dir) {
    ...
    case 'in':
    default: {
      /* Zoom : la fenêtre reste centrée et c'est l'ÉCHELLE qui varie.
       * `scale` accepte des expressions en `t` ; on garde des dimensions
       * paires via `floor(../2)*2` — x264 refuse toute dimension impaire. */
      const z0 = dir === 'out' ? (1 + marge) : 1;
      const z1 = dir === 'out' ? 1 : (1 + marge);
      const zw = `floor(${W}*(${z0}+(${z1}-${z0})*${p})/2)*2`;
      const zh = `floor(${H}*(${z0}+(${z1}-${z0})*${p})/2)*2`;
      zoomFiltre = `scale=w='${zw}':h='${zh}':eval=frame:flags=bicubic`;
      x = '(iw-ow)/2'; y = '(ih-oh)/2';
      break;
    }
  }
  ...
  const out = [...pre];
  if (zoomFiltre) {
    // On repart de la source agrandie, on l'anime, puis on recadre au format
    out.push(zoomFiltre, `crop=${W}:${H}:${x}:${y}:exact=1`);
  } else {
...
async function xfadeGroup(group, out, ctx, round) {
  ...
  await ffmpeg([
    ...
    progressStallMs: Number(process.env.XFADE_STALL_MS)
      || Math.max(180000, Math.round(total * 12000)),
  });
  return total;
}
