...
async function renderShot(shot, ctx, onProgress) {
  ...
  // Filigrane : symbole compact en vertical, logo complet en horizontal.
  // Les deux fichiers ont la même identité visuelle, mais le mot-symbole
  // horizontal devient trop petit sur un écran 9:16.
  ...
  if (useLogo) {
    ...
    const opacity = Number.isFinite(envOpacity) && envOpacity >= 0
      ...
      : (Number.isFinite(styleOpacity) ? Math.max(0, Math.min(1, styleOpacity)) : 0.86);
    const idx = inputs.filter(a => a === '-i').length;
