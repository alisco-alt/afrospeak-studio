...
async function renderShot(shot, ctx, onProgress) {
  ...
  if (shot.asset && shot.asset.genereParIA) {
    const coin = String(ctx.creditCorner || 'bottom-right').startsWith('bottom')
      ? 'top-right' : 'bottom-right';
    ov.addCredit(L, {
      text: 'ILLUSTRATION IA', corner: coin, size: ctx.creditSize || 'small',
      duration: dur,
    });
  }
  ...
  // Deux entrées (plan + logo) imposent filter_complex ; -vf ne gère qu'un flux.
  const videoArgs = logoFilter
    ? ['-filter_complex',
      `[0:v]${chain.join(',')}[base];`
      + `[1:v]scale=${logoFilter.w}:-1:flags=lanczos,format=rgba,`
      + `colorchannelmixer=aa=${logoFilter.opacity}[lg];`
      + `[base][lg]overlay=${logoFilter.x}:${logoFilter.y}:format=auto[v]`,
      '-map', '[v]']
    : ['-vf', chain.join(',')];
