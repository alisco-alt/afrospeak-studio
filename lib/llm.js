...
const CLOUD_PROVIDERS = [
  {
     ...
     * Nvidia NEMOTRON 70B est gratuit, excellent en français, et sans les
     * limites par minute qui bloquent Groq (429 fréquents). On le place en
     * tête : c'est le modèle par défaut pour la rédaction des scripts.
     * Les autres modèles ":free" servent de bascule si NEMOTRON sature. */
    models: [
  ...
  for (const m of models) {
    try {
      ...
      const budget = Math.max(
        900,
        Math.min(maxTokens, quotaMinute - tailleEntree - 300),
      );
...
async function status() {
  ...
  return {
    ...
    install: (available || cloudReady.length) ? null : {
      ...
      cloud: [
        'OpenRouter — https://openrouter.ai/keys (Nvidia NEMOTRON 70B gratuit, recommandé)',
