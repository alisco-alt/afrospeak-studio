...
/**
 * MOTEUR LLM — OpenRouter en priorité, Groq en secours.
 *
 * Le studio utilise les fournisseurs cloud configurés par l'utilisateur
 * (OpenRouter, puis les replis disponibles) : aucune installation locale
 * supplémentaire n'est requise. Ollama reste un adaptateur facultatif, mais
 * il est explicitement opt-in (`ENABLE_OLLAMA=1`) afin qu'un service local
 * découvert par hasard ne détourne pas la cascade cloud ni le diagnostic.
 *
 * Ordre de préférence :
 *   1. OpenRouter / fournisseurs cloud configurés
 *   2. Serveur compatible OpenAI local explicite (llama.cpp, LM Studio, vLLM)
 *   3. Ollama uniquement si ENABLE_OLLAMA=1
...
async function chatCloudProvider(p, messages, opts = {}) {
  ...
  const models = model
    ? [model]
    : (Number(maxModels) > 0 ? p.models.slice(0, Number(maxModels)) : p.models);
  let lastErr;
      ...
      if (isGroq && dispo < 900) {
        ...
        if (total > budgetCar) {
          ...
          for (const cand of ordre) {
            ...
            const compresse = compresserParBlocs(texte, aRetirer, QUEUE);
            if (!compresse) continue;
            aRetirer -= (texte.length - compresse.length);
            msgs[cand.i].content = compresse;
          }
 ...
 *
 * Ordre : fournisseurs cloud configurés (OpenRouter en tête) → serveur local
 * OpenAI-compatible explicite → Ollama uniquement si ENABLE_OLLAMA=1 →
 * moteur AfroWriter local. Sur une machine sans Ollama, aucune détection ni
 * attente locale n'est donc ajoutée à la production.
 */
async function chat(messages, opts = {}) {
  const errors = [];

  // Ollama n'est jamais sondé par défaut. Il faut l'activer explicitement
  // pour éviter qu'un service local découvert sur la machine détourne
  // OpenRouter ou ajoute des délais réseau inutiles.
  if (process.env.ENABLE_OLLAMA === '1' && process.env.DISABLE_OLLAMA !== '1') {
    try {
  ...
  /* Les étapes qui disposent d'un secours local (notamment la rédaction
   * longue) ne doivent pas parcourir toute la cascade : six modèles lents
   * à 75 s chacun ont transformé un chapitre en attente de plusieurs
   * minutes. Elles peuvent borner explicitement le nombre de fournisseurs
   * et de modèles sans modifier la cascade normale des autres appels. */
  if (opts.providerOnly) cloud = cloud.filter(p => p.id === opts.providerOnly);
  if (Number(opts.maxProviders) > 0) cloud = cloud.slice(0, Number(opts.maxProviders));
...
async function status() {
  const ollamaEnabled = process.env.ENABLE_OLLAMA === '1' && process.env.DISABLE_OLLAMA !== '1';
  const ollamaDisabled = !ollamaEnabled;
...
/* ── Compression par blocs entiers ───────────────────────────────────────
 * L'ancienne compression rognait le prompt à la hache : elle coupait les
 * articles EN DEUX (le log montrait « prompt ramené de 6823 à 3943 jetons »
 * et la matière perdait ses données : chiffres, sources, contexte).
 * Désormais on retire des ARTICLES ENTIERS (blocs `[N] …`) en partant de la
 * fin — les articles sont classés par pertinence décroissante — en gardant
 * toujours : l'amorce, les 2 premiers blocs, la fin du prompt (schéma JSON)
 * et QUEUE caractères de queue. Repli : troncature de la tête si le texte
 * n'a pas de matière balisée. Renvoie null si rien n'est compressable. */
function compresserParBlocs(texte, aRetirer, QUEUE = 2600) {
  texte = String(texte || '');
  if (aRetirer <= 0 || texte.length <= QUEUE + 1200) return null;
  const queue = texte.slice(-QUEUE);
  const tete = texte.slice(0, texte.length - QUEUE);
  const MARQUEUR = 'MATIÈRE PREMIÈRE';
  const debut = tete.indexOf(MARQUEUR);
  if (debut < 0) {
    if (tete.length <= 1200) return null;
    const garder = Math.max(1200, tete.length - aRetirer);
    if (garder >= tete.length) return null;
    return tete.slice(0, garder)
  ...
  const avantMatiere = tete.slice(0, debut);
  const matiere = tete.slice(debut);
  const blocs = matiere.split(/\n(?=\[\d+\]\s)/);
  const amorce = blocs.shift() || '';
  const retenus = [];
  let enleve = 0;
  for (let i = blocs.length - 1; i >= 0; i--) {
    if (i < 2 || enleve >= aRetirer) { retenus.unshift(blocs[i]); continue; }
    enleve += blocs[i].length + 1;
  }
  if (enleve <= 0) return null;
  const notice = '\n\n[…' + (blocs.length - retenus.length)
    + ' source(s) entière(s) retirée(s) pour tenir dans le quota du fournisseur — '
    + 'rédige avec ce qui reste, n\'invente aucun fait…]\n\n';
  const nouveau = avantMatiere + amorce + retenus.join('\n') + notice + queue;
  return nouveau.length < texte.length ? nouveau : null;
}
