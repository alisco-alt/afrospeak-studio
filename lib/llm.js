...
async function chatCloudProvider(p, messages, opts = {}) {
  ...
  for (const m of models) {
    try {
      ...
      /* Prompt à lui seul plus gros que le quota : aucun budget de sortie
       * ne peut sauver l'appel. On passe au modèle suivant tout de suite
       * plutôt que de brûler un aller-retour pour un 413 certain. */
      if (isGroq && dispo < 600) {
        lastErr = new Error(
          `${p.label} : prompt de ${tailleEntree} jetons au-dessus du quota ${quotaMinute} de ${m}`,
        );
        continue;
      }
      let msgs = messages;
      ...
      const body = { model: m, messages: msgs, temperature, max_tokens: budget, stream: false };
      if (json) body.response_format = { type: 'json_object' };
...
async function chatJSON(messages, opts = {}) {
  const essais = Math.max(1, Number(process.env.LLM_JSON_ESSAIS) || 3);
  let derniere = null;
