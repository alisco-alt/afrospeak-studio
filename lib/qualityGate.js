...
function auditStoryboard(storyboard, opts = {}) {
  ...
  shots.forEach((shot, i) => {
    ...
    if (key || (shot && shot.motion && shot.motion.type) || (shot && shot._contextualFallback)) covered++;
    if (shot && shot.asset) {
      if (shot.asset._reemploye || shot._reemploi) {
        reused++;
        if (phase !== 'timeline') {
          issues.push(issue('MEDIA_REUSED', 'error',
            'Visuel réemployé automatiquement : couverture réelle non valide.', i));
        }
      } else if (shot.asset.fondLocal || shot.asset.genereParIA) {
        contextualCovered++;
        if (phase !== 'timeline') {
          issues.push(issue('CONTEXTUAL_VISUAL', 'warning',
            'Visuel contextualisé ou généré : ce plan ne compte pas comme source réelle.', i));
        }
      } else {
        realCovered++;
      }
    }
    if (shot && shot._motionFailed) {
      motionFailures++;
      issues.push(issue('MOTION_FAILED', 'error',
        `Motion ${shot._motionFailed} échoué : aucun score parfait ne peut être conservé.`, i));
    }
    ...
    if (key) {
      ...
      seenAssets.add(key);
      previousAsset = key;
    } else if (shot && shot._contextualFallback) {
      contextualCovered++;
      if (phase !== 'timeline') {
        issues.push(issue('CONTEXTUAL_SLIDE', 'warning',
          'Slide de secours : aucune source média exacte retenue.', i));
      }
      previousAsset = '';
  ...
  return {
    ...
    stats: {
      ...
      reused,
      motionFailures,
      sourceCoverage: shots.length ? +(realCovered / shots.length).toFixed(3) : 0,
      sameAssetRuns,
      shortShots,
      longShots,
...
function auditMaster(info, opts = {}) {
  const issues = [];
  if (opts.logoRequired && !opts.logoApplied) {
    issues.push(issue('MASTER_LOGO_MISSING', 'error',
      'Logo obligatoire 168917.png absent de la composition finale.'));
  }
  if (Number(opts.motionFailures) > 0) {
    issues.push(issue('MASTER_MOTION_FAILED', 'error',
      `${Number(opts.motionFailures)} slide(s) motion ont échoué avant le master.`));
  }
  ...
  return {
    ...
    stats: { width, height, duration, errors: errors.length, warnings: warnings.length,
      logoApplied: !!opts.logoApplied, motionFailures: Number(opts.motionFailures) || 0 },
  };
}
...
function compact(report) {
  ...
  const couverture = report.phase === 'timeline'
    ? 'n/a'
    : `${Math.round((s.sourceCoverage != null ? s.sourceCoverage : (s.coverage || 0)) * 100)} % sources`;
  const reemploi = s.reused ? ` · ${s.reused} réemploi(s)` : '';
  return `score ${report.score}/100 · ${s.shots || 0} plans · ${couverture}${reemploi} · ${s.errors || 0} erreur(s) · ${s.warnings || 0} avertissement(s)`;
}
