...
const CONSIGNE_BASE = `Tu es RÉDACTEUR EN CHEF d'une chaîne panafricaine d'information
 ...
 * Le rédacteur en chef retouche CHAQUE narration : sans la boussole,
 * ses réécritures aplatissaient l'angle émancipation issu de la
 * sélection (run « Sénégal/FMI »). blocPrompt() renvoie '' quand
 * LIGNE_EDITORIALE=0 : désactivable comme partout ailleurs. */
const CONSIGNE = CONSIGNE_BASE + require('./ligne').blocPrompt();

/* ── REPRISE APRÈS REFUS ─────────────────────────────────────────────────
 * Constaté en run réel : « Relecture refusée (chiffre(s) inventé(s) : 10,
 * 000, 60) — brouillon conservé ». Le contrôle a raison de refuser, mais
 * abandonner au premier écart jetait une relecture qui coûte un appel :
 * on demande maintenant UNE copie corrigée, avec le motif du refus injecté
 * dans la consigne. Deux échecs = on conserve le brouillon (comme avant).
 * `CHEF_REPRISE=0` rétablit l'abandon immédiat. */
function messageReprise(raison) {
  return '\n\n⚠ COPIE PRÉCÉDENTE REFUSÉE PAR LE CONTRÔLE QUALITÉ : ' + String(raison || 'non conforme') + '.\n'
    + 'Règles absolues pour la nouvelle version :\n'
    + '- N\'invente AUCUN chiffre : réutilise uniquement les nombres exacts\n'
    + '  présents dans la narration ci-dessus, à l\'identique.\n'
    + '- Garde le même nombre de lignes, le même ordre, tout en français.\n'
    + '- Ne perds pas de matière : chaque ligne reste au moins aussi riche.\n'
    + '- Réponds UNIQUEMENT en JSON : {"lignes":["texte du plan 0",...]}\n';
}
...
async function relire(script, brief = {}, onLog = () => {}) {
  ...
  const baseUser = `SUJET : ${brief.topic || script.title || ''}\n`
    + `FORMAT : ${brief.format === 'vertical' ? 'vertical court' : 'horizontal long'}\n`
    + `ANGLE AFROSPEAK : ${brief.angle || 'contrôle, valeur, dépendance et capacité d’action africaines'}\n`
    + `RÈGLE DE CLARTÉ : ne rends pas le texte plus abstrait ou plus militant. Chaque ligne doit rester une phrase parlée claire, fidèle au fait et reliée au plan précédent.\n`
    + `SÉCURITÉ : pour le Mali et le Sahel, ne présente pas l’État malien comme coupable par défaut. Distingue fait, accusation et contexte. Ne nie pas les responsabilités établies, mais n’efface pas les menaces, les rapports de force et les acteurs extérieurs lorsque la matière les documente.\n\n`
  ...
  /* L'avertissement de reprise est APPENDU au message utilisateur (jamais
   * un 2ᵉ tour user/user) : un seul message, la consigne est indissociable
   * de la matière. */
  const messagesDe = (avertissement = '') => [
    { role: 'system', content: CONSIGNE },
    { role: 'user', content: baseUser + avertissement },
  ];
  const opts = {
    json: true,
    temperature: 0.85,
  ...
  let res;
  try {
    res = await llm.chat(messagesDe(), opts);
  } catch (e) {
  ...
  const reinjecte = (cand) => {
    const lignes = Array.isArray(cand) ? cand
      : (cand.lignes || cand.narrations || cand.plans || null);

    if (Array.isArray(lignes) && lignes.length) {
      const fusionne = JSON.parse(JSON.stringify(script));
      let k = 0;
    ...
    return {
      ...script,
      ...cand,
      sections: Array.isArray(cand.sections) ? cand.sections : script.sections,
    };
  };

  let fusionne = reinjecte(candidat);
  let verdict = controler(script, fusionne);

  /* ── UNE REPRISE, PAS UN ABANDON ──────────────────────────────────
   * Premier refus → on redemande UNE copie avec le motif du refus.
   * La matière de base reste le brouillon ORIGINAL (jamais la copie
   * fautive). Second refus ou appel raté → brouillon conservé. */
  if (!verdict.ok && process.env.CHEF_REPRISE !== '0') {
    onLog(`Copie refusée (${verdict.raison}) — reprise demandée au relecteur (1 essai)`);
    try {
      res = await llm.chat(messagesDe(messageReprise(verdict.raison)), opts);
      candidat = llm.parseJSON(res.content);
      const seconde = reinjecte(candidat);
      const verdict2 = controler(script, seconde);
      if (verdict2.ok) {
        fusionne = seconde;
        verdict = verdict2;
      } else {
        onLog(`Reprise refusée à son tour (${verdict2.raison}) — brouillon conservé`, 'warn');
        log.warn(`copie refusée : ${verdict2.raison}`);
        return { script, ameliore: false, raison: verdict2.raison };
      }
    } catch (e) {
      onLog(`Reprise impossible (${String(e.message).slice(0, 60)}) — brouillon conservé`, 'warn');
  ...
  const premiereAvant = narrationDe(script)[0] || '';
  const premiereApres = narrationDe(fusionne)[0] || '';
  if (premiereAvant !== premiereApres) {
    /* ── L'ACCROCHE RETRAVAILLÉE DOIT ÊTRE APPLIQUÉE, PAS SEULEMENT ANNONCÉE ──
     * Le journal disait « Accroche retravaillée : "Alors que l'on imagine
     * souvent l'Afrique pauvre en richesses…" », mais `script.hook`
     * gardait l'ancienne valeur (« Le Mali est un pays africain. ») :
     * vérifié à l'exécution, l'accroche affichée après relecture était
     * identique à celle d'avant.
     * Or `hook` sert de promesse d'ouverture en aval. Le rédacteur en
     * chef améliorait donc la narration sans que la vidéo en profite.
     * On réaligne le champ sur la première narration réellement retenue. */
    if (fusionne.hook && fusionne.hook === script.hook) {
      fusionne.hook = premiereApres;
    }
    onLog(`Accroche retravaillée : « ${premiereApres.slice(0, 60)}… »`);
  }
...
module.exports = {
  ...
  messageReprise,   // exposé pour les tests (round 4)
};
