...
function estimateShots(minutes, style) {
  ...
  /* En mode reel (vertical), on plafonne le nombre de plans à 15 :
   * un reel de 1:30 à 6s/plan = 15 plans, pas plus. */
  const raw = Math.max(6, Math.round((minutes * 60) / avg));
...
function _requetesDuSujet(topic) {
  ...
  const propres = mots.filter(m => /^[A-ZÀ-Þ]/.test(m));
  const socle = (propres.length ? propres : mots).slice(0, 3).join(' ');
  const out = [socle];
  if (propres.length) {
    out.push(`${propres[0]} historic archive photograph`);
    out.push(`${propres[0]} heritage site`);
  }
  const theme = mots.filter(m => !propres.includes(m)).slice(0, 2).join(' ');
  if (theme) out.push(`${theme} africa`);
