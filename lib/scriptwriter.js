...
═══ INVERSION DE CADRAGE — FAITS CONSERVÉS, CADRAGE RÉANALYSÉ ═══
Quand la matière vient de RFI, Jeune Afrique, AFP, Reuters, BBC, Le Monde ou
un autre média occidental, généraliste ou agrégateur, CONSERVE les faits, les
dates, les chiffres et les citations réellement présents. En revanche, ne
reprends JAMAIS automatiquement son titre, son vocabulaire ni son cadrage
paternaliste, défaitiste, alarmiste ou dépréciatif.
Réexamine chaque sujet depuis les réalités africaines : souveraineté économique,
rapports géopolitiques, acteurs et solutions locales, dignité et autonomie des
peuples. Demande qui décide, qui possède, qui capte la valeur et quelle capacité
africaine est créée ou peut l'être. Distingue explicitement le fait vérifiable,
l'analyse du média et l'interprétation AfroSpeak. Déconstruis un récit orienté
quand les faits le justifient, sans désinformation, sans omission trompeuse,
sans procès d'intention et sans rien inventer.
Ces médias restent des sources de recoupement utiles : ils ne sont ni à ignorer
ni autorisés à imposer seuls la narration. Cherche, quand elle existe, une
source africaine locale, panafricaine, institutionnelle ou indépendante pour
compléter la vérification et le point de vue.
...
/* ── BOUSSOLE ÉMANCIPATION DANS CHAQUE RÉDACTION (correctif 5) ──────
 * lib/ligne.js — la boussole, déjà en place — n'était câblée que dans
 * la SÉLECTION de sujets (editor, autopilot, intelligence) : la
 * rédaction elle-même écrivait sans elle, d'où des scripts génériques
 * sans rapports de force ni levier d'émancipation (run « Sénégal/FMI »).
 * Elle est injectée dans TOUTES les rédactions : SYSTEM (jet unique,
 * chapitrage, secours) et SYSTEM_COURT (quota). blocPrompt() renvoie ''
 * quand LIGNE_EDITORIALE=0 : la boussole reste désactivable. */
const SYSTEM = SYSTEM_BASE + require('./ligne').blocPrompt();

/* ── VERSION CONDENSÉE DE LA LIGNE ÉDITORIALE ────────────────────────
 * Le SYSTEM complet pèse 5 681 jetons. Sur un fournisseur dont le quota
 * total est de 8 000 (Groq), il ne reste alors que ~570 jetons pour
 * ÉCRIRE — moins que les 543 nécessaires à un script de 263 mots. Aux
 * tentatives suivantes, le prompt grossit (re-prompt de réparation) et le
 * budget de sortie devient carrément NÉGATIF.
 *
 * Mesuré sur le run « cacao », d'après les chiffres du journal :
 *   prompt 8 882 → comprimé 6 931 → sortie 569 jetons  (≈ la cible, juste)
 *   prompt 9 713 → comprimé 7 525 → sortie  −25 jetons
 *   prompt 9 772 → comprimé 7 585 → sortie  −85 jetons
 * D'où : 61 mots sur 263, puis échec complet, puis chute sur AfroWriter —
 * dont le remplissage a produit « Écoutez bien : Reprenons depuis le
 * début. La Chine exporte… » sur un sujet africain.
 *
 * La compression ne pouvait pas sauver la situation : elle ne rogne que
 * le message utilisateur, alors que le SYSTEM à lui seul dépassait déjà
 * la cible d'entrée. Il fallait donc une ligne éditoriale plus courte.
 *
 * Cette version tient l'ESSENTIEL — français obligatoire, interdiction de
 * compter les mots, prisme souveraineté, volume, ton, clôture de débat —
 * en ~6 fois moins de jetons. Elle n'est utilisée QUE lorsque le quota du
 * fournisseur l'impose ; le SYSTEM complet reste la référence. */
const SYSTEM_COURT_BASE = `Tu écris UNIQUEMENT EN FRANÇAIS. Aucun mot d'anglais dans la narration
(seul le champ "query" est en anglais). Un script contenant de l'anglais est un ÉCHEC.

NE COMPTE JAMAIS TES MOTS dans ta réponse : pas de décompte, pas de commentaire.
Premier caractère : {   Dernier caractère : }

Tu es le chef d'écriture d'AfroSpeak, chaîne panafricaine en voix off (économie,
géopolitique, sociétés). Tu racontes l'Afrique DEPUIS l'Afrique.

LIGNE ÉDITORIALE — à faire affleurer dans le script :
· QUI DÉCIDE, QUI POSSÈDE, QUI CAPTE LA VALEUR ? Nomme les rapports de force.
· Les acteurs africains AGISSENT : États, entreprises, chercheurs — jamais un décor passif.
· Zéro misérabilisme, zéro complaisance, zéro slogan militant. Les faits portent l'éveil.
· Nomme les PAYS précisément, jamais « l'Afrique » en bloc.
· La conclusion nomme un levier concret d'autonomie, jamais un constat d'impuissance.

CADRAGE DES SOURCES — FAITS CONSERVÉS, CADRAGE RÉANALYSÉ. Règle obligatoire,
même en format court : conserve les faits, dates, chiffres et citations de RFI,
Jeune Afrique, AFP, Reuters, BBC, Le Monde et autres médias généralistes,
mais réanalyse leur cadrage depuis l'Afrique. Ne reprends pas automatiquement
un récit défaitiste, paternaliste,
alarmiste ou dépréciatif. Lis chaque sujet par la souveraineté économique, les
rapports géopolitiques, les solutions locales, la dignité et l'autonomie
africaines. Distingue fait, analyse et interprétation AfroSpeak ; déconstruis
les récits orientés sans désinformation ni invention. Ces sources servent au
recoupement et ne doivent jamais imposer seules l'angle narratif.

VOLUME — non négociable : respecte EXACTEMENT le nombre de plans et de mots
demandés dans le prompt utilisateur. Deux phrases pleines par plan : le fait,
puis sa portée. Un script deux fois trop court est un ÉCHEC TOTAL.

TON : alterne une phrase longue et une phrase BRÈVE. Annonce par des
deux-points. Pose de vraies questions. Isole le contraste
(« Les artistes, eux, ne touchent presque rien. »).

ACCROCHE : 8 à 14 mots, un choc ou un paradoxe, compréhensible sans contexte.

CLÔTURE : une question FERMÉE sur une alternative réelle (A ou B), nommant les
institutions ou acteurs du dossier — jamais « et vous, qu'en pensez-vous ? »
dans le vide. Les deux réponses doivent être défendables. L'abonnement vient
après, en une demi-phrase sobre.

MONOSUJET : tu ne traites QUE le sujet annoncé. Si la matière première évoque
un autre pays ou un autre événement, ignore-le complètement.`;

/* La version condensée porte la boussole elle aussi : le quota ne doit
 * pas se payer en ligne éditoriale (correctif 5). */
const SYSTEM_COURT = SYSTEM_COURT_BASE + require('./ligne').blocPrompt();
  ...
  return `Réponds UNIQUEMENT en JSON valide, ce schéma exact :
{
  ...
  "description": "description YouTube structurée : question d'ouverture, contexte, 3 puces de promesse, débat et appel à s'abonner (sans inventer de source)",
  "tags": ["12 tags"],
...
DESCRIPTION YOUTUBE — elle doit déjà pouvoir être publiée :
· ouvre par une question adressée au spectateur ;
· donne le contexte en 2 ou 3 phrases, puis 3 puces avec 👉, 🔍 ou ✅ ;
· termine par une question fermée liée au sujet et un appel à s'abonner ;
· n'invente ni source, ni chiffre, ni chapitre : le studio ajoutera les crédits
  et les chapitres calculés à partir du montage ;
· propose 10 à 16 hashtags utiles dans "tags" : AfroSpeak, pays, ville,
  institution et thème. Pas de hashtags génériques sans rapport.`;
}
...
/**
 * Sujets qui demandent un garde-fou chronologique renforcé.
 *
 * Les mots « or », « transition » ou « président » seuls sont trop larges :
 * on ne déclenche ce contrat que pour le Burkina, l'AES ou Ibrahim Traoré.
 */
function sujetTransitionBurkina(texte) {
  const t = String(texte || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
  return /ibrahim\s+traore|\btraore\b|methode\s+ib|\bib\b|burkina(?:\s+faso)?|alliance\s+des\s+etats\s+du\s+sahel|\baes\b/.test(t);
}

/** Contrat éditorial appliqué aux dossiers IB/Burkina/AES. */
function dateSource(a) {
  const value = a && (a.date || a.publishedAt || a.published_at || a.upload_date || a.year);
  return value ? String(value).slice(0, 32) : 'date non fournie';
}

function blocChronologie(brief = {}) {
  const sujet = `${brief.topic || ''} ${brief.angle || ''}`;
  if (!sujetTransitionBurkina(sujet)) return '';
  return `
═══ CONTRAT CHRONOLOGIQUE — BURKINA / AES / IB (OBLIGATOIRE) ═══
Ce dossier peut contenir trois couches qui ne doivent jamais être fusionnées :

1. AVANT 2022 : toute donnée antérieure à l'arrivée d'Ibrahim Traoré est une
   situation de référence. Donne son année ou sa période, sa source et son
   indicateur exact. Ne la présente jamais comme une mesure de la « méthode IB »
   et ne l'utilise pas pour juger automatiquement la politique actuelle.
2. DEPUIS 2022 : les décisions, annonces et résultats attribués à la transition
   dirigée par Ibrahim Traoré doivent être postérieurs à son arrivée, datés et
   rattachés à une source. Sépare une décision annoncée, sa mise en œuvre et un
   résultat effectivement mesuré : une annonce n'est pas un résultat.
3. DONNÉES DISPONIBLES À LA DATE DE PRODUCTION : indique la date de référence
   lorsqu'un chiffre décrit la situation actuelle. Si la source ne permet pas
   une comparaison avant/après réellement comparable, dis-le et ne fabrique pas
   de tendance.

RÈGLE DE FORMULATION : chaque chiffre ou résultat important porte sa période et
sa source. Étiquette explicitement « fait documenté », « analyse » ou « opinion ».
Une séquence historique peut expliquer l'héritage d'un secteur, mais elle doit
être annoncée comme historique et ne peut pas servir de preuve d'un bilan IB.
Interdit : relier un chiffre ancien à « la méthode IB », « sous Traoré » ou à
une réussite actuelle sans preuve temporelle. Interdit aussi d'écrire « aucun
autre président n'a fait cela », « sans précédent » ou tout autre superlatif
non démontré par une source. Si un résultat contemporain est documenté, commence
par le reconnaître depuis le point de vue des acteurs africains et de la
souveraineté ; expose ensuite seulement les limites documentées, sans procès
d'intention ni cadrage hostile par défaut.

RÈGLE VISUELLE : un plan contemporain sur le Burkina, Ibrahim Traoré ou l'AES
requiert une requête mentionnant l'acteur, le lieu ou l'initiative concernée.
Un plan historique requiert un marqueur « archive », « historical » ou une date.
N'utilise pas une mine, un bureau ou une ville générique d'un autre pays pour
illustrer un résultat burkinabè. Les sources vidéo doivent être officielles,
autorisées, Creative Commons ou sous licence ; conserve l'URL, l'auteur, la
licence et le crédit. Ne supprime jamais un logo ou un filigrane tiers.
`;
}

/** Vérification légère de la séparation temporelle après rédaction. */
function validerChronologie(script, brief = {}) {
  if (!sujetTransitionBurkina(`${brief.topic || ''} ${brief.angle || ''}`)) return [];
  const issues = [];
  const shots = getScriptShots(script);
  const ancien = /\b(19\d{2}|20(?:0\d|1\d|20|21))\b|\bavant\s+2022\b|\b(royaume|colonial|concession|historique|à\s+l'[ée]poque)\b/i;
  const ib = /\b(m[ée]thode\s+ib|sous\s+(?:ibrahim\s+)?traor[ée]|bilan\s+(?:ib|de\s+traor[ée])|depuis\s+(?:ib|traor[ée]))\b/i;
  for (let i = 0; i < shots.length; i++) {
    const narration = String(shots[i] && shots[i].narration || '');
    if (ancien.test(narration) && ib.test(narration)
      && !/avant\s+2022|avant\s+l['’]arriv[ée]e|depuis\s+2022|à\s+partir\s+de\s+2022|comparaison|r[ée]f[ée]rence/i.test(narration)) {
      issues.push(`Plan ${i + 1} : donnée historique mêlée à la période IB sans séparation datée.`);
    }
  }
  const all = shots.map(s => String(s && s.narration || '')).join(' ');
  if (/aucun\s+autre\s+pr[ée]sident|sans\s+pr[ée]c[ée]dent|jamais\s+un\s+pr[ée]sident/i.test(all)) {
    issues.push('Superlatif politique non démontré : remplacer par un fait sourcé ou une formulation prudente.');
  }
  return issues;
}
...
function buildUserPrompt(brief) {
  ...
  let src = '';
  if (sources.length) {
    /* ── LA MATIÈRE PREMIÈRE A ÉTÉ TRIPLÉE (correctif 5, run Sénégal/FMI) ─
     * Le plafonnement historique (4 articles × 1 100 car.) laissait le
     * rédacteur aveugle : script « générique », sans faits vérifiables.
     * Depuis la lecture complète des articles (voir pipeline), chaque
     * source apporte son TEXTE INTÉGRAL : on passe à 12 articles ×
     * 3 300 caractères (×3 sur les deux axes). Ajustable par env pour
     * les fournisseurs à petit contexte : PROMPT_MAX_ARTICLES,
     * PROMPT_MAX_CAR_ARTICLE. */
    const MAX_ART = Number(process.env.PROMPT_MAX_ARTICLES) || 12;
    const MAX_CAR = Number(process.env.PROMPT_MAX_CAR_ARTICLE) || 3300;
    src = '\n\nMATIÈRE PREMIÈRE — faits vérifiés se rapportant AU SUJET CI-DESSUS :\n'
      + sources.slice(0, MAX_ART).map((a, i) => {
        const role = a.role === 'perspective'
          ? ' — PERSPECTIVE MARCHÉ / RÉACTION À ATTRIBUER'
          : '';
        return `[${i + 1}] ${a.title} — ${a.source || a.site || ''} — DATE: ${dateSource(a)}${role}\n${(a.summary || a.text || '').slice(0, MAX_CAR)}`;
      }).join('\n\n')
...
Les sources marquées « PERSPECTIVE MARCHÉ / RÉACTION » servent à rapporter un
usage, une réaction ou un point de vue local ; elles ne prouvent pas à elles
seules un nombre de souscripteurs, une saturation ou un succès national.`;
  }
...
`
    : '';
  const verdictLigne = ligne.scoreSujet(`${topic} ${angle || ''}`);
  const themeLigne = verdictLigne.label || 'transformation et capacité d’action africaine';
  const blocPerspective = `
═══ CADRE DE LECTURE AFROSPEAK — ${themeLigne.toUpperCase()} ═══
Ce sujet doit être raconté depuis l'Afrique, pour les Africains et la diaspora.
Conserve les faits, dates, chiffres et citations réellement présents dans RFI,
Jeune Afrique, AFP, Reuters ou tout autre média généraliste. Mais ne reprends
pas automatiquement son titre, son vocabulaire ou son cadrage méfiant,
défaitiste, paternaliste ou dépréciatif : réanalyse-le depuis la souveraineté
économique, les rapports géopolitiques, les solutions locales, la dignité et
l'autonomie africaines. Distingue le fait vérifiable, l'analyse de la source et
l'interprétation AfroSpeak ; déconstruis un récit orienté sans désinformation,
sans omission trompeuse et sans invention. Les médias généralistes restent
utiles au recoupement, jamais seuls maîtres de l'angle.
Commence par le fait concret, l'acteur africain et la capacité nouvelle créée.
Puis réponds, avec les sources disponibles, à trois questions :
1. Qu'est-ce que cette initiative change réellement pour les citoyens, les
   travailleurs, les entrepreneurs ou les petits épargnants africains ?
2. Qui possède, qui décide et quelle part de la valeur reste sur le continent ?
3. Ce modèle peut-il être reproduit, intégré à l'échelle régionale ou renforcé
   par le capital africain ?
Pour une introduction en bourse, distingue impérativement : le montant officiel,
le nombre ou la nature des souscripteurs s'il est documenté, les réactions du
marché rapportées comme telles, et l'analyse AfroSpeak. « Les plateformes sont
saturées » ne peut être affirmé que si une source le rapporte ; sinon écris
« selon les témoignages rapportés » ou supprime la formulation.
Une critique documentée est bienvenue, mais elle doit compléter l'accomplissement
et non le remplacer. Ne transforme pas une initiative africaine en scandale par
défaut, ne fabrique aucun enthousiasme et ne fabrique aucun risque.
`;

  return `SUJET (contrat à respecter mot pour mot) : ${topic}
${angle ? 'ANGLE IMPOSÉ : ' + angle : ''}
${blocPerspective}
${blocChronologie(brief)}
${blocConsignes}
...
async function planifierChapitres(brief, opts, onLog) {
  ...
  const matiere = sources.length
    ? '\n\nMATIÈRE PREMIÈRE (textes lus, intégraux quand disponibles) :\n' + sources.slice(0, 12)
      .map((a, i) => `[${i + 1}] ${a.title} — DATE: ${dateSource(a)}\n${(a.summary || a.text || '').slice(0, 2100)}`).join('\n\n')
    : '';
  const cadreChronologique = blocChronologie({ topic, angle });
...
DURÉE VISÉE : ${minutes} minutes.${cadreChronologique}${matiere}
...
async function redigerSequentiel(brief, chapitres, opts, onLog) {
  ...
  /* ── LE VOLUME DOIT SE RECALER EN COURS DE RÉDACTION ──────────────
   * Le budget par chapitre était figé. Quand un chapitre rendait 12
   * plans au lieu de 8, rien ne le rattrapait : le script final pesait
   * 1 273 mots pour 910 demandés (+40 %), il partait au montage « tel
   * quel », et la vidéo durait 7:34 au lieu de 7:00.
   * On recalcule donc la cible restante après CHAQUE chapitre. */
  const _cibleMotsTotal = wordsTarget(minutes, style);

  /* Un chapitre ne doit pas renvoyer les douze articles entiers au LLM.
   * Répéter cette matière à chaque appel dépassait le quota Groq avant même
   * de laisser une place à la narration. Les quatre sources les plus
   * pertinentes, résumées, suffisent ; les faits précis restent dans le
   * plan et la mémoire des chapitres. */
  const matiere = sources.length
    ? '\n\nMATIÈRE PREMIÈRE RÉSUMÉE (faits à vérifier, sans invention) :\n' + sources.slice(0, 4)
      .map((a, i) => `[${i + 1}] ${a.title} — DATE: ${dateSource(a)}${a.role === 'perspective' ? ' — PERSPECTIVE MARCHÉ À ATTRIBUER' : ''}\n${(a.summary || a.text || '').slice(0, 1400)}`).join('\n\n')
    : '';
  const cadreChronologique = blocChronologie({ topic, angle });
  ...
  for (let k = 0; k < chapitres.length; k++) {
    ...
    /* Budget adaptatif : on répartit ce qu'il RESTE à écrire sur les
     * chapitres restants. Un chapitre trop bavard réduit donc les
     * suivants, au lieu de faire déborder le script entier.
     * Bornes : jamais moins de 3 plans (un chapitre doit exister),
     * jamais plus de 1,4 × la part nominale (sinon on déséquilibre). */
    const _motsEcrits = sections.reduce((n, sec) => n
      + sec.shots.reduce((m, sh) => m
        + String(sh.narration || '').split(/\s+/).filter(Boolean).length, 0), 0);
    const _restant = Math.max(0, _cibleMotsTotal - _motsEcrits);
    const _chapRestants = Math.max(1, chapitres.length - k);
    /* Plancher à la part nominale : mesuré, quand les premiers chapitres
     * sont tronqués par le fournisseur (réponse coupée), le budget
     * calculé sur « ce qu'il reste » restait bas et le script finissait
     * à 545 mots pour 910 (−40 %). Le retard doit au contraire être
     * RATTRAPÉ par les chapitres suivants, pas entériné. */
    const _plansCeChapitre = Math.max(parChap, Math.min(
      Math.round(parChap * 1.5),
      Math.round(_restant / _chapRestants / 19) || parChap,
    ));
    ...
    if (k > 0) {
      const pause = Number(process.env.CHAPITRE_PAUSE_MS || 5000);
      if (pause > 0) {
...
⚠ VOLUME EXIGÉ POUR CE CHAPITRE — vérifie avant de répondre :
  · EXACTEMENT ${_plansCeChapitre} plans
  · 18 à 20 mots de narration DANS CHAQUE plan
  · soit ${_plansCeChapitre * 19} mots environ pour ce seul chapitre
...
1. EXACTEMENT ${_plansCeChapitre} plans — pas un de plus, pas un de moins.
...
N'ajoute aucun champ ni commentaire hors du JSON. Ferme proprement.`,
    ...
    /* Aligné sur le budget ADAPTATIF du chapitre : sinon un chapitre de
     * fin, volontairement plus court parce que le quota est déjà
     * consommé, serait rejeté comme « trop court » et re-généré pour
     * rien — c'est une des sources de lenteur du run précédent. */
    const minPlans = Math.max(2, Math.floor(_plansCeChapitre * 0.5));
    let ch, shots = [];
    try {
      ch = res.data || {};
...
${Math.max(3, Math.round(parChap * 0.7))} plans.`,
    ...
    /* ── LA MÉMOIRE DOIT PORTER LE TEXTE, PAS UN RÉSUMÉ ──────────────
     * On ne transmettait qu'UNE ligne de résumé au chapitre suivant. Le
     * modèle ne pouvait donc pas savoir quelles PHRASES avaient déjà été
     * écrites : il reformulait les mêmes idées avec d'autres mots.
     * C'est la cause des répétitions signalées (« on sent qu'il y a des
     * répétitions », script gonflé à 1 273 mots pour 910 demandés).
     * On ajoute les premières phrases réellement rédigées : le modèle
     * voit le texte, pas seulement son intention. Borné pour ne pas
     * gonfler le prompt — c'est ce qui déclenche la compression Groq. */
    const _extraits = shots.slice(0, 4)
      .map(s => String(s.narration || '').split(/[.!?]/)[0].trim())
      .filter(x => x.length > 15).map(x => x.slice(0, 90));
    dejaDit.push((ch.resume || c.angle || c.titre)
      + (_extraits.length ? `\n   déjà formulé : « ${_extraits.join(' » « ')} »` : ''));
...
async function generateWithLLM(brief, onLog = () => {}) {
  ...
  if (minutes > seuilEffectif && process.env.LONG_FORM !== '0') {
    try {
      ...
      /* Un format long ne doit pas parcourir six modèles pendant 75 s
       * chacun. Deux fournisseurs, un modèle prioritaire par fournisseur,
       * une seule tentative JSON et un délai borné suffisent : le chapitre
       * possède déjà son repli, tandis que la cascade complète est réservée
       * aux appels courts qui ont davantage de marge. */
      const longOpts = {
        maxProviders: Number(process.env.LONG_FORM_MAX_PROVIDERS) || 2,
        maxModels: Number(process.env.LONG_FORM_MAX_MODELS) || 1,
        timeout: Number(process.env.LONG_FORM_LLM_TIMEOUT_MS) || 45000,
        essais: Number(process.env.LONG_FORM_JSON_ESSAIS) || 1,
      };
      const long = await generateLongForm(brief, longOpts, onLog);
  ...
  const repairPrompt = brief.repair
    ...
    : userPrompt;
  /* ── QUELLE LIGNE ÉDITORIALE ENVOYER ? ────────────────────────────
   * Le SYSTEM complet (5 681 jetons) est excellent quand le fournisseur a
   * de la place. Mais sur un quota de 8 000 il ne laisse que ~570 jetons
   * pour écrire — moins que les 543 nécessaires à un script de 263 mots,
   * et carrément négatif dès le premier re-prompt.
   * Résultat mesuré : 61 mots sur 263, puis chute sur AfroWriter.
   *
   * On choisit donc la version condensée quand le quota le plus étroit de
   * la cascade ne permet pas de tenir le volume. Elle garde l'essentiel
   * éditorial pour 565 jetons, ce qui libère 6 035 jetons d'écriture.
   * SCRIPT_SYSTEM=long force le comportement d'avant. */
  const _forcer = String(process.env.SCRIPT_SYSTEM || '').toLowerCase();
  const _quotaEtroit = Number(process.env.LLM_TOKENS_PER_MIN) || 8000;
  const _cibleMots = wordsTarget(
    Number(brief.minutes) || 1, STYLES[brief.style] ? brief.style : 'ecofin',
  );
  /* Un plan complet en JSON pèse ~316 caractères pour ~19 mots de
   * narration (mesuré) : la structure — visual, query, queryAlt, kind,
   * onscreen, figure — coûte plus cher que le texte lui-même. Compter
   * 6,5 caractères par mot sous-estimait donc largement le besoin.
   * On dimensionne sur le plan complet, plus une marge pour les champs
   * de tête (title, titles, hook, description, tags, chapters). */
  const _besoinSortie = Math.round((_cibleMots || 200) / 19 * 316 / 3.15) + 600;
  /* Le prompt utilisateur n'est pas un forfait : avec la matière première
   * (4 articles × 1100 caractères, plafonnés dans buildUserPrompt) il
   * approche facilement 2 000 jetons. L'estimer à 900 rendait la place
   * disponible trop optimiste, et le SYSTEM long passait alors que le
   * budget d'écriture était déjà épuisé. */
  const _placeAvecLong = _quotaEtroit - 5681 - 2000 - 500;
  const _utiliserCourt = _forcer === 'court'
    || (_forcer !== 'long' && _placeAvecLong < _besoinSortie);
  const messages = [
    { role: 'system', content: _utiliserCourt ? SYSTEM_COURT : SYSTEM },
...
function generateLocal(brief, onLog = () => {}) {
  ...
  const sujet = String(topic || '').replace(/^(le|la|les|l')\s+/i, '');
  const nShots = estimateShots(minutes, style);
  ...
  const topicTerms = String(topic).toLowerCase()
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
    .split(/[^a-z0-9]+/).filter(w => w.length > 3 && !MOTS_VIDES_SUJET.has(w));
  const sujetFidelite = ancresSujet(topic);
  const topicAliases = sujetFidelite.aliases.map(texteNormaliseSujet);
  const onTopic = txt => {
    const h = texteNormaliseSujet(txt);
    if (topicAliases.length) return topicAliases.some(a => h.includes(a));
    return topicTerms.filter(w => h.includes(w)).length >= Math.min(2, topicTerms.length || 1);
  };
  let pool = [];
  for (const a of sources) {
    ...
    pool.push(...splitSentences(a.text || a.summary || '')
      .filter(s => s.length < 320 && onTopic(s))
  ...
  if (pool.length < nShots) {
    ...
    let gi = 0;
    while (pool.length < nShots) {
      const phrase = generic[gi++ % generic.length];
      /* Même un repli local doit rester compréhensible hors contexte : les
       * phrases de remplissage sont rattachées explicitement au sujet au
       * lieu de produire une dissertation générique sur « l'Afrique ». */
      pool.push({ s: onTopic(phrase) ? phrase : `${sujet} : ${phrase}`, src: '' });
    }
  }
  ...
  for (const sk of SECTION_KINDS) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (sk.id === 'hook' && i === 0) {
        const first = take().s;
        /* L'ouverture ne peut pas être « Voici ce qu'il faut retenir » :
         * elle nomme le dossier et son enjeu dès la première phrase. */
        n = `${sujet} : ${first}`;
      }
  ...
  return normalize({
    ...
    description: `Pourquoi ${topic.toLowerCase()} mérite-t-il notre attention ?\n\nDans cette analyse AfroSpeak, nous remettons les faits dans leur contexte et suivons ce qu'ils changent concrètement pour les acteurs africains.\n\n👉 Ce que les chiffres racontent\n🔍 Qui décide et qui capte la valeur\n✅ Les leviers d'action à surveiller\n\nLe choix est-il celui d'une rupture réelle ou d'une continuité sous un autre nom ? ${ch.cta}`,
    tags: entites.hashtagSujet(topic, { max: 14, marque: 'AfroSpeak' }).map(x => x.slice(1)),
...
/* ══════════════ RÉ-ÉTIQUETAGE LOCAL DES SECTIONS (correctif 4) ══════════════
 * Run « Sénégal / FMI » : le modèle étiquetait les chapitres avec son
 * propre vocabulaire (« contexte », « analyse », « décryptage »,
 * « conclusion »…). `validateScript` n'entend que les kinds canoniques
 * hook/body/outro : il réclamait « Section 'body' (développement)
 * manquante. » ALORS QUE le développement existait — et le pipeline
 * enclenchait jusqu'à 2 re-prompts complets qui n'avaient RIEN à
 * réparer (minutes perdues, script final identique).
 * On ré-étiquette donc LOCALEMENT, avant validation :
 *   1. chaque kind passe par une table de synonymes FR/EN vers les
 *      cinq kinds du schéma (hook, intro, body, twist, outro) ;
 *   2. si aucun « body » n'émerge et qu'il existe des sections
 *      médianes, la plus longue d'entre elles reçoit « body » :
 *      un développement, ça se repère — c'est là où il y a le plus
 *      de texte.
 * La validation garde ainsi du sens : elle ne tire plus sur des
 * manques imaginaires, seulement sur de vrais problèmes. */
const _sansAccentsKind = (s) => String(s || '').normalize('NFD')
  .replace(/[\u0300-\u036f]/g, '').toLowerCase().trim().replace(/\s+/g, ' ');
const KIND_SYNONYMES = {
  hook: 'hook', accroche: 'hook', ouverture: 'hook', teasing: 'hook',
  intro: 'intro', introduction: 'intro', contexte: 'intro',
  'mise en contexte': 'intro', 'mise en place': 'intro',
  body: 'body', developpement: 'body', corps: 'body', constat: 'body',
  enjeu: 'body', enjeux: 'body', analyse: 'body', explication: 'body',
  decryptage: 'body', revelation: 'body', problematique: 'body',
  mecanisme: 'body', mecanique: 'body', chapitre: 'body', partie: 'body',
  section: 'body', sequence: 'body', exemple: 'body', histoire: 'body',
  origines: 'body', origine: 'body', details: 'body', detail: 'body',
  twist: 'twist', bascule: 'twist', 'point de bascule': 'twist',
  tournant: 'twist', climax: 'twist', chute: 'twist',
  outro: 'outro', conclusion: 'outro', synthese: 'outro', resume: 'outro',
  fermeture: 'outro', cloture: 'outro', cta: 'outro',
};
function reetiqueterSections(sections) {
  if (!Array.isArray(sections) || !sections.length) return sections;
  for (const s of sections) {
    const cle = _sansAccentsKind(s.kind);
    if (KIND_SYNONYMES[cle]) s.kind = KIND_SYNONYMES[cle];
  }
  if (!sections.some(s => s.kind === 'body') && sections.length >= 3) {
    const medianes = sections.slice(1, -1);
    let meilleure = null, max = -1;
    for (const s of medianes) {
      const n = s.shots.reduce((a, sh) => a + String(sh.narration || '').split(/\s+/).filter(Boolean).length, 0);
      if (n > max) { max = n; meilleure = s; }
    }
    if (meilleure) meilleure.kind = 'body';
  }
  return sections;
}

/* ── RAMASSAGE PROFOND DES PLANS ─────────────────────────────────────────
 * Dernier filet : un LLM rend parfois une forme jamais vue (clés exotiques,
 * profondeurs arbitraires) alors que les narrations SONT là, bien rédigées.
 * On parcourt tout l'objet et on récupère toute chaîne de ≥ 40 caractères
 * portée par une clé qui ressemble à une narration. Mesuré : deux re-prompts
 * de ~90 s perdus sur des JSON valides dans des formes non lues (« 0/228
 * mots »). */
function ramasserPlans(noeud, sortis = [], profondeur = 0) {
  if (!noeud || typeof noeud !== 'object' || profondeur > 6 || sortis.length >= 60) return sortis;
  if (Array.isArray(noeud)) {
    for (const x of noeud) ramasserPlans(x, sortis, profondeur + 1);
    return sortis;
  }
  for (const [cle, val] of Object.entries(noeud)) {
    if (typeof val === 'string') {
      if (val.trim().length >= 40 && /narration|texte|voix/i.test(cle)) {
        sortis.push({
          narration: val.trim(), visual: '', query: '', queryAlt: '',
          kind: 'broll', onscreen: '', figure: null,
        });
      }
    } else if (val && typeof val === 'object') {
      ramasserPlans(val, sortis, profondeur + 1);
    }
  }
  return sortis;
}
...
function normalize(data, brief) {
  ...
  let brut = data;
  /* ── FORMES ALTERNATIVES : chapitres / chapters / document / script / storyboard ──
   * Un modèle peut emballer ses sections sous une autre clé de premier
   * niveau (observé : `chapitres` d'un modèle francophone). Si la structure
   * ressemble à des sections, on l'accepte au lieu de rendre un script vide. */
  if (!Array.isArray(brut.sections) || !brut.sections.length) {
    const alt = brut.chapitres || brut.chapters
      || (brut.document && brut.document.sections)
      || (brut.script && brut.script.sections)
      || (brut.storyboard && brut.storyboard.sections);
    if (Array.isArray(alt) && alt.length) brut = { ...brut, sections: alt };
  }
  if ((!Array.isArray(brut.sections) || !brut.sections.length)
      && Array.isArray(brut.shots) && brut.shots.length) {
    brut = { ...brut, sections: [{ kind: 'body', heading: '', shots: brut.shots }] };
  }
  let sections = (brut.sections || []).map(sec => ({
    kind: sec.kind || 'body',
    ...
    shots: (sec.shots || sec.plans || sec.segments || sec.scenes || [])
      .map(s => {
        if (typeof s === 'string') return { narration: s };
        if (s && !s.narration) {
          return { ...s, narration: s.texte || s.text || s.voix || s.voiceover || '' };
        }
        return s;
      })
      .filter(s => s && s.narration && String(s.narration).trim()).map(s => ({
      id: uid('shot'),
  ...
  /* Dernier filet : forme totalement non standard mais narrations présentes
   * quelque part dans l'objet — on les ramasse plutôt que de rendre 0 plan. */
  if (!sections.length) {
    const plans = ramasserPlans(brut);
    if (plans.length) {
      log.warn(`forme de script non standard — ${plans.length} plan(s) récupéré(s) par ramassage`);
      sections = [{ kind: 'body', heading: '', shots: plans }];
    }
  }
  reetiqueterSections(sections);
...
function figureFiable(figure, narration) {
  ...
  /* La narration peut écrire le nombre en toutes lettres pour la voix,
   * mais la carte doit rester instantanément lisible : « soixante-sept
   * pour cent » devient « 67 % », sans modifier le texte prononcé. */
  const valeur = nombresEnChiffres(String(figure.value).trim());
  ...
  const out = { value: valeur, label };
  /* La carte ne peut pas inventer la preuve, mais elle ne doit pas non plus
   * perdre les métadonnées fournies par le script : unité, période et source
   * restent attachées au chiffre jusqu'au renderer. Les champs absents restent
   * absents pour conserver la compatibilité du schéma historique. */
  for (const key of ['source', 'unit', 'unite', 'period', 'periode']) {
    if (figure[key] != null && String(figure[key]).trim()) out[key] = String(figure[key]).trim();
  }
  return out;
}
...
async function generate(brief, onLog = () => {}) {
  /* `forcerLocal` : l'appelant a déjà constaté que le LLM rend un script
   * inexploitable (mesuré : 21 mots pour 228, trois fois de suite). Le
   * rappeler ne ferait que répéter l'échec en consommant du quota.
   * On va donc directement au moteur local, qui tient le volume. */
  if (brief && brief.forcerLocal) {
...
async function ideas(items, n = 8) {
  ...
  /* Ligne éditoriale : émancipation, unité, souveraineté, éveil. */
  const ligne = require('./ligne');
  const res = await llm.chat([
    { role: 'system', content: SYSTEM },
    {
      role: 'user', content: `Voici l'actualité africaine du jour :\n${list}\n\nPropose ${n} sujets de vidéo AfroSpeak à fort potentiel de vues.${ligne.blocPrompt()} Réponds en JSON :
...
/* ── FIDÉLITÉ DÉTERMINISTE AU SUJET ET À LA MATIÈRE ────────────────
 * Le volume et la structure ne prouvent pas qu'un texte parle du bon
 * dossier. Cette validation volontairement simple intervient avant la voix :
 * elle bloque un texte générique qui aurait seulement repris un mot comme
 * « Afrique », et tout chiffre absent des documents transmis.
 */
const MOTS_VIDES_SUJET = new Set([
  'avec', 'dans', 'pour', 'sans', 'entre', 'cette', 'cette', 'plus', 'moins',
  'tient', 'promesses', 'faire', 'fait', 'enjeu', 'change', 'comment',
  'pourquoi', 'quelle', 'quels', 'leur', 'leurs', 'notre', 'votre', 'dans',
  'les', 'des', 'une', 'un', 'du', 'de', 'la', 'le', 'et', 'ou', 'au', 'aux',
  'sur', 'qui', 'que', 'est', 'son', 'ses', 'ces', 'ce', 'cest', 'l', 'afrique',
]);

function texteNormaliseSujet(value) {
...
function nombresDuTexte(value) {
  return (String(value || '').replace(/\u00a0/g, ' ').match(/\d[\d .,]*/g) || [])
    .map(x => x.replace(/[^\d]/g, ''))
    .filter(Boolean);
}

function ancresSujet(topic) {
  const original = String(topic || '');
  const n = texteNormaliseSujet(original);
  const aliases = [];
  /* Sigles et noms institutionnels doivent survivre aux reformulations du
   * modèle : ZLECAf/AfCFTA désignent le même dossier, mais un simple mot
   * « marché » ne suffit jamais à l'identifier. */
  if (/\bzlecaf\b|zone de libre echange continentale africaine/.test(n)) {
    aliases.push('zlecaf', 'afcfta', 'zone de libre echange continentale africaine');
  }
  if (/\bcedeao\b/.test(n)) aliases.push('cedeao', 'communaute economique des etats de lafrique de louest');
  if (/\bua\b|union africaine/.test(n)) aliases.push('union africaine', 'ua');
  if (/\baes\b|alliance des etats du sahel/.test(n)) aliases.push('aes', 'alliance des etats du sahel');
  if (/\bbceao\b/.test(n)) aliases.push('bceao');
  if (/\bafcfta\b/.test(n)) aliases.push('afcfta', 'zlecaf');
  const mots = n.split(' ').filter(w => w.length >= 5 && !MOTS_VIDES_SUJET.has(w));
  return { aliases: [...new Set(aliases)], mots };
}

/**
 * @returns {string[]} erreurs bloquantes de fidélité.
 * `sources` est un tableau de documents {title, summary, text, source}.
 */
function validateSubjectFidelity(script, { topic = '', angle = '', sources = [] } = {}) {
  /* Les appels historiques de la librairie peuvent valider un script sans
   * brief (tests/imports). La fidélité devient obligatoire dès qu'un sujet
   * est réellement fourni par le pipeline. */
  if (!String(topic || '').trim()) return [];
  const shots = getScriptShots(script);
  const narrations = shots.map(s => String(s && (s.narration || s.text) || '')).filter(Boolean);
  const texte = texteNormaliseSujet(narrations.join(' '));
  const premier = texteNormaliseSujet(narrations.slice(0, 2).join(' '));
  const titre = texteNormaliseSujet(script && (script.title || script.hook) || '');
  const { aliases, mots } = ancresSujet(`${topic} ${angle}`);
  const issues = [];
  const sourceText = texteNormaliseSujet((sources || []).map(a =>
    `${a && a.title || ''} ${a && a.summary || ''} ${a && a.text || ''}`).join(' '));
  const sourceNumbers = new Set(nombresDuTexte((sources || []).map(a =>
    `${a && a.title || ''} ${a && a.summary || ''} ${a && a.text || ''}`).join(' ')));
  const scriptNumbers = nombresDuTexte(narrations.join(' '));

  const anchorFound = aliases.length
    ? aliases.some(a => texte.includes(texteNormaliseSujet(a)))
    : mots.slice(0, 2).some(w => texte.includes(w));
  const anchorInOpening = aliases.length
    ? aliases.some(a => premier.includes(texteNormaliseSujet(a)))
    : mots.slice(0, 2).some(w => premier.includes(w));
  const anchorInSource = !sources.length || (aliases.length
    ? aliases.some(a => sourceText.includes(texteNormaliseSujet(a)))
    : mots.slice(0, 2).some(w => sourceText.includes(w)));

  if (!shots.length || !narrations.length) {
    issues.push('Fidélité sujet : narration vide.');
  } else {
    if (!anchorFound) {
      issues.push(`Fidélité sujet : le script ne nomme pas clairement « ${String(topic).slice(0, 100)} ».`);
    }
    if (!anchorInOpening) {
      issues.push('Fidélité sujet : les deux premiers plans ne nomment pas le sujet et son enjeu.');
    }
    if (aliases.length && titre && !aliases.some(a => titre.includes(texteNormaliseSujet(a)))) {
      issues.push('Fidélité sujet : le titre ne reprend pas l’entité principale du sujet.');
    }
    if (!anchorInSource) {
      issues.push('Fidélité source : aucune matière transmise ne confirme l’entité principale du sujet.');
    }
    const distinctMots = mots.filter((w, i, a) => a.indexOf(w) === i && texte.includes(w));
    const minimumMots = aliases.length ? 1 : Math.min(2, mots.length);
    if (distinctMots.length < minimumMots) {
      issues.push('Fidélité sujet : le développement ne reprend pas assez de termes spécifiques du sujet.');
    }
    /* Un nombre présent dans le brouillon mais absent des sources est une
     * invention, même si un relecteur LLM l'a introduit. */
    const inventes = scriptNumbers.filter(n => !sourceNumbers.has(n));
    if (sources.length && inventes.length) {
      issues.push(`Fidélité source : chiffre(s) absent(s) des sources (${[...new Set(inventes)].slice(0, 4).join(', ')}).`);
    }
    const periodesBrief = nombresDuTexte(`${topic} ${angle}`).filter(n => /^(?:1\d{3}|20\d{2})$/.test(n));
    const periodesManquantes = periodesBrief.filter(n => !scriptNumbers.includes(n));
    if (periodesManquantes.length) {
      issues.push(`Fidélité période : année(s) du brief absente(s) du script (${[...new Set(periodesManquantes)].join(', ')}).`);
    }
    if (sources.length && !sourceNumbers.size && scriptNumbers.length) {
      issues.push('Fidélité source : le script contient un chiffre, mais aucune source ne fournit de donnée chiffrée.');
    }
  }
  return issues;
}
...
/* ── ACCROCHE FAIBLE : RÉÉCRITURE CIBLÉE (round 4) ──────────────────
 * Constaté en run réel : « Script utilisé tel quel malgré 1 alerte(s):
 * Ton accroche est un constat neutre… ». Les 0-1 s décident de tout, et
 * un re-prompt COMPLET coûte ~90 s : la réécrire ne demande qu'UNE
 * phrase. On réécrit donc l'accroche seule, avec contrôle de qualité
 * strict (aucun chiffre inventé, tension obligatoire). */

const ANGLAIS_ACCROCHE = /\b(the|and|with|from|this|that|will|what|your|about|there|would|could|people|more|than)\b/i;

/** Une alerte de validation concerne-t-elle l'accroche ? (pur, testé) */
function estAlerteAccroche(issue) {
  return /accroche/i.test(String(issue || ''));
}

/** Groupes de chiffres normalisés d'un texte (même logique que le chef). */
function chiffresDeTxt(txt) {
  const trouves = String(txt || '').replace(/\u00A0/g, ' ').match(/\d[\d .,]*/g) || [];
  return trouves.map(x => x.replace(/[^\d]/g, '')).filter(x => x.length > 0);
}

/** Contrôle qualité d'une accroche proposée (pur, testé).
 * Refuse : vide, longueur hors bornes, identique à l'ancienne, anglais,
 * chiffre absent de la matière, constat neutre (accrochePlate), formule
 * creuse (accrocheCreuse). */
function accrocheAcceptable(nouvelle, { ancienne = '', contexte = '' } = {}) {
  const t = String(nouvelle || '').trim();
  if (!t) return false;
  const mots = t.split(/\s+/).filter(Boolean);
  if (mots.length < 8 || mots.length > 30 || t.length > 190) return false;
  if (ancienne && t === String(ancienne).trim()) return false;
  if (ANGLAIS_ACCROCHE.test(t)) return false;
  const permis = new Set(chiffresDeTxt(String(ancienne) + ' ' + String(contexte)));
  if (chiffresDeTxt(t).some(c => !permis.has(c))) return false;
  /* Les détecteurs existants font foi, sur un script fictif minimal :
   * pas de constat neutre (plate), pas de formule creuse. */
  const fictif = { sections: [{ shots: [{ narration: t }, { narration: '' }] }] };
  if (accrochePlate(fictif)) return false;
  if (accrocheCreuse(fictif)) return false;
  return true;
}

/** Réécrit l'accroche seule (1 appel LLM, 1 proposition, contrôle strict).
 * @returns {Promise<boolean>} true si l'accroche a été remplacée. */
async function reecritAccroche(script, brief, { motif = '', say = () => {} } = {}) {
  if (process.env.HOOK_REPRISE === '0') return false;
  const shots = getScriptShots(script);
  const ancienne = String((shots[0] && (shots[0].narration || shots[0].text)) || '').trim();
  if (!ancienne) return false;
  const contexte = shots.slice(0, 3)
    .map((s, i) => `[${i}] ${String((s && (s.narration || s.text)) || '').trim()}`)
    .filter(l => l.length > 4).join('\n');

  const demande = 'Tu réécris UNE SEULE PHRASE : l\'accroche (plan 0) d\'un short '
    + 'd\'information économique en FRANÇAIS.\n'
    + `Accroche actuelle (faible) : « ${ancienne} »\n`
    + (motif ? `Reproche du rédacteur en chef : ${String(motif).slice(0, 200)}\n` : '')
    + 'Règles NON NÉGOCIABLES :\n'
    + '- 12 à 22 mots. Zéro anglais. Zéro guillemet, zéro préfixe (« Accroche : »).\n'
    + '- Ouvre par une question directe, une opposition (« alors que…, … »), '
    + 'un chiffre DÉJÀ PRÉSENT dans les extraits ci-dessous, ou une rupture '
    + '(aucun, jamais, premier, seul).\n'
    + '- INTERDIT d\'inventer un chiffre, un nom ou un fait : utilise uniquement '
    + 'ce qui figure dans l\'accroche actuelle et les extraits.\n\n'
    + `EXTRAITS DU SCRIPT :\n${contexte}\n\n`
    + 'Réponds UNIQUEMENT en JSON : {"accroche":"la nouvelle phrase"}';

  let res;
  try {
    res = await llm.chat([
      { role: 'system', content: 'Tu es rédacteur en chef d\'une chaîne '
        + 'd\'information économique panafricaine. Tu réponds uniquement en JSON valide.' },
    ...
    say(`Réécriture d'accroche : appel impossible (${String(e.message).slice(0, 60)})`, 'warn');
    return false;
  }
  let cand;
  try {
    cand = llm.parseJSON(res.content);
  } catch (e) {
    say('Réécriture d\'accroche : réponse illisible — accroche conservée', 'warn');
    return false;
  }
  const nouvelle = String((cand && (cand.accroche || cand.hook || cand.ligne)) || '').trim()
    .replace(/^["«”"\s]+/, '').replace(/["«»”"\s]+$/, '');
  if (!accrocheAcceptable(nouvelle, { ancienne, contexte: ancienne + ' ' + contexte })) {
    say('Réécriture d\'accroche : proposition rejetée au contrôle (chiffre inventé, '
      + 'constat neutre ou hors consigne) — accroche conservée', 'warn');
    return false;
  }
  /* Application : plan 0 ET champ hook (même alignement que le rédacteur
   * en chef — le champ sert de promesse d'ouverture en aval). */
  shots[0].narration = nouvelle;
  script.hook = nouvelle;
  updateScriptStats(script);
  say(`Accroche réécrite : « ${nouvelle.slice(0, 70)} »`);
  return true;
}

function validateScript(script, options = {}) {
  const { targetWords, format, topic = '', angle = '', sources = [] } = options || {};
  ...
  if (target > 0) {
    ...
    } else if (wordCount > maxWords * 1.6) {
      /* ── UN SCRIPT TROP LONG NE SE RÉÉCRIT PAS, IL SE COUPE ─────────
       * Mesuré sur le run « cacao » : le script a été REPLANIFIÉ trois
       * fois de suite (1146 puis 1273 mots pour 910), soit ~15 minutes
       * de rédaction — quatre chapitres et trois pauses de quota par
       * passe — pour finalement retenir la version la plus longue.
       * Un dépassement de 25 % n'est pas un défaut de qualité : c'est un
       * script riche, que le montage sait raccourcir. On ne déclenche
       * donc une alerte qu'au-delà de 60 % au-dessus du plafond, là où
       * la durée deviendrait réellement ingérable.
       * Le manque de volume, lui, reste sanctionné : un script court ne
       * se répare pas au montage. */
  ...
  /* Correctif 4 : les scripts hérités d'avant le ré-étiquetage (projets
   * sauvegardés, réponses LLM rechargées) portent encore les kinds
   * « contexte/analyse/conclusion » — on les ré-étiquette ici aussi,
   * sinon la validation rejouerait le faux « body manquant ». */
  reetiqueterSections(sections);
  ...
  issues.push(...validerChronologie(script, { topic, angle }));
  issues.push(...validateSubjectFidelity(script, { topic, angle, sources }));
...
module.exports = {
  generate, generateLocal, ideas, estimateShots, wordsTarget, estimerDuree, SYSTEM, SYSTEM_COURT,
  sujetTransitionBurkina, blocChronologie, validerChronologie,
  detecterDerive,
  buildUserPrompt, cleanNarration, cleanIncrustation, phrasesDeRemplissage,
  figureFiable, nombresEnChiffres,   // exposés pour les tests
  reetiqueterSections, // exposé pour les tests (correctif 4)
  normalize, ramasserPlans,   // exposés pour les tests (round 2)
  estAlerteAccroche, accrocheAcceptable, reecritAccroche,   // round 4 (tests + pipeline)
  validateScript, validateSubjectFidelity, repairPrompt, mergeScript, updateScriptStats, accrocheCreuse, accrochePlate,
};
