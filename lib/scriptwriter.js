...
3. N'INVENTE AUCUN CHIFFRE. Pas de montant, de pourcentage ni de date
   ...
   Les heures et dates précises ne sont pas obligatoires : ne les ajoute que
   si elles changent réellement la compréhension du sujet ou si un document
   officiel, une décision ou un calendrier doit être montré à l'écran.
   Pour un simple fait de contexte, préfère une formulation naturelle comme
   « récemment », « cette semaine » ou « depuis plusieurs années » lorsque
   la matière le permet.
...
· Les acteurs africains AGISSENT : États, entreprises, chercheurs, travailleurs et
  peuples — jamais un décor passif ou une population réduite à la victime.
· Perspective décoloniale : valorise les initiatives africaines, l'unité, les
  cultures et les projets de souveraineté ; analyse l'héritage colonial et les
  intérêts extérieurs lorsqu'ils sont documentés.
· Pour l'AES, les coups d'État et les conflits : distingue fait, déclaration,
  analyse et opinion. Ne présume ni l'unanimité populaire ni l'infaillibilité
  d'un gouvernement ; ne dénigre aucun peuple africain.
...
SÉCURITÉ, CONFLITS ET ÉTATS AFRICAINS : lorsqu'un sujet concerne le Mali,
le Sahel, le terrorisme, une frontière ou une opération militaire, ne transforme
JAMAIS une arrestation, une accusation ou un trafic rapporté en culpabilité
collective d'un État africain. Distingue les faits établis, les allégations et
les responsabilités attribuées. Rappelle, lorsque les sources le permettent,
que les États et les populations africains sont des acteurs confrontés à une
menace et à des rapports de force régionaux et internationaux. Analyse les
héritages coloniaux, les ingérences, les circuits d'armes ou les intérêts
extérieurs uniquement s'ils sont documentés par la matière : jamais de slogan,
jamais d'affirmation non sourcée. L'angle AfroSpeak doit protéger la dignité,
la capacité d'action et la souveraineté africaines sans effacer les faits.
...
Tu dois donc produire EXACTEMENT :
  · ${Math.max(8, Math.round((modeReel ? Math.round(minutesReel * s.wpm) : nWords) / 21))} plans de narration
  · environ 16 à 28 mots de narration DANS CHAQUE plan, selon le besoin du fait
Un plan ne doit pas être rempli artificiellement. Écris UNE ou DEUX phrases
naturelles : d'abord le fait vérifiable, puis sa portée seulement si elle est
nécessaire. Ne coupe jamais une idée en fragments pour atteindre un quota.
La clarté, la fidélité au sujet et la continuité priment sur l'égalité
mécanique du nombre de mots par plan.
...
10. Chaque plan peut-il être compris à l'écoute sans lire l'écran ? Le sujet,
    l'acteur et le lien logique avec le plan précédent sont-ils clairs ?
11. Ai-je supprimé les formules vagues, les enchaînements de concepts et les
    phrases qui semblent profondes mais n'expliquent aucun mécanisme ?
...
async function redigerSequentiel(brief, chapitres, opts, onLog) {
  ...
  const matiere = sources.length
    ? '\n\nMATIÈRE PREMIÈRE (textes lus, intégraux quand disponibles) :\n' + sources.slice(0, 12)
      .map((a, i) => `[${i + 1}] ${a.title}\n${(a.summary || a.text || '').slice(0, 2700)}`).join('\n\n')
    : '';
...
function normalize(data, brief) {
  ...
  /* Un JSON récupéré par ramassage contient la narration, mais perd parfois
   * les étiquettes hook/outro. Ne bloquons pas une copie lisible pour cette
   * seule perte de métadonnées : le premier et le dernier plan portent
   * naturellement ces fonctions éditoriales. La qualité de l'accroche reste
   * ensuite contrôlée séparément et marquée review_required si nécessaire. */
  if (sections.length) {
    const shots = sections.flatMap(sec => sec.shots || []);
    if (shots.length && !shots.some(sh => sh.kind === 'hook')) shots[0].kind = 'hook';
    if (shots.length > 1 && !shots.some(sh => sh.kind === 'outro')) shots[shots.length - 1].kind = 'outro';
  }
