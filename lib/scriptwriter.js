...
async function generateWithLLM(brief, onLog = () => {}) {
  ...
  const repairPrompt = brief.repair
    ? userPrompt + '\n\n── CORRECTIONS À APPORTER AU SCRIPT PRÉCÉDENT ──\n' + brief.repair
      + '\n\nReproduis le script ENTIER avec les corrections, dans le même format JSON.'
    : userPrompt;
  const messages = [
    { role: 'system', content: SYSTEM },
...
function repairPrompt(script, issues = [], options = {}) {
  ...
  if (isShort) {
    instructions.push('Développe le corps avec 2 faits supplémentaires.');
  } else if (isLong) {
    instructions.push('Raccourcis le développement pour respecter la durée cible.');
  }
