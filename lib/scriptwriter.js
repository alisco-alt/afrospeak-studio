...
function estimateShots(minutes, style) {
   ...
   * gratuits et la rédaction séquentielle prend le relais. */
...
passant. Ne fusionne jamais deux actualités distinctes dans la même vidéo.`;
  ...
  const minutesReel = modeReel ? Math.min(minutes, 1.5) : minutes;
...
DURÉE CIBLE : ${minutesReel} minutes${modeReel && minutes > 1.5 ? ' (mode REEL — plafonné à 1:30)' : ''}, soit environ ${modeReel ? Math.round(minutesReel * s.wpm) : nWords} mots de narration au total.
...
Compte tes mots plan par plan avant de répondre. Un plan de 5 mots est un
échec ; un plan de 40 mots aussi. Cette règle prime sur toute autre
considération de concision.
...
Compte les mots plan par plan. Un plan de 5 à 10 mots est un ÉCHEC :
mesuré en production, des chapitres rendaient 12 mots par plan là où
19 étaient attendus, et la vidéo finissait à 108 mots au lieu de 220.
...
async function generateWithLLM(brief, onLog = () => {}) {
  ...
  if (st.ready && st.ollama && st.ollama.available && st.ollama.best) {
    ...
    onLog('Rédaction par IA (cascade cloud : Groq / OpenRouter)…');
  }
...
function hasFigureOrNumber(shot) {
  ...
  if (/(milliards?|millions?|pours*cent)/i.test(text)) return true;
  return false;
}
...
module.exports = {
  ...
  validateScript, repairPrompt, mergeScript, updateScriptStats,
};
