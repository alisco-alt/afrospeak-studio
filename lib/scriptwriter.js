...
La narration du TOUT PREMIER plan est exactement la phrase "hook" : rien ne la précède. "figure" seulement pour kind="data" (sinon null).
...
const ETIQUETTES = String.raw`intro(?:duction)?|accroche|hook|contexte|d[ée]veloppement|corps|body`
  + String.raw`|transition|analyse|explication|conclusion|outro|synth[èe]se|r[ée]sum[ée]`
  + String.raw`|chapitre|partie|section|plan|s[ée]quence|plan\s*\d+|cta|appel\s*[àa]\s*l['’]action`;
...
function cleanIncrustation(t) {
  ...
  // Étiquette seule (éventuellement numérotée) → rien à afficher
...
module.exports = {
  ...
  buildUserPrompt, cleanNarration, cleanIncrustation,   // exposés pour les tests
};
