...
async function upload(filePath, key, { contentType } = {}) {
  ...
  try {
    ...
    // ★ FLUX plutôt que buffer : readFileSync chargerait la vidéo entière
    // en mémoire (50 Mo et plus), ce qui suffit à faire tomber un conteneur
    // de 512 Mo. Un ReadStream ne garde que quelques kilo-octets à la fois.
    if (size > 4 * 1024 * 1024) {
      ...
      const upload = new Upload({
        ...
        params: {
          ...
          Body: fs.createReadStream(filePath, { highWaterMark: 512 * 1024 }),
          ContentType: ct,
          CacheControl: 'public, max-age=31536000, immutable',
        },
        queueSize: 1,            // un fragment à la fois : mémoire minimale
        partSize: 5 * 1024 * 1024,
      });
      await upload.done();
    } else {
      // Petits fichiers (SRT, miniature) : un simple buffer suffit
      const { PutObjectCommand } = require('@aws-sdk/client-s3');
      await client.send(new PutObjectCommand({
        Bucket: bucket, Key: key, Body: fs.readFileSync(filePath), ContentType: ct,
        CacheControl: 'public, max-age=31536000, immutable',
      }));
    }

    const url = publicBase ? `${publicBase}/${key}` : await signedUrl(key);
    log.info(`téléversé : ${key} (${(size / 1e6).toFixed(1)} Mo${size > 4e6 ? ', en flux' : ''})`);
...
async function uploadBundle(result, userId, videoId) {
  ...
  // Téléversements STRICTEMENT séquentiels : quatre flux simultanés
  // multiplieraient les tampons réseau sur une instance déjà contrainte.
  if (result.video && fs.existsSync(result.video)) {
    const r = await upload(result.video, `${prefix}/video.mp4`);
    out.video_url = r.url; out.storage = r.storage;
  }
  if (result.thumbnail && fs.existsSync(result.thumbnail)) {
    out.thumb_url = (await upload(result.thumbnail, `${prefix}/thumb.jpg`)).url;
  }
  if (result.srt && fs.existsSync(result.srt)) {
    out.srt_url = (await upload(result.srt, `${prefix}/captions.srt`)).url;
  }
  if (result.metadata && fs.existsSync(result.metadata)) {
    out.meta_url = (await upload(result.metadata, `${prefix}/youtube.txt`)).url;
  }
  return out;
}
 ...
 */
function pruneLocal({ keep = 6, maxBytes = 1.5e9, protect = [] } = {}) {
  /* `protect` permet à l'appelant de déclarer des livrables encore en
   * cours de téléchargement. Les fichiers protégés ne doivent jamais être
   * supprimés, même si la limite de disque est dépassée. */
  const protectedNames = new Set((protect || []).map(x => path.basename(String(x))));
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      if (protectedNames.has(files[i].f)) continue;
