...
/* ── TIMEOUT EDGE-TTS : 75 s (correctif 6, run « Sénégal / FMI ») ────
 * edge-tts peut rester suspendu sans flux (réseau instable vers les
 * endpoints Microsoft) : le process mourait à 120 s, et CHAQUE plan
 * suspendu coûtait deux minutes de production — d'où les ~7 min
 * perdues du run. Aligné sur le timeout LLM (75 s) : l'échec remonte
 * plus tôt, la re-tentative repart plus tôt. Ajustable :
 * EDGE_TTS_TIMEOUT_MS. */
function runHelper(payload, timeout = Number(process.env.EDGE_TTS_TIMEOUT_MS) || 75000) {
