...
const { logger, sha1 } = require('./util');
 ...
 */
async function inspire({ stream = 'all', limit = 12, refresh = false } = {}) {
  if (stream === 'actualite') {
    const r = await trendingNews({ limit, refresh });
    return { topics: r.topics, cached: r.cached, offline: r.offline, streams: ['actualite'] };
  }
  if (stream === 'conscientisation') {
    const r = await consciousness({ limit, fresh: refresh });
    return { topics: r.topics, themes: r.themes, streams: ['conscientisation'] };
  }
...
module.exports = {
  inspire, trendingNews, consciousness,
