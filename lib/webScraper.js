...
/* MODE QUALITÉ : Playwright pilote un vrai navigateur ; sur un site lourd
 * (réseaux sociaux, sites de presse), `networkidle` en 30 s est souvent
 * trop court et la page est abandonnée alors qu'elle allait se charger.
 * Ce multiplicateur allonge toutes les navigations d'un facteur 2,5.
 * AFROSPEAK_RAPIDE=1 revient aux délais d'origine. */
const _WS_MULT = process.env.AFROSPEAK_RAPIDE === '1' ? 1
  : (Number(process.env.SCRAPER_TIMEOUT_MULT) || 2.5);
const _wt = ms => Math.round(ms * _WS_MULT);
...
async function captureChart(url, opts = {}) {
  ...
  try {
    await page.setViewportSize({ width, height });
    await page.goto(url, { waitUntil: 'networkidle', timeout: _wt(30000) });
    if (waitSelector) {
      await page.waitForSelector(waitSelector, { timeout: _wt(15000) }).catch(() => {});
    }
    await sleep(delay);
    await _waitForMedia(page);
...
async function scrollSocial(url, opts = {}) {
  ...
  try {
    await page.goto(url, { waitUntil: 'domcontentloaded', timeout: _wt(30000) });
    const items = [];
    const screenshots = [];
...
async function captureYouTubeFrame(url, timestamps = [10, 30, 60], opts = {}) {
  ...
  try {
    await page.setViewportSize({ width, height });
    await page.goto(embedUrl, { waitUntil: 'networkidle', timeout: _wt(30000) });
    await sleep(2000);
...
async function searchDuckDuckGoImages(query, opts = {}) {
  ...
  try {
    ...
    await page.goto(searchUrl, { waitUntil: 'domcontentloaded', timeout: _wt(30000) });
    await sleep(1500);
...
async function extractOGImages(url) {
  ...
  try {
    await page.goto(url, { waitUntil: 'domcontentloaded', timeout: _wt(20000) });
...
async function downloadPageMedia(url, opts = {}) {
  ...
  for (const media of filtered) {
    try {
      ...
      const res = await fetchBuf(media.url, { timeout: _wt(60000), retries: 1 });
...
/**
 * État réel de l'outil.
 *
 * ── CORRECTION D'UN DIAGNOSTIC MENSONGER ──
 * L'ancienne version ne testait que la présence du MODULE npm et
 * annonçait « Playwright opérationnel » même quand le NAVIGATEUR
 * n'était pas téléchargé. Le pipeline plaçait alors `webScraper` en
 * tête de sa cascade de sources, chaque appel retournait zéro image
 * en 0,3 s, et la production basculait sur des illustrations IA sans
 * qu'aucun message n'explique pourquoi.
 *
 * On vérifie donc que l'exécutable Chromium existe vraiment.
 */
let _cacheEtat = null;
function toolStatus() {
  if (_cacheEtat) return _cacheEtat;
  const moduleOk = _tryRequire();
  let navigateurOk = false;
  let chemin = null;
  if (moduleOk) {
    try {
      const pw = require('playwright');
      chemin = pw.chromium.executablePath();
      navigateurOk = !!chemin && require('fs').existsSync(chemin);
    } catch (e) { navigateurOk = false; }
  }
  const available = moduleOk && navigateurOk;
  _cacheEtat = {
    available,
    browser: available ? 'chromium' : null,
    version: available ? 'playwright' : null,
    note: available
      ? 'Playwright opérationnel — scraping web dynamique activé'
      : (moduleOk
        ? 'Navigateur Chromium absent — lancez : npx playwright install chromium'
        : 'Playwright non installé — npm install playwright && npx playwright install chromium'),
  };
  if (!available) log.warn(_cacheEtat.note);
  return _cacheEtat;
}
