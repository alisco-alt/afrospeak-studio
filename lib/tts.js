...
function clampWords(words, duration) {
  ...
  return words.map(w => {
    /* Un timing Edge placé exactement sur la fin du MP3 pouvait être
     * tronqué en start=end après le clamp, ce qui alimentait les rapports
     * qualité avec une erreur par plan et produisait parfois un mot invisible.
     * On réserve toujours 50 ms au mot, sans jamais dépasser la durée audio. */
    let start = Math.max(0, Math.min(Math.max(0, d - 0.05), Number(w.start) || 0));
    let end = Math.min(d, Math.max(Number(w.end) || 0, start + 0.05));
    if (end <= start) {
      start = Math.max(0, d - 0.05);
      end = d;
    }
    return { word: w.word, start: +start.toFixed(3), end: +end.toFixed(3) };
  });
}
