...
let edgeReady = false;
try {
  require('./edgetts').available().then(v => { edgeReady = v; }).catch(() => {});
...
async function speak(text, opts = {}) {
  ...
  const cacheKey = sha1([text, provider, lang, voiceId, speed].join('|'));
  ...
  const order = provider === 'auto' ? availableProviders() : [provider, ...availableProviders().filter(p => p !== provider)];
