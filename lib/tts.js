...
/**
 * Moteur vocal local configurable (XTTS-v2, OpenVoice, CosyVoice, etc.).
 * Le studio ne télécharge ni n'envoie la voix de l'utilisateur : un petit
 * adaptateur local est lancé par l'utilisateur et écrit le fichier audio.
 *
 * Configuration :
 *   VOICE_CLONE_ENABLED=1
 *   VOICE_CLONE_BIN=python3
 *   VOICE_CLONE_ARGS='["scripts/voice_clone.py","--text","{text}","--out","{out}","--speaker","{voice}"]'
 *   VOICE_CLONE_REFERENCE=/chemin/vers/reference.wav
 *
 * L'adaptateur doit produire un fichier lisible par FFmpeg. Les timings
 * restent estimés, comme pour OpenAI/Google, puis sont recalés sur la durée.
 */
async function localCloneSynth(text, outFile, { voice = '' } = {}) {
  const bin = process.env.VOICE_CLONE_BIN;
  if (process.env.VOICE_CLONE_ENABLED !== '1' || !bin) {
    throw new Error('clonage local non configuré');
  }
  let args;
  try { args = JSON.parse(process.env.VOICE_CLONE_ARGS || '[]'); }
  catch (e) { throw new Error('VOICE_CLONE_ARGS doit être un tableau JSON'); }
  const repl = value => String(value || '')
    .replaceAll('{text}', text)
    .replaceAll('{out}', outFile)
    .replaceAll('{voice}', voice || process.env.VOICE_CLONE_REFERENCE || '');
  args = args.map(repl);
  await new Promise((resolve, reject) => {
    const child = spawn(bin, args, {
      cwd: process.cwd(),
      env: { ...process.env, VOICE_CLONE_TEXT: text, VOICE_CLONE_OUT: outFile,
        VOICE_CLONE_REFERENCE: process.env.VOICE_CLONE_REFERENCE || '' },
    ...
    child.on('close', code => code === 0 ? resolve() : reject(new Error(
      `moteur vocal local (${code}) ${err.slice(0, 180)}`)));
  });
  if (!fs.existsSync(outFile)) throw new Error("le moteur vocal local n'a pas produit de fichier");
...
function availableProviders() {
  ...
  if (process.env.VOICE_CLONE_ENABLED === '1' && process.env.VOICE_CLONE_BIN) list.push('localclone');
...
async function speak(text, opts = {}) {
  ...
  /* `rate` DOIT figurer dans la cle : la prosodie fait varier le debit
   * plan par plan. Sans lui, deux plans au meme texte mais au debit
   * different se partageraient le meme MP3, et la modulation serait
   * silencieusement perdue. */
  const cacheKey = sha1([
    text, provider, lang, voiceId, speed,
    opts.style || '', opts.lockVoice ? 'lock' : '', opts.rate || '',
  ].join('|'));
...
async function listVoices() {
  ...
  if (process.env.VOICE_CLONE_ENABLED === '1' && process.env.VOICE_CLONE_BIN) {
    out.unshift({ provider: 'localclone', id: process.env.VOICE_CLONE_REFERENCE || 'local-reference', name: 'Voix AfroSpeak locale — clonage autorisé' });
  }
