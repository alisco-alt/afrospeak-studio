...
function reinitialiserEchecs() { _echecsFournisseur.clear(); }
...
async function download(asset, opts = {}) {
  ...
  const dlTimeout = isVideo
    ...
    : Number(process.env.DOWNLOAD_IMAGE_TIMEOUT) || 12000;
  const res = await fetchBuf(asset.url, {
    timeout: dlTimeout, retries: 1, maxBytes: 120 * 1024 * 1024, headers: enTetes,
  });
...
async function acquire(queries, opts = {}) {
  ...
  for (const cand of liste) {
     ...
     * qui répondent. */
    if (_fournisseurMort(cand.provider)) { exclude.add(cand.url); continue; }
