...
function droitsEtablis(asset) {
  ...
  if (/usage éditorial|droits à vérifier|à vérifier/.test(l)) return false;
...
async function search(query, opts = {}) {
  ...
  const _webAutorise = process.env.ALLOW_UNLICENSED_WEB_ASSETS === '1';
  const _eligiblesDroits = _webAutorise ? uniq : uniq.filter(droitsEtablis);
