 ...
 * Compte les échecs CONSÉCUTIFS de chaque source. Au-delà du seuil, la
 * source est considérée en panne et n'est plus sollicitée de la
 * production : c'est ce qui empêche un Pexels injoignable de dévorer le
 * budget média plan après plan.
 * Un succès remet le compteur à zéro : une source qui se rétablit
 * redevient utilisable.
 * `MEDIA_ECHECS_MAX=0` désactive complètement le mécanisme. */
const _echecsFournisseur = new Map();
const _MAX_ECHECS = Number.isFinite(Number(process.env.MEDIA_ECHECS_MAX))
  ? Number(process.env.MEDIA_ECHECS_MAX) : 3;
...
function _noterEchec(provider, message) {
  ...
  /* Une définition insuffisante n'est PAS une panne du fournisseur :
   * la source répond, c'est le média qui ne convient pas. On ne compte
   * que les défaillances de transport. */
  if (/définition insuffisante|media illisible|média illisible/i.test(String(message || ''))) return;
  const k = _cleFournisseur(provider);
  const n = (_echecsFournisseur.get(k) || 0) + 1;
  _echecsFournisseur.set(k, n);
  if (n === _MAX_ECHECS) {
    log.warn(`${provider} : ${n} échecs consécutifs — source écartée pour cette production`);
  }
}
...
function _fournisseurMort(provider) {
  ...
  return (_echecsFournisseur.get(_cleFournisseur(provider)) || 0) >= _MAX_ECHECS;
}
...
function reinitialiserEchecs() { _echecsFournisseur.clear(); _msGaspilles = 0; }
