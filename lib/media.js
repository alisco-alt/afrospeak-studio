...
const mediaCatalog = require('./mediaCatalog'); // alias pays/capitales/langues
const {
...
/* ── DÉLAIS ADAPTÉS À LA LIAISON RÉELLE ──
 *
 * Les timeouts de ce fichier (8 à 20 s, sans aucun réessai) ont été
 * calibrés sur une liaison rapide. Diagnostic exécuté chez l'utilisateur :
 *
 *   Wikimedia    0 résultat en 10,6 s        (ici : 12 résultats en 0,3 s)
 *   Openverse    0 résultat en 10,5 s
 *   Archive.org  0 résultat en 10,0 s  « This operation was aborted »
 *   DuckDuckGo   0 résultat en  8,0 s  « This operation was aborted »
 *   Bing        19 résultats en  6,7 s
 *
 * Des échecs tombant PILE sur la valeur du timeout ne signalent pas des
 * sources muettes : ce sont des abandons prématurés. Sur cette machine,
 * ouvrir example.com prend déjà 6,5 s — soit vingt fois plus qu'ici.
 *
 * Conséquence en production : une seule banque sur sept répondait, d'où
 * 22 visuels sur 25 générés par IA et la vidéo « qui n'illustre pas le
 * propos ». La cause n'était ni les sources ni l'IA : c'était l'attente.
 *
 * `MEDIA_TIMEOUT_MULT` multiplie tous les délais (2 par défaut) et
 * `MEDIA_RETRIES` autorise un réessai — un hoquet ne condamne plus une
 * source pour toute la vidéo. Sur une liaison rapide, le surcoût est nul :
 * une requête qui aboutit en 0,3 s n'attend jamais son plafond. */
/* MODE QUALITÉ : multiplicateur 2 → 3 par défaut.
 * Diagnostic mesuré chez l'utilisateur : Wikimedia et Openverse
 * répondaient 0 résultat en 21,5 s — soit exactement le plafond ×2, donc
 * des TIMEOUTS et non des sources muettes (la même requête aboutit en
 * 0,3 s depuis un réseau rapide). ×3 laisse ces sources répondre.
 * Sur une liaison rapide le surcoût est nul : une requête qui aboutit en
 * 0,3 s n'attend jamais son plafond. */
const _TMULT = Number(process.env.MEDIA_TIMEOUT_MULT)
  || (process.env.AFROSPEAK_RAPIDE === '1' ? 2 : 3);
const _RETRIES = Number(process.env.MEDIA_RETRIES);
/** Délai d'attente ajusté à la liaison. */
function _to(ms) { return Math.round(ms * _TMULT); }
/** Nombre de réessais : 1 par défaut, 0 si l'utilisateur l'impose. */
function _rt() { return Number.isFinite(_RETRIES) ? _RETRIES : 1; }

/* ── DISJONCTEUR PAR FOURNISSEUR — MISE EN PAUSE, JAMAIS EXCLUSION ────
 *
 * CORRECTION D'UNE RÉGRESSION QUE J'AI INTRODUITE.
 * La version précédente écartait une source DÉFINITIVEMENT après 3
 * échecs consécutifs. Mesuré sur le run « cacao » : Pexels a été écarté
 * dès le PLAN 4, pour les 56 plans suivants — alors que c'est la
 * principale source de VIDÉOS réelles. Wikimedia Commons a subi le même
 * sort. Le garde-fou censé protéger le budget a donc directement causé
 * l'absence de vidéos que l'utilisateur reproche.
 *
 * Deux défauts précis :
 *   1. les 3 échecs peuvent survenir sur UN SEUL plan (un plan lance
 *      plusieurs requêtes), donc la sanction tombe immédiatement ;
 *   2. l'exclusion était définitive, alors qu'un « fetch failed » est le
 *      plus souvent un hoquet réseau de quelques secondes.
 *
 * Nouvelle règle :
 *   · la source est mise en PAUSE (60 s), pas exclue ;
 *   · les sources STRUCTURANTES (Pexels, Pixabay, Wikimedia) — celles
 *     qui portent la vidéo et l'archive libre — ont un seuil bien plus
 *     élevé et une pause plus courte : les perdre coûte la qualité de
 *     toute la production ;
 *   · un succès remet le compteur à zéro.
 * `MEDIA_ECHECS_MAX=0` désactive le mécanisme.
 */
const _echecsFournisseur = new Map();
const _pauseFournisseur = new Map();
const _MAX_ECHECS = Number.isFinite(Number(process.env.MEDIA_ECHECS_MAX))
  ? Number(process.env.MEDIA_ECHECS_MAX) : 6;

/* Sources sans lesquelles la vidéo perd sa matière : on ne les lâche
 * qu'après une défaillance longue et répétée. */
const _STRUCTURANTES = /pexels|pixabay|wikimedia|commons|openverse/i;

function _cleFournisseur(p) {
  return String(p || 'inconnu').toLowerCase().trim();
}
function _seuilPour(provider) {
  return _STRUCTURANTES.test(String(provider || '')) ? _MAX_ECHECS * 3 : _MAX_ECHECS;
}
function _pauseMsPour(provider) {
  return _STRUCTURANTES.test(String(provider || '')) ? 30000 : 90000;
}
function _noterEchec(provider, message) {
  if (_MAX_ECHECS <= 0) return;
  /* Une définition insuffisante n'est PAS une panne du fournisseur : la
   * source répond, c'est le média qui ne convient pas. Idem pour un
   * média illisible. On ne compte que les défaillances de transport. */
  if (/définition insuffisante|media illisible|média illisible|trop petite/i
    .test(String(message || ''))) return;
  const k = _cleFournisseur(provider);
  const n = (_echecsFournisseur.get(k) || 0) + 1;
  _echecsFournisseur.set(k, n);
  if (n >= _seuilPour(provider)) {
    const ms = _pauseMsPour(provider);
    _pauseFournisseur.set(k, Date.now() + ms);
    _echecsFournisseur.set(k, 0);          // on repart à zéro après la pause
    log.warn(`${provider} : ${n} échecs — mise en pause ${Math.round(ms / 1000)}s `
      + '(la source reste utilisable ensuite)');
  }
}
function _noterSucces(provider) {
  const k = _cleFournisseur(provider);
  if (_echecsFournisseur.get(k)) _echecsFournisseur.set(k, 0);
  if (_pauseFournisseur.has(k)) _pauseFournisseur.delete(k);
}
/** La source est-elle en pause à cet instant ? (jamais définitif) */
function _fournisseurMort(provider) {
  if (_MAX_ECHECS <= 0) return false;
  const k = _cleFournisseur(provider);
  const jusqua = _pauseFournisseur.get(k) || 0;
  if (!jusqua) return false;
  if (Date.now() >= jusqua) { _pauseFournisseur.delete(k); return false; }
  return true;
}

/* ── BUDGET GLOBAL D'ATTENTE GASPILLÉE ────────────────────────────────
 * Le disjoncteur par fournisseur ne suffit pas : chaque site web croisé
 * (portabidjan.ci, sosghana.org, africaintelligence.fr…) possède son
 * propre compteur, et il en faut trois avant de l'écarter. Sur une
 * recherche qui brasse des dizaines de domaines inconnus, on repaie donc
 * le plein délai à chaque nouveau site.
 * Mesuré : ~30 timeouts pleins = 15 minutes perdues, sur une phase
 * visuelle de 42 minutes finalement court-circuitée.
 * On plafonne donc le TEMPS TOTAL réellement gaspillé en échecs. Au-delà,
 * seules les sources déjà éprouvées (banques avec clé) sont sollicitées :
 * le scraping opportuniste s'arrête, mais la production continue. */
let _msGaspilles = 0;
const _MAX_MS_GASPILLES = Number(process.env.MEDIA_MAX_MS_ECHECS) || 240000;
const _SOURCES_FIABLES = new Set(['pexels', 'pixabay', 'unsplash', 'wikimedia commons', 'openverse']);
function _scrapingEpuise(provider) {
  if (_msGaspilles < _MAX_MS_GASPILLES) return false;
  return !_SOURCES_FIABLES.has(_cleFournisseur(provider));
}
/** Remet tous les compteurs à zéro (début d'une nouvelle production). */
function reinitialiserEchecs() {
  _echecsFournisseur.clear();
  _pauseFournisseur.clear();
  _msGaspilles = 0;
  /* Les coupe-circuits historiques doivent repartir propres eux aussi,
   * sinon une production hérite des pannes de la précédente. */
  _pexelsFails = 0;
  _pexelsPauseJusqua = 0;
  _dlFails.clear();
  _dlDisabled.clear();
}
...
/* ── PEXELS : PAUSE, PLUS JAMAIS DÉSACTIVATION DÉFINITIVE ────────────
 * Ce coupe-circuit désactivait Pexels pour TOUTE la production après 3
 * échecs d'affilée. C'est la cause profonde du « aucune vidéo » : Pexels
 * est la principale source de clips libres, et 3 hoquets réseau en début
 * de production suffisaient à s'en priver pour les 60 plans suivants.
 * Le journal du run « cacao » le montre : Pexels écarté dès le plan 4.
 *
 * L'intention initiale était juste — éviter 40 minutes d'attentes — mais
 * la sanction était disproportionnée. On passe donc à une PAUSE de 45 s,
 * qui laisse retomber un rate-limit sans sacrifier la source. Un succès
 * remet le compteur à zéro. */
let _pexelsFails = 0;
let _pexelsPauseJusqua = 0;
const _PEXELS_PAUSE_MS = Number(process.env.PEXELS_PAUSE_MS) || 45000;
function _pexelsEnPause() {
  if (!_pexelsPauseJusqua) return false;
  if (Date.now() >= _pexelsPauseJusqua) { _pexelsPauseJusqua = 0; _pexelsFails = 0; return false; }
  return true;
}
function _pexelsEchec(motif) {
  _pexelsFails++;
  if (_pexelsFails >= 3) {
    _pexelsPauseJusqua = Date.now() + _PEXELS_PAUSE_MS;
    _pexelsFails = 0;
    log.warn(`Pexels en pause ${Math.round(_PEXELS_PAUSE_MS / 1000)}s (${motif}) — `
      + 'la source reste utilisable ensuite');
  }
}

/* ── CIRCUIT BREAKER TÉLÉCHARGEMENT (par provider) ──
 * Le circuit _pexelsFails ci-dessus protège la RECHERCHE API.
 * Celui-ci protège le TÉLÉCHARGEMENT : si un provider échoue 5 fois
 * en téléchargement (timeout, 403, corruption), on le désactive
 * pour la suite de la production. */
const _dlFails = new Map();   // provider -> count
const _dlDisabled = new Set();
const DL_FAIL_THRESHOLD = 5;
function dlTrackFail(provider) {
  if (!provider) return;
  const c = (_dlFails.get(provider) || 0) + 1;
  _dlFails.set(provider, c);
  if (c >= DL_FAIL_THRESHOLD && !_dlDisabled.has(provider)) {
    _dlDisabled.add(provider);
    log.warn(`téléchargements ${provider} désactivés après ${c} échecs`);
  }
}
function dlTrackOk(provider) {
  if (provider) _dlFails.set(provider, 0);
}
function dlIsDisabled(provider) {
  return provider && _dlDisabled.has(provider);
}

async function searchPexels(q, { type = 'image', perPage = 15, orientation } = {}) {
  if (_pexelsEnPause()) return [];
  ...
  let res;
  try {
    res = await fetchBuf(u.toString(), { headers: { authorization: key }, timeout: _to(20000), retries: _rt() });
  } catch (e) {
    _pexelsEchec('réseau');
  ...
  if (type === 'video') {
    return (d.videos || []).map(v => {
      /* Même raisonnement que pour Pixabay : le tri décroissant plaçait
       * le 3840×2160 60 fps en tête, soit des dizaines de Mo par clip
       * pour un montage qui travaille en 1620 px. On vise la variante
       * juste suffisante — la plus PETITE au-dessus de 1600 px — et on
       * ne retombe sur la plus grande que si rien d'autre n'existe. */
      const files = (v.video_files || []).filter(f => f.file_type === 'video/mp4')
        .sort((a, b) => (a.width || 0) - (b.width || 0));
      const best = files.find(f => (f.width || 0) >= 1600)
...
async function searchPixabay(q, { type = 'image', perPage = 15 } = {}) {
  ...
  const res = await fetchBuf(u.toString(), { timeout: _to(20000), retries: _rt() });
  if (!res.ok) return [];
  const d = res.json();
  return (d.hits || []).map(h => {
    if (type === 'video') {
      /* ── PRENDRE LA VARIANTE UTILE, PAS LA PLUS LOURDE ──────────────
       * On demandait `large` en priorité : du 3840×2160 pesant jusqu'à
       * 98 Mo pour UN clip. Résultat mesuré : `fetchBuf` rendait
       * « response too large », l'erreur remontait en « Pixabay média
       * illisible », et AUCUNE vidéo n'était jamais retenue — d'où les
       * « 0 videos » du dernier run.
       * Or le montage travaille en 1620 px de large : le 4K est
       * immédiatement réduit, donc intégralement téléchargé pour rien.
       * Tailles réelles d'un même clip (API Pixabay) :
       *   large  3840×2160  34,3 Mo
       *   small  1920×1080   8,0 Mo   <- suffisant, 4× plus léger
       *   tiny   1280×720    4,4 Mo
       * On privilégie donc `small`, avec `medium` en secours si absent. */
      const v = h.videos && (h.videos.small || h.videos.medium
        || h.videos.tiny || h.videos.large);
...
async function searchUnsplash(q, { perPage = 15, orientation } = {}) {
  ...
  const res = await fetchBuf(u.toString(), {
    headers: { authorization: 'Client-ID ' + key }, timeout: _to(10000), retries: _rt(),
  });
  if (!res.ok) return [];
  const d = res.json();
...
/** Coverr : clips curatés HD/4K, API officielle avec clé et attribution. */
async function searchCoverr(q, { perPage = 10, orientation } = {}) {
  const key = config.keys().coverr;
  if (!key) return [];
  try {
    const u = new URL('https://api.coverr.co/videos');
    u.searchParams.set('query', q);
    u.searchParams.set('page_size', String(Math.min(20, perPage)));
    u.searchParams.set('urls', 'true');
    const res = await fetchBuf(u.toString(), {
      headers: { authorization: 'Bearer ' + key, accept: 'application/json' },
      timeout: _to(15000), retries: _rt(),
    });
    if (!res.ok) return [];
    const hits = (res.json().hits || []);
    return hits.map(v => {
      const vertical = !!v.is_vertical;
      if (orientation === 'portrait' && !vertical) return null;
      if (orientation === 'landscape' && vertical) return null;
      const urls = v.urls || {};
      const url = urls.mp4_download || urls.mp4 || urls.mp4_preview;
      if (!url) return null;
      return {
        kind: 'video', provider: 'Coverr', url, thumb: v.thumbnail || v.poster,
        width: Number(v.max_width) || 0, height: Number(v.max_height) || 0,
        duration: Number(v.duration) || undefined, author: 'Coverr',
        authorUrl: 'https://coverr.co/', pageUrl: 'https://coverr.co/videos/' + (v.id || ''),
        license: 'Coverr API License (attribution requise)', licenseUrl: 'https://api.coverr.co/docs',
        requiresAttribution: true, title: v.title || v.description || q,
        id: 'coverr_' + v.id,
      };
    }).filter(Boolean);
  } catch (e) { log.warn('coverr', String(e.message || '').slice(0, 70)); return []; }
}



/* ---------------------------- Shutterstock ---------------------------- */
/* Shutterstock : vidéos et images premium. OAuth2 client_credentials.
 * Clés : SHUTTERSTOCK_CONSUMER_KEY, SHUTTERSTOCK_CONSUMER_SECRET (env).
 * Le token est mis en cache (1h) pour éviter un appel OAuth par requête. */
let _sstToken = null;
let _sstTokenExpiry = 0;

async function _sstGetToken() {
  if (_sstToken && Date.now() < _sstTokenExpiry) return _sstToken;
  const cid = process.env.SHUTTERSTOCK_CONSUMER_KEY;
  const csec = process.env.SHUTTERSTOCK_CONSUMER_SECRET;
  if (!cid || !csec) return null;
  try {
    const res = await fetchBuf('https://api.shutterstock.com/v2/oauth/access_token', {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        client_id: cid,
        client_secret: csec,
        scope: 'licenses.view media.view',
      }),
      timeout: _to(15000), retries: _rt(),
    });
    if (!res.ok) return null;
    const d = res.json();
    _sstToken = d.access_token;
    _sstTokenExpiry = Date.now() + (d.expires_in || 3600) * 1000 - 60000; // -1min marge
    log.info('Shutterstock token obtenu (expire dans ' + Math.round((d.expires_in || 3600) / 60) + ' min)');
    return _sstToken;
  } catch (e) {
    log.warn('Shutterstock OAuth échoué: ' + String(e.message || '').slice(0, 60));
    return null;
  }
}

async function searchShutterstock(q, { type = 'image', perPage = 15, orientation } = {}) {
  const token = await _sstGetToken();
  if (!token) return [];
  ...
  else if (orientation === 'landscape') u.searchParams.set('orientation', 'horizontal');
  ...
  try {
    ...
    if (!res.ok) {
      log.warn('Shutterstock', res.status);
      return [];
    }
    const d = res.json();
    if (isVideo) {
      return (d.data || []).map(v => {
        ...
        const preview = assets.preview_mp4 || {};
        const thumb = assets.thumb_jpgs || {};
        return {
          kind: 'video', provider: 'Shutterstock',
          url: preview.url || '',
          thumb: (thumb && thumb[0] && thumb[0].url) || null,
          width: preview.width || 1920, height: preview.height || 1080,
          duration: v.duration || 0,
          author: v.contributor && v.contributor.id ? 'Shutterstock/' + v.contributor.id : 'Shutterstock',
          authorUrl: '', pageUrl: '',
          license: 'Shutterstock License (licence éditoriale requise)',
          licenseUrl: 'https://www.shutterstock.com/license',
          requiresAttribution: false,
          title: v.description || q, id: 'sst_v_' + v.id,
        };
      }).filter(x => x.url);
    }
    return (d.data || []).map(p => {
      const assets = p.assets || {};
      const fullPreview = assets.full_jpg || assets.preview || {};
      return {
        kind: 'image', provider: 'Shutterstock',
        url: fullPreview.url || '',
        thumb: (assets.small_thumb || {}).url || '',
        width: p.width || 0, height: p.height || 0,
        author: p.contributor && p.contributor.id ? 'Shutterstock/' + p.contributor.id : 'Shutterstock',
        authorUrl: '', pageUrl: '',
        license: 'Shutterstock License (licence éditoriale requise)',
        licenseUrl: 'https://www.shutterstock.com/license',
        requiresAttribution: false,
        title: p.description || q, id: 'sst_i_' + p.id,
      };
    }).filter(x => x.url);
  } catch (e) {
    log.warn('Shutterstock search échoué: ' + String(e.message || '').slice(0, 60));
    return [];
  }
}
...
async function searchArchive(q, { perPage = 8, wantVideo = true } = {}) {
  try {
    /* ── LE TYPE « image » SEUL RATE L'ESSENTIEL DES ARCHIVES ──
     * Mesuré : `mediatype:image` renvoie 0 document sur « Thomas
     * Sankara », alors que `movies` en renvoie 5. Sur Internet Archive,
     * les photographies historiques sont massivement rangées sous
     * `texts` (livres et journaux numérisés, dont on extrait les
     * planches) ou `image`, rarement les deux. En n'interrogeant qu'un
     * seul type, on se privait de la source la plus riche en archives
     * africaines — précisément ce que le studio cherche.
     * On élargit donc aux trois types pertinents. */
    const type = wantVideo ? 'movies' : '"image" OR "texts"';
    ...
    const res = await fetchBuf(u.toString(), {
      headers: { 'user-agent': UA_NAV }, timeout: _to(10000), retries: _rt(),
    });
    if (!res.ok) return [];
    const docs = ((res.json().response || {}).docs) || [];

    /* ── LES MÉTADONNÉES SE LISENT EN PARALLÈLE ──
     * Une requête `metadata` PAR résultat, en série : sur 8 documents,
     * cela fait 8 allers-retours enchaînés. Ici la recherche prend 0,4 s,
     * mais chez l'utilisateur (liaison ~20× plus lente) l'ensemble
     * dépassait le budget et Archive.org rendait 0 résultat en 6,2 s —
     * avant même d'atteindre son plafond, donc sans message clair.
     *
     * On interroge désormais les documents de front, et on en limite le
     * nombre : au-delà de 5, le coût dépasse le bénéfice puisqu'un seul
     * visuel sera retenu par plan. */
    const aInterroger = docs.slice(0, Math.min(perPage, 5));
    const metas = await Promise.allSettled(aInterroger.map(doc =>
      fetchBuf('https://archive.org/metadata/' + encodeURIComponent(doc.identifier),
        { headers: { 'user-agent': UA_NAV }, timeout: _to(15000), retries: 0 })));

    const sorties = [];
    for (let i = 0; i < aInterroger.length; i++) {
      const doc = aInterroger[i];
      try {
        const r = metas[i];
...
async function searchNasa(q, { perPage = 8 } = {}) {
  try {
    ...
    const res = await fetchBuf(u.toString(), { timeout: _to(8000), retries: _rt() });
    if (!res.ok) return [];
...
async function ddgToken(q) {
  const res = await fetchBuf('https://duckduckgo.com/?q=' + encodeURIComponent(q) + '&iax=images&ia=images',
    { headers: { 'user-agent': UA_NAV }, timeout: _to(8000), retries: _rt() });
  if (!res.ok) return null;
...
let gdeltDernier = 0;
/* ── DISJONCTEUR GDELT ───────────────────────────────────────────────
 * Relevé sur un run complet de l'utilisateur : 20 messages
 * « gdelt saturé (429), nouvelle tentative dans 7s / 14s », soit 203 s
 * d'attente pure pour ZÉRO résultat. Le budget média (792 s) a explosé
 * et 13 plans sur 25 ont basculé d'un coup en illustration IA.
 *
 * Le rythme de 6 s entre appels et les 2 réessais étaient conçus pour
 * un GDELT ponctuellement saturé. Quand il l'est DURABLEMENT, chaque
 * plan repaie l'addition. On coupe donc la source pour le reste de la
 * production après quelques refus consécutifs : une source qui ne rend
 * rien ne doit pas consommer le temps des autres. */
let gdeltRefus = 0;
let gdeltCoupeJusqua = 0;
const GDELT_MAX_REFUS = Number(process.env.GDELT_MAX_REFUS) || 3;
let gdeltFile = Promise.resolve();     // sérialise les appels concurrents
async function searchGdelt(q, { perPage = 12, timespan = '3months' } = {}) {
  /* Disjoncteur : si GDELT a refusé plusieurs fois de suite, on ne le
   * sollicite plus pendant un moment. Rendre la main immédiatement laisse
   * le budget aux sources qui répondent. */
  if (Date.now() < gdeltCoupeJusqua) return [];
  ...
  try {
    ...
    /* ── GDELT COUTE PLUS QU'IL NE RAPPORTE ────────────────────────
     * Observé sur TOUS les journaux de production : « gdelt saturé (429),
     * nouvelle tentative dans 7s » puis « dans 14s » puis « écarté pour
     * 20 min ». Soit 21 s perdues par run, à chaque run, sans qu'aucun
     * visuel n'en soit jamais issu.
     * Mesuré ici : une recherche vidéo Pexels a mis 83 s, dont l'essentiel
     * passé à attendre GDELT.
     * On passe donc à UN SEUL essai et une coupure immédiate au premier
     * 429 : la source reste disponible quand elle fonctionne, mais elle
     * ne retarde plus la cascade quand elle est saturée.
     * GDELT_ESSAIS=3 rétablit l'ancien comportement. */
    const _gdeltEssais = Number(process.env.GDELT_ESSAIS) || 1;
    let res = null;
    for (let essai = 0; essai < _gdeltEssais; essai++) {
      ...
      if (res && res.status === 429) {
        gdeltRefus++;
        if (gdeltRefus >= GDELT_MAX_REFUS || _gdeltEssais === 1) {
          /* Saturation durable : on écarte la source pour 20 minutes.
           * Sans cela, chaque plan repayait jusqu'à 21 s d'attente pour
           * rien — 203 s perdues sur un seul run, mesuré. */
          gdeltCoupeJusqua = Date.now() + (Number(process.env.GDELT_PAUSE_MS) || 1200000);
          log.warn('gdelt saturé durablement — source écartée pour 20 min');
          return [];
        }
        if (essai < 2) {
      ...
      if (res && res.ok) gdeltRefus = 0;   // la source est revenue
      break;
    }
...
const PROVIDERS = {
  ...
  coverr: searchCoverr, shutterstock: searchShutterstock,
...
function defaultProviders() {
  ...
  const banques = ['pexels', 'pixabay', 'unsplash', 'coverr', 'shutterstock', 'openverse', 'wikimedia', 'archive'];
...
/** Les visuels web indexés ne sont pas automatiquement réutilisables. */
function droitsEtablis(asset) {
  ...
  const proof = String(asset.licenseUrl || asset.licenceUrl || '').trim();
  const licenseTextIsProof = /creative commons|\bcc\s*(?:by|0|zero|nc|nd|sa)?\b|domaine public|public domain/i.test(l);
  if (!l || (!proof && !licenseTextIsProof)
      || /usage éditorial|droits à vérifier|à vérifier|crédit affiché/.test(l)) return false;
  return /pexels|pixabay|coverr|unsplash|openverse|wikimedia|creative commons|archive\.org|domaine public|nasa/.test(`${p} ${l}`);
}
...
async function search(query, opts = {}) {
  ...
  /* Le pipeline ne passe pas toujours par l'interface. On applique donc les
   * mêmes alias pays/capitales/langues ici : « Bobo-Dioulasso », « Tombouctou »
   * ou « Agadez » restent ancrés dans leur territoire jusque dans les
   * recherches automatiques. Les requêtes purement régionales ne sont pas
   * forcées vers l'un des trois pays prioritaires. */
  const expanded = mediaCatalog.enrichQuery(query).query || query;
  const jobs = [];
  for (const p of providers) {
    const fn = PROVIDERS[p];
    if (!fn) continue;
    if (p === 'archive') {
      /* Internet Archive sépare ses films de ses images/textes. L'ancien
       * appel implicite (`wantVideo=true`) masquait ses images dès qu'un
       * clip était demandé, tandis que l'appel sans option pouvait ramener
       * une vidéo dans une recherche photo du pipeline. */
      jobs.push(fn(expanded, { wantVideo: false, perPage: 8, orientation, format, timespan }).catch(() => []));
      if (wantVideo) jobs.push(fn(expanded, { wantVideo: true, perPage: 8, orientation, format, timespan }).catch(() => []));
      continue;
    }
    if (p === 'coverr') {
      if (wantVideo) jobs.push(fn(expanded, { perPage: 10, orientation, format }).catch(() => []));
      continue;
    }
    jobs.push(fn(expanded, { type: 'image', perPage: 12, orientation, format, timespan }).catch(() => []));
    if (wantVideo && (p === 'pexels' || p === 'pixabay' || p === 'shutterstock')) {
      jobs.push(fn(expanded, { type: 'video', perPage: 8, orientation }).catch(() => []));
    }
  }
  ...
  if (wantVideo) {
    jobs.push(searchWikimediaVideo(expanded, { perPage: 8, format }).catch(() => []));
    ...
    const ancre = ancrageGeographique(expanded);
    if (ancre && ancre !== expanded.trim().toLowerCase()) {
  ...
  /* Politique de droits : Bing/DDG/GDELT restent consultables, mais leurs
   * images ne deviennent pas des assets de montage sans licence ou accord.
   * Le mode web éditorial est un choix explicite et conserve l'attribution. */
  /* Les moteurs web restent consultables pour la veille, mais un résultat
   * indexé n'est jamais un droit de montage. Seules les métadonnées portant
   * une licence ou une autorisation explicite franchissent ce filtre. */
  const _eligiblesDroits = uniq.filter(droitsEtablis);
  const opt = { format, wantVideo, query, contexte: ctx };
  for (const r of _eligiblesDroits) r._score = scoreAsset(r, opt);
  _eligiblesDroits.sort((a, b) => b._score - a._score);
  // Si des résultats vraiment pertinents existent, on jette le hors-sujet.
  const good = _eligiblesDroits.filter(r => (r._rel || 0) >= 0.5);
  const list = good.length >= 3 ? good : _eligiblesDroits.filter(r => (r._rel || 0) >= 0.3);
  return (list.length ? list : _eligiblesDroits).slice(0, limit);
}
...
async function download(asset, opts = {}) {
  /* Garde-fou au dernier point avant le réseau : une URL fournie par un
   * scraper ou importUrl ne peut pas contourner la politique en appelant
   * download directement. */
  if (!droitsEtablis(asset)) {
    throw new Error('téléchargement refusé : licence/autorisation non établie');
  }
  if (dlIsDisabled(asset.provider)) throw new Error(`provider ${asset.provider} désactivé (circuit breaker)`);
  ...
  /* ── TIMEOUT ADAPTATIF PAR TYPE DE MÉDIA ──
   * Avant : 8 s fixes pour TOUT (image 200 Ko comme vidéo 40 Mo).
   * Les vidéos Pexels faisaient timeout systématiquement → dizaines de
   * 'operation aborted' en boucle, budget média dépassé, visuels réemployés.
   * Maintenant : image = 12 s, vidéo = 35 s, avec 1 retry sur timeout.
   * Le range request (partiel) limite déjà les gros fichiers à ~12 Mo. */
  const isVideo = asset.kind === 'video' || asset.partiel;
  const dlTimeout = isVideo
    ? Number(process.env.DOWNLOAD_VIDEO_TIMEOUT) || 35000
    : Number(process.env.DOWNLOAD_IMAGE_TIMEOUT) || 8000;
  /* ── UN SITE LENT NE MÉRITE PAS DEUX CHANCES ──────────────────────
   * `retries: 1` DOUBLAIT chaque attente : un site injoignable coûtait
   * 24 s au lieu de 12. Mesuré sur le run « cacao » : une trentaine de
   * « This operation was aborted », soit ~15 minutes de pure attente,
   * sur une phase visuelle de 42 minutes qui a fini court-circuitée.
   * Ces sites sont des résultats de recherche opportunistes : s'ils ne
   * répondent pas du premier coup, la source suivante est toujours
   * meilleure que d'insister. On ne réessaie donc que les VIDÉOS, plus
   * rares et plus précieuses. */
  const res = await fetchBuf(asset.url, {
    timeout: dlTimeout, retries: isVideo ? 1 : 0,
    maxBytes: 120 * 1024 * 1024, headers: enTetes,
  });
  // 206 = contenu partiel : c'est le cas normal d'un téléchargement Range
  if (!res.ok && res.status !== 206) {
    dlTrackFail(asset.provider);
    throw new Error(`download ${res.status}`);
  }
  dlTrackOk(asset.provider);
...
async function acquire(queries, opts = {}) {
  ...
  /* ── GARDE-FOU GÉOGRAPHIQUE (avant téléchargement) ──────────────────
   * Run « procès Bella Bah en Guinée » : le sujet a été illustré par un
   * magasin Vodacom à Johannesburg et un palais de justice de l'Ontario.
   * Le filtre existant portait sur les REQUÊTES ; rien ne contrôlait les
   * RÉSULTATS, or les banques répondent par similarité approximative.
   *
   * On écarte ici tout candidat dont le titre, la légende ou l'URL trahit
   * un pays autre que celui du sujet. Placé AVANT `download`, le contrôle
   * évite en plus de télécharger un fichier qui sera jeté.
   * Ne s'applique que si le sujet désigne un pays identifiable. */
  let _entSujet = null;
  try {
    if (opts.topic) _entSujet = entites.extraire(String(opts.topic));
  } catch (e) { _entSujet = null; }
  ...
  for (const cand of liste) {
     ...
     * Mesuré sur le run « dette africaine » : « skip Pexels fetch
     * failed » revient jusqu'à SIX fois par plan, sur presque tous les
     * plans. Avec MEDIA_TIMEOUT_MULT=3, chaque tentative pouvait coûter
     * 60 s : un seul plan brûlait donc jusqu'à 360 s, et le budget de
     * 2 100 s s'épuisait en deux plans lents.
     * Conséquence en bout de chaîne : phase visuelle court-circuitée
     * après 2 984 s, et 39 illustrations IA sur 60 plans — soit 65 % de
     * visuels inventés, exactement ce qui est reproché.
     * Un fournisseur qui vient d'échouer trois fois d'affilée est en
     * panne ou inatteignable : insister ne le réparera pas. On l'écarte
     * pour le reste de la production, et on laisse la place aux sources
     * qui répondent. */
    if (_fournisseurMort(cand.provider) || _scrapingEpuise(cand.provider)) {
      exclude.add(cand.url); continue;
    }
    if (_entSujet && _entSujet.pays) {
      const txt = [cand.title, cand.description, cand.alt, cand.pageUrl, cand.url]
        .filter(Boolean).join(' ');
      const geo = entites.lieuCompatible(txt, _entSujet);
      if (!geo.ok) {
        if (_rejetsGeo++ < 3) log.warn(`ecarte — ${geo.raison}`);
    ...
    try {
      ...
      /* ── UN CANDIDAT QUI ÉCHOUE NE DOIT PLUS ÊTRE REPROPOSÉ ──────────
       * `exclude` n'était alimenté qu'en cas de SUCCÈS (ou de rejet
       * géographique). Une URL qui échoue au téléchargement restait donc
       * éligible pour tous les plans suivants, et le pipeline la
       * retentait indéfiniment.
       * Mesuré en production (run « 28 février ») : les quatre mêmes URL
       * — jesuisfrancais.blog, mon-diplome.fr, albin-michel-imaginaire.fr,
       * fr.linkedin.com — ont été réessayées SEIZE fois chacune, soit une
       * soixantaine de téléchargements perdus. Pendant ce temps le budget
       * média s'épuisait, et huit plans ont fini en illustration IA faute
       * d'avoir eu le temps de chercher ailleurs.
       * Une définition insuffisante ou un 403 ne se répare pas en
       * réessayant : on écarte définitivement pour cette production. */
      exclude.add(cand.url);
      _msGaspilles += (Date.now() - _t0);
      _noterEchec(cand.provider, e.message);
      log.warn('skip', cand.provider, e.message);
    }
  }
  return null;
}
...
async function importUrl(url, meta = {}) {
  ...
  const asset = {
    ...
    licensed: meta.licensed === true,
    licenceCC: meta.licenceCC === true,
...
module.exports = {
  search, download, acquire, reinitialiserEchecs, importUrl, creditLine, scoreAsset, relevance, normalize, droitsEtablis,
  searchPexels, searchPixabay, searchUnsplash, searchCoverr, searchShutterstock, searchOpenverse, searchWikimedia, searchNasa,
  searchDuckDuckGo, searchBing, searchGdelt, searchWikimediaVideo, searchArchive,
  droitsEtablis, defaultProviders, PROVIDERS,
};
