...
const {
  DIRS, fetchBuf, sha1, extFromContentType, mediaInfo, logger, stripHtml,
} = require('./util');
...
function scoreAsset(a, { format, wantVideo, query } = {}) {
  ...
  const px = w * h;
  if (px >= 1920 * 1080) s += 26; else if (px >= 1280 * 720) s += 16; else if (px >= 800 * 600) s += 7; else s += 1;
  const ar = h ? w / h : 1.5;
  ...
  s += rank[a.provider] != null ? rank[a.provider] : (String(a.provider).startsWith('Openverse') ? 6 : 0);
...
async function search(query, opts = {}) {
  ...
  for (const p of providers) {
    ...
    if (!fn) continue;
    jobs.push(fn(query, { type: 'image', perPage: 12, orientation }).catch(() => []));
...
async function download(asset, { dir = DIRS.media } = {}) {
  ...
  const res = await fetchBuf(asset.url, { timeout: 45000, retries: 2, maxBytes: 120 * 1024 * 1024 });
...
module.exports = {
  search, download, acquire, importUrl, creditLine, scoreAsset, relevance,
