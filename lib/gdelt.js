...
async function monitor(opts = {}) {
  try {
    ...
    /* ── FALLBACK : GDELT vide ou saturé (429) ──
     * Si aucune article n'est revenue de GDELT, on bascule sur Google News
     * RSS et DuckDuckGo News pour ne jamais avoir un script vide. */
    if (frEnArticles.length === 0) {
      log.info('GDELT vide — bascule sur Google News + DuckDuckGo');
      try {
        const s = require('./sources');
        const [gnews, ddg] = await Promise.allSettled([
          s.searchGoogleNewsRSS(topic, { limit: 50 }),
          s.searchDuckDuckGoNews(topic, { limit: 50 }),
        ]);
        const fallback = [];
        if (gnews.status === 'fulfilled') fallback.push(...gnews.value);
        if (ddg.status === 'fulfilled') fallback.push(...ddg.value);
        const articles = fallback.map(item => ({
          id: item.id || ('fb_' + sha1(item.link || item.title || '').slice(0, 12)),
          url: item.link || '',
          title: item.title || '',
          seendate: item.date || '',
          sourcecountry: '',
        ...
        if (articles.length) {
          return { articles, total: articles.length, fresh: articles.length };
        }
      } catch (e) {
        log.warn('Fallback actualités échoué : ' + e.message);
      }
    }
