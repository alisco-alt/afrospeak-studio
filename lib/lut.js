 ...
 *
 * 17³ (4 913 points) était un choix de prudence hérité du conteneur : le
 * fichier reste minuscule, mais entre deux points la valeur est INTERPOLÉE,
 * et sur un ciel ou un fond dégradé — omniprésents dans nos plans — cela
 * produit du banding, des bandes de couleur visibles.
 *
 * 33³ (35 937 points) est la résolution des LUT de production. Le .cube
 * pèse ~1 Mo, il est généré une fois puis mis en cache, et le coût à
 * l'application est nul : lut3d interpole au même prix quelle que soit la
 * taille de la table. Réglable pour les petites machines. */
const TAILLE = Math.max(2, Number(process.env.LUT_TAILLE) || 33);
...
const LOOKS = {
  ...
  ecodoc(r, g, b) {
    ...
    /* +4 % de luminosité et une courbe en S adoucie (0,28 → 0,18).
     * L'ancien réglage creusait les noirs, ce qui, cumulé à la vignette
     * et au voile des sous-titres, donnait l'image « de film » que
     * l'utilisateur ne veut pas sur une vidéo d'actualité. Les ombres
     * restent lisibles, ce qui compte sur un téléphone en plein jour. */
    const gain = 0.04;
    return [
      borne(courbeS(r, 0.18) + gain + haute * 0.045 - ombre * 0.010),
      borne(courbeS(g, 0.18) + gain + haute * 0.012 + ombre * 0.012),
      borne(courbeS(b, 0.18) + gain - haute * 0.032 + ombre * 0.045),
    ];
  },
...
/**
 * Look conseillé selon le style et la nature du sujet.
 *
 * ── UNE CHAÎNE D'ACTUALITÉ N'EST PAS UN FILM ──
 * Retour de visionnage : « éviter de rendre la vidéo trop sombre, c'est
 * pas un film, c'est une vidéo d'actualité ». Constat fondé : cinq
 * couches d'assombrissement se cumulaient — le grade du style, la LUT
 * `tension` (courbe en S à 0,42 et ombres bleutées), la vignette, le
 * voile des sous-titres et le fond flouté. Aucune n'éclaircissait.
 *
 * Agence Ecofin travaille en image CLAIRE, contraste modéré, sans
 * vignette marquée : l'information doit se lire d'un coup d'œil sur un
 * téléphone, souvent en plein jour.
 *
 * `tension` est donc réservé aux sujets qui l'exigent vraiment
 * (conflit, sécurité), et `ecodoc` — nettement plus lumineux — devient
 * le choix par défaut, y compris en style documentaire.
 */
function lookPour(style, ctxSujet) {
  const nature = ctxSujet && ctxSujet.nature ? String(ctxSujet.nature) : '';
  if (/conflit|guerre|securite|sécurité/i.test(nature)) return 'tension';
...
function filtre({ look, force } = {}) {
  ...
  const f = force != null ? force : Number(process.env.LUT_INTENSITE || 0.22);
  if (!(f > 0)) return '';
