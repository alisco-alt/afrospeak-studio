...
async function youtubeBatch(topic, opts = {}) {
  ...
  if (!_nClips && candidates.length && !argsCookiesYT().length) {
    onLog('Aucun clip YouTube : la plateforme exige une session (PO Token). '
      + 'Exportez vos cookies dans cookies/youtube_cookies.txt pour débloquer '
      + 'les extraits vidéo — les banques Pexels/Pixabay prennent le relais en attendant.', 'warn');
  }
  return assets;
}
