...
async function youtubeBatch(topic, opts = {}) {
  ...
  for (const req of requetes) {
    ...
    const pertinents = lot.filter(c => titrePertinent(c.title, req));
  ...
  const clipPromises = candidates.slice(0, maxClips).map(async (c, k) => {
    ...
    try {
      ...
      const args = [
        ...
        '--max-filesize', (Number(process.env.YT_MAX_MO) || 60) + 'M',
        '-o', brut,
        c.url,
      ];
      ...
      if (r.ok && fs.existsSync(brut) && fs.statSync(brut).size > 10000) {
        ...
        return { file: brut, provider: 'YouTube', title: c.title, platform: 'youtube', isVideo: true, duration: c.duration, source: c.url };
      }
...
module.exports = { batchSource, youtubeBatch, construireRequetesYT, titrePertinent, newsImageBatch, newsArticleBatch, galleryDlBatch, dedupAssets, runCmd };
...
async function galleryDlBatch(topic, opts = {}) {
  ...
  /* Facebook est RETIRÉ de cette liste, sur constat :
   *   · `gallery-dl --list-extractors` n'expose AUCUN extracteur de
   *     recherche Facebook (seulement profils, albums, photos par ID) —
   *     l'URL /search/posts/?q= ne correspondait à rien ;
   *   · même sur une page publique, gallery-dl renvoie « AuthRequired ».
   * Le remplacer par une requête impossible ne servait qu'à consommer un
   * timeout par run. Les photos de presse africaine (pressePhotos.js)
   * couvrent ce besoin, sans cookie et avec un meilleur rendement. */
  const platforms = [
