...
const TIMEOUT_COOKIE_MS = 10000; // 10 s strict, imposé par consigne
...
async function openSourceFallback(topic, opts = {}) {
  ...
  try {
    ...
    const [pexels, pixabay] = await Promise.all([
      withTimeout(media.searchPexels(topic, { type: 'image' }), TIMEOUT_COOKIE_MS, 'Pexels fallback').catch(() => []),
      withTimeout(media.searchPixabay(topic, { type: 'image' }), TIMEOUT_COOKIE_MS, 'Pixabay fallback').catch(() => []),
    ]);
...
async function newsImageBatch(topic, opts = {}) {
  ...
  try {
    ...
    const resp = await withTimeout(
      fetchBuf(bingUrl, { timeout: TIMEOUT_COOKIE_MS, retries: 1, headers: hdrs }),
      TIMEOUT_COOKIE_MS, 'Bing images'
    );
...
module.exports = { batchSource, youtubeBatch, newsImageBatch, newsArticleBatch, galleryDlBatch, dedupAssets, runCmd };
...
async function galleryDlBatch(topic, opts = {}) {
  ...
  const platforms = [
    { name: 'tiktok', search: 'https://www.tiktok.com/tag/' + encodeURIComponent(topic.split(' ').slice(0, 3).join('')) },
    { name: 'instagram', search: 'https://www.instagram.com/explore/tags/' + encodeURIComponent(topic.split(' ').slice(0, 2).join('')) },
    { name: 'x', search: 'https://x.com/search?q=' + encodeURIComponent(topic) + '&f=live' },
    { name: 'facebook', search: 'https://www.facebook.com/search/posts/?q=' + encodeURIComponent(topic) },
  ];
  ...
  for (const p of platforms) {
    ...
    try {
      ...
      if (!r.ok) {
        onLog('Gallery-dl ' + p.name + ': échec/timeout (cookie invalide ?) — fallback ouvert', 'warn');
        continue;
      }
      // Parse les résultats
      for (const line of (r.stdout || '').split('\n').filter(Boolean)) {
        try {
          ...
          const dlArgs = ['--quiet', '-o', outFile, ...cookieArgs(p.name), url];
