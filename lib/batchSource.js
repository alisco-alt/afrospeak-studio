...
async function youtubeBatch(topic, opts = {}) {
  ...
  const thumbPromises = candidates.slice(0, maxThumbs).map(async (c, k) => {
    ...
    try {
      ...
      if (buf && buf.length > 3000) {
        ...
        return { file: outFile, provider: _provT, licenceCC: !!c.cc, title: c.title, platform: 'youtube', isVideo: false, source: c.url, uploader: c.uploader || '' };
      }
    } catch (e) {}
    return null;
  });
  ...
  onLog('YouTube batch : ' + _nClips + ' clips + ' + assets.filter(a => !a.isVideo).length + ' thumbnails');
...
async function newsImageBatch(topic, opts = {}) {
  ...
  for (let i = 0; i < imgUrls.length; i += batchSize) {
    ...
    const results = await Promise.allSettled(batch.map(async (imgUrl, j) => {
      ...
      if (fs.existsSync(outFile) && fs.statSync(outFile).size > 5000) {
        return { file: outFile, provider: 'Web/Bing', title: topic, requete: topic,
          platform: 'web', isVideo: false };
      }
      try {
...
module.exports = { batchSource, youtubeBatch, construireRequetesYT, titrePertinent, noteChaine, CHAINES_AFRICAINES, newsImageBatch, newsArticleBatch, galleryDlBatch, dedupAssets, runCmd, bgutilPoTokenInfo };
...
async function galleryDlBatch(topic, opts = {}) {
  ...
  const _tag = require('./entites').normTag;   // une seule définition, voir lib/entites.js
  ...
  for (const p of platforms) {
    ...
    if (social) {
      ...
      if (!has) {
        /* ── DIRE COMMENT DÉBLOQUER, PAS SEULEMENT QUE C'EST BLOQUÉ ────
         * X et Facebook sont les plateformes les plus riches en vidéos
         * d'actualité, et l'auteur y tient. Techniquement, gallery-dl
         * sait déjà en télécharger (le code accepte .mp4/.webm/.mov) :
         * il ne manque QUE la session. Sans cookies, la plateforme
         * répond « AuthRequired » même sur une page publique.
         * Plutôt qu'un message passif, on indique la marche à suivre —
         * c'est la seule voie légitime, on ne contourne aucune
         * protection : on présente une vraie session, la sienne. */
        const _quoi = p.name === 'x'
          ? 'cookies/x_cookies.txt (cookies auth_token et ct0)'
          : `cookies/${p.name}_cookies.txt`;
        onLog(`Gallery-dl ${p.name} : aucune session — plateforme ignorée. `
          + `Pour activer les vidéos ${p.name}, exportez ${_quoi} `
          + '(extension Cookie-Editor, format Netscape) depuis un navigateur connecté.', 'info');
        continue;
      }
      ...
      for (const line of (r.stdout || '').split('\n').filter(Boolean)) {
        try {
          ...
          if (dl.ok && fs.existsSync(outFile) && fs.statSync(outFile).size > 5000) {
            ...
            /* Le compte d'origine doit apparaître au crédit (@compte) :
             * c'est la condition qui rend la citation défendable. */
            const compte = String(meta.username || meta.uploader
              || (meta.author && (meta.author.name || meta.author.nick))
              || (meta.user && (meta.user.name || meta.user.nick)) || '')
              .replace(/^@/, '').slice(0, 30);
            let asset = {
              file: outFile,
              provider: (({ tiktok: 'TikTok', instagram: 'Instagram', x: 'X (Twitter)', facebook: 'Facebook' })[p.name] || p.name)
                + (compte ? ' · @' + compte : ''),
              title: (meta.content || meta.description || meta.title || '').slice(0, 60),
              platform: p.name, isVideo, source: url,
              author: compte ? '@' + compte : '',
            };
            /* ── RÈGLE DE CITATION SUR LES CLIPS SOCIAUX ────────────────
             * Une vidéo X/Instagram/TikTok est protégée comme n'importe
             * quelle œuvre — et sa provenance est souvent douteuse (repost
             * de repost). On applique le même régime que la cascade
             * par-plan : extrait ≤ DUREE_MAX (4 s), sans audio, marqué
             * `citation` pour que le crédit écran soit OBLIGATOIRE et que
             * verifierMontage borne le plan. Un clip de divertissement
             * pur (non citable) est écarté d'emblée. */
            if (isVideo) {
              try {
                const citation = require('./citation');
                const jugement = citation.extraitCitable(asset);
                if (!jugement.ok) {
                  onLog('Gallery-dl ' + p.name + ' : clip écarté — ' + jugement.raison, 'info');
                  try { fs.unlinkSync(outFile); } catch (e2) {}
                  continue;
                }
                const { mediaInfo } = require('./util');
                asset.info = await mediaInfo(outFile);
                asset = await citation.preparerExtrait(asset, {});
              } catch (e2) {
                /* extrait brut conservé : verifierMontage bornera en aval */
              }
            }
            assets.push(asset);
          }
