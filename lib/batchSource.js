...
/**
 * Si gallery-dl est installé, cherche des posts TikTok/X/Instagram
...
async function galleryDlBatch(topic, opts = {}) {
  ...
  if (!check.ok) {
    onLog('Gallery-dl : non installé (pip install gallery-dl pour TikTok/X/IG)', 'info');
    return assets;
  }
  ...
  for (const p of platforms) {
    try {
      ...
      const r = await runCmd('gallery-dl', args, { timeout: 60000 });
      if (!r.ok) {
        onLog('Gallery-dl ' + p.name + ': échec (cookies requis?)', 'warn');
        continue;
      }
      // Parse les résultats
      for (const line of (r.stdout || '').split('\n').filter(Boolean)) {
        try {
          ...
          const dl = await runCmd('gallery-dl', dlArgs, { timeout: 30000 });
          if (dl.ok && fs.existsSync(outFile) && fs.statSync(outFile).size > 5000) {
            const isVideo = /\.(mp4|webm|mov)/i.test(outFile);
            assets.push({
              file: outFile, provider: p.name === 'tiktok' ? 'TikTok' : 'X (Twitter)',
      ...
      onLog('Gallery-dl ' + p.name + ': ' + String(e.message).slice(0, 60), 'warn');
    }
  }
