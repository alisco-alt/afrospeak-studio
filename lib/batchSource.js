...
async function openSourceFallback(topic, opts = {}) {
  ...
  try {
    ...
    const videos = [...(pexelsVideo || []), ...(pixabayVideo || []), ...(commonsVideo || [])]
      .filter(item => item && item.url).slice(0, 6);
    ...
    const results = await Promise.allSettled(batch.map(async (imgUrl, j) => {
      ...
      if (fs.existsSync(outFile) && fs.statSync(outFile).size > 5000) {
        return { file: outFile, provider: 'Web/Bing', title: topic, requete: topic,
          platform: 'web', isVideo: false, source: imgUrl, pageUrl: imgUrl,
          license: 'Usage éditorial — droits à vérifier', licenseUrl: imgUrl,
          requiresAttribution: true, rights: 'verify' };
      }
      try {
      ...
      try {
        if (fs.existsSync(outFile) && fs.statSync(outFile).size > 5000) {
          assets.push({ file: outFile, provider: 'Web/Article', title: r.value.url, platform: 'web', isVideo: false,
            source: imgUrl, pageUrl: r.value.url, license: 'Usage éditorial — droits à vérifier',
            licenseUrl: r.value.url, requiresAttribution: true, rights: 'verify' });
          continue;
        }
        const resp = await fetchBuf(imgUrl, { timeout: 10000, retries: 0 });
        const buf = resp && resp.buffer;
