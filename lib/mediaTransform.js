...
async function standardizeMediaClip(inputPath, outputPath, options = {}) {
  ...
  let mode = fitMode;
  if (mode === 'auto') {
    /* Le rendu final préfère remplir le cadre : un plan avec deux bandes
     * floues énormes paraît être un asset mal monté. Le caller peut demander
     * explicitement `blur` lorsqu'il faut préserver toute une archive. */
    if (process.env.MEDIA_FIT_MODE === 'blur') mode = 'contain';
    else if (options.isChart || options.noCrop) mode = 'contain';
    else {
