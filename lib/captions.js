...
async function buildASS(words, opts = {}) {
  ...
  const fitMin = Number(process.env.CAPTION_FIT_MIN) || 0.86;
  ...
  if (popActif && process.env.CAPTION_PILL !== '0') {
    ...
    const maxMots = unParUn ? 1
      : (Number(process.env.CAPTION_MAX_WORDS)
        || (format === 'vertical' ? 2 : 4));
    const lines = groupLines(words, {
      maxChars: Math.min(maxChars, unParUn ? 24 : (format === 'vertical' ? 22 : 38)),
      maxWords: maxMots,
    ...
    for (let li = 0; li < lines.length; li++) {
      ...
      let fsGroupe = fs;
      const espaceBrut = metrics.espace || Math.round(fs * 0.26);
      /* Certaines mesures FFmpeg/cache peuvent renvoyer un espace anormalement
       * large. Un sous-titre ne doit jamais ressembler à une suite de mots
       * dispersés : on borne l'interlettrage à 12–28 % du corps. */
      const espace = Math.max(Math.round(fs * 0.12), Math.min(Math.round(fs * 0.28), espaceBrut));
       ...
       * large que la mesure. */
      const yLigne = posY;
      const espReel = espace * (fsGroupe / fs);
      let curseur = -total / 2;
