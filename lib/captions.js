 ...
 */
function groupLines(words, { maxChars = 42, maxWords = 3, maxDur = 3.2, breakOnPunct = true } = {}) {
  /* Une séquence verticale est volontairement courte. Même si un ancien
   * preset ou une variable d'environnement demande davantage de mots, le
   * générateur ne laisse jamais ASS décider du retour à la ligne : 4 est un
   * plafond dur, 3 la valeur par défaut. */
  const maxWordsSafe = Math.min(4, Math.max(1, Number(maxWords) || 3));
  const lines = [];
  let cur = [];
  ...
  for (const w of words) {
    ...
    if (cur.length && (projected.length > maxChars || cur.length >= maxWordsSafe || dur > maxDur)) flush();
    cur.push(w);
    if (breakOnPunct && /[.!?…]$/.test(w.word)) flush();
    else if (/[,;:]$/.test(w.word) && cur.length >= Math.max(3, maxWordsSafe - 2)) flush();
  }
  flush();
...
async function buildASS(words, opts = {}) {
  const {
    ...
    pillText = '#FFFFFF',      // encre du mot actif : BLANC sur le nuage (demande)
    accentOnPill = '#0D47A1',  // variante foncée d'un mot important posé sur le nuage
    ...
    maxWidthRatio = 0.85,      // contrat vertical : jamais plus de 85 % de W
  } = opts;
  ...
  /* La largeur ne dépend plus d'un calcul de marge divergent entre ASS,
   * pastilles et layout. Le bloc est centré par construction, avec deux
   * marges identiques. */
  const widthRatio = Math.min(0.85, Math.max(0.50, Number(maxWidthRatio) || 0.85));
  const boxW = Math.floor(W * widthRatio);
  const marginLR = Math.floor((W - boxW) / 2);
  ...
  const popActif = mode === 'pop' || (mode === 'word')
    ...
    /* En 9:16, le layout manuel est obligatoire : même un appel direct
     * sans `activeBox` ne doit pas rendre une phrase libre à libass. */
    || (format === 'vertical' && mode !== 'none');
  ...
  /* Le fond global est interdit en vertical. La pastille individuelle est
   * dessinée plus bas, sur son propre événement et ses propres timings. */
  const fondAutorise = format !== 'vertical' && boxOpacity > 0;
  const bordW = fondAutorise ? Math.max(6, Math.round(fs * 0.10)) : outlineW;
  const shadW = fondAutorise ? 0 : shadow;
  const boxAlpha = fondAutorise
    ? Math.round((1 - boxOpacity) * 255).toString(16).padStart(2, '0').toUpperCase()
    : 'FF';
  /* L'encre ASS et son contour doivent aussi rester dans le contrat de
   * largeur : on réserve le contour de chaque côté avant le layout. */
  const layoutW = Math.max(1, usableW - 2 * outlineW);
...
`;
  ...
  const inkC = hexToAss(pillText || '#FFFFFF').replace('&H00', '&H');
  const accOnPillC = hexToAss(accentOnPill || '#0D47A1').replace('&H00', '&H');
  ...
  /* En vertical, même le preset « phrase » passe par le layout pop
   * strict : 3 mots par séquence, pastille individuelle et largeur mesurée. */
  if (mode === 'phrase' && format !== 'vertical') {
  ...
  /* ══════════════════════════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════════════════════════ */
  if (popActif && (format === 'vertical' || process.env.CAPTION_PILL !== '0')) {
    const unParUn = mode === 'word';
    const maxMots = unParUn ? 1
      : Math.min(4, Math.max(1, Number(process.env.CAPTION_MAX_WORDS)
        || (format === 'vertical' ? 3 : 3)));
    /* Une ligne de 2 à 4 mots reste plus naturelle qu'un mot isolé,
     * surtout quand le mot actif ne change que toutes les 300 ms. Le nuage
     * reste INDIVIDUEL : seul le mot prononcé est surligné, la ligne de base
     * donne simplement le contexte autour de lui. */
    const maxCharsPop = unParUn ? 24 : (format === 'vertical' ? 32 : 42);
    const lines = groupLines(words, {
      /* Le budget de caractères est volontairement un plafond de
       * groupement, pas le budget de largeur de la police : la mesure exacte
       * ci-dessous réduit le corps si nécessaire. Sinon l'ancienne estimation
       * coupait trop tôt (souvent deux mots seulement) et donnait l'impression
       * que chaque mot flottait seul. */
      maxChars: maxCharsPop,
      maxWords: maxMots,
      maxDur: unParUn ? 1.2 : 2.2,
    });
    ...
    for (let li = 0; li < lines.length; li++) {
      ...
      let fsGroupe = fs;
      /* Le budget comprend la largeur des mots, des espaces ET le padding de
       * la pastille extrême. Avant ce calcul, une ligne pouvait respecter
       * 85 % pour son encre puis dépasser avec le nuage du premier/dernier
       * mot. */
      const gapEnv = Number(process.env.CAPTION_WORD_GAP_EM);
      const gapEm = Number.isFinite(gapEnv) ? Math.max(0.02, Math.min(0.16, gapEnv)) : 0.035;
      const espaceStable = Math.round(fs * gapEm);
      const padFs = Math.round(fs * pillPadEm);
      const totalPour = (taille) => {
        const k = taille / fs;
        const motsPx = line.reduce((a, w) => a + Math.round(largeur(w.word) * k), 0);
        return motsPx
          + Math.round(espaceStable * k) * (line.length - 1)
          + 2 * Math.round(padFs * k);
      };
      let total = totalPour(fs);
      if (total > layoutW) {
        /* Réduction déterministe, sans plancher qui réintroduirait un
         * débordement sur un mot exceptionnellement long. */
        fsGroupe = Math.max(1, Math.floor(fs * layoutW / total));
        total = totalPour(fsGroupe);
        /* Les arrondis des avances typographiques ne doivent jamais recréer
         * un pixel de dépassement après le fit principal. */
        while (total > layoutW && fsGroupe > 1) {
          fsGroupe--;
          total = totalPour(fsGroupe);
        }
      }
       ...
       * La pastille et le mot partagent ces coordonnées. Le padding est
       * inclus dans `total`, donc les marges gauche/droite sont symétriques. */
      const yLigne = posY;
      const scale = fsGroupe / fs;
      const espReel = Math.round(espaceStable * scale);
      const padReel = Math.round(padFs * scale);
      let curseur = -total / 2 + padReel;
      const centres = line.map(w => {
        const lw = largeur(w.word) * scale;
      ...
      line.forEach((w, i) => {
        ...
        /* En vertical, le contraste éditorial est binaire : les mots en
         * attente restent blancs. Les seules couleurs réservées au mot actif
         * sont le jaune opaque de sa pastille et son encre blanche. */
        const coul = format === 'vertical' ? null : couleurMot(w, prec);
      ...
      for (let i = 0; i < line.length; i++) {
        ...
        const pw = Math.round(lw + 2 * padReel);
        ...
        /* Le mot actif se redessine par-dessus sa copie blanche (calque
         * supérieur, même position exacte). Règle : mot prononcé = BLANC
         * sur le nuage ; sauf s'il est un nom propre / mot important —
         * il garde alors sa couleur, en nuance foncée lisible sur jaune.
         * NB : precedent = la CHAÎNE du mot précédent (comme dans la
         * boucle de la copie de base), pas l'objet ligne. */
        const coulActuelle = format === 'vertical'
          ? null : couleurMot(w, i > 0 ? line[i - 1].word : null);
        const encreActif = coulActuelle ? accOnPillC : inkC;
        texteActif.push({
          s: debut, e: fin, layer: 3,
          t: `{${fsTag}\\an5\\pos(${centres[i]},${yLigne})\\c${encreActif}\\bord0\\shad0}${tx(w.word)}`,
        });
      }
    }
  ...
  const lines = groupLines(words, {
    ...
    maxWords: chunkCourt
      ? Math.min(4, Math.max(1, Number(process.env.CAPTION_MAX_WORDS) || 3))
      : (format === 'vertical' ? 4 : 12),
    maxDur: 1.9,
  });
  const events = [];
  for (let li = 0; li < lines.length; li++) {
...
/* ── Slides de données : plans où les sous-titres gênent ─────────────
 * Une carte chiffre plein écran doit respirer seule : on retire les mots
 * des plans `dataSlide` du flux des sous-titres (le SRT, lui, reste
 * complet pour l'accessibilité). */
function motsHorsPlans(words, indices) {
  const exclus = new Set(indices);
  return words.filter(w => !exclus.has(w.shotIndex));
}

module.exports = { buildASS, buildSRT, buildVTT, groupLines, hexToAss, assTime, srtTime, emWidth, motsHorsPlans };
