...
`;
  ...
  for (let li = 0; li < lines.length; li++) {
    ...
    const finBrute = line[line.length - 1].end + 0.08;
    const end = suivante
      ? Math.min(finBrute, suivante[0].start - 0.001)
      : finBrute;

    if (mode === 'phrase') {
      // ── MODE PHRASE : texte complet, pas de fad, mots-clés colorés ──
      const fsTag = shotFsTag(line[0]);
    ...
    if (mode === 'word') {
      ...
      for (let wi = 0; wi < line.length; wi++) {
        ...
        events.push({ s: w.start, e: Math.max(w.end, w.start + 0.12), t: `${fsTag}${posTag}${text}` });
      }
      continue;
    }
  ...
  if (boxOpacity > 0) {
    ...
    for (const ev of events) {
      ...
      if (!nu.trim()) continue;
      const nLignes = nu.split('\\N').length;
      ...
      const w = Math.min(W - 8, Math.ceil(brute / pas) * pas);
