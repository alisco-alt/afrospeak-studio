...
`;
  ...
  if (popActif && process.env.CAPTION_PILL !== '0') {
    ...
    for (let li = 0; li < lines.length; li++) {
      ...
      let fsGroupe = fs;
      /* Le TTF mesure l'encre, pas toujours l'avance typographique de
       * l'espace. Selon la version de FFmpeg/libass, cette mesure pouvait
   * gonfler l'écart entre deux mots jusqu'à donner une ligne « éclatée ».
   * On borne donc l'écart à une valeur typographique stable et réglable :
   * 0,03 em par défaut, compact mais encore lisible sur mobile. */
      const gapEnv = Number(process.env.CAPTION_WORD_GAP_EM);
      /* Les mots sont posés séparément pour synchroniser la pastille, mais
       * l'écart doit rester celui d'une phrase compacte — pas une suite de
       * cartons isolés. */
      const gapEm = Number.isFinite(gapEnv) ? Math.max(0.02, Math.min(0.16, gapEnv)) : 0.035;
      const espaceStable = Math.round(fs * gapEm);
      let total = line.reduce((a, w) => a + largeur(w.word), 0) + espaceStable * (line.length - 1);
      if (total > usableW) {
       ...
       * large que la mesure. */
      const yLigne = posY;
      /* Le nuage et le mot utilisent toujours les mêmes centres calculés ici. */
      const espReel = espaceStable * (fsGroupe / fs);
      let curseur = -total / 2;
