...
function construireConsigne(requete, { style = 'ecofin', sujet = '' } = {}) {
  ...
  /* Une illustration de substitution doit respecter l'époque du plan.
   * Sans cette contrainte, un sujet comme « Dahomey, 1892 » est facilement
   * rendu avec des vêtements, bâtiments et équipements contemporains. On
   * demande une scène d'archive reconstituée, sans la faire passer pour une
   * photographie authentique. */
  const historique = /\b(18\d{2}|19[0-4]\d|19th century|18th century|histor|archiv|colonial|dahomey|abomey|amazone|amazon warriors|royaume du benin|benin kingdom)\b/i
    .test(`${requete} ${sujet}`);
  const contrainteEpoque = historique
    ? ', historically accurate period reconstruction, period clothing and architecture, no modern objects, no cars, no smartphones, no contemporary buildings, documentary archive aesthetic'
    : '';
  ...
  return `${requete} ${lieu}${precision}${contrainteEpoque}, ${ambiance}${sansPortrait}, no text, no watermark, no logo, `
    + 'realistic proportions, photographic, 35mm lens, '
    + 'dignified professional setting, no poverty imagery, no slum, no dust haze, '
    + ANTI_SOMBRE;
}
