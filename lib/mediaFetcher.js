...
2. Les noms de lieux, monuments et institutions sont ENCOURAGÉS (Sankoré,
   Zamfara, CEDEAO, Gold Fields). Évite seulement les visages de personnalités.
...
async function fetchForSegments(segments, opts = {}) {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (!got) {
      try {
        /* ── SOURCES CHOISIES PAR LE CONTEXTE (§2) ──
         * Un sujet historique interroge les archives (Wikimedia, Internet
         * Archive) ; un sujet d'actualité interroge la presse et les banques
         * vidéo. C'est le réflexe du documentariste, appliqué automatiquement
         * plutôt qu'imposé par une liste figée. */
        const ctx = s.contexte;
        const plan = ctx ? contexte.sourcesPour(ctx) : null;
        got = await mediaLib.acquire(s.queries, {
          format,
          // Alterner image/vidéo évite la monotonie ; les segments longs
          // supportent mieux une vidéo. Sur un sujet historique, le contexte
          // impose l'image fixe : il n'existe pas de captation du XVe siècle.
          wantVideo: (plan ? plan.wantVideo : true)
            && wantVideo && (s.duration > 2.5) && (i % 2 === 0),
          providers: plan ? plan.providers : undefined,
          contexte: ctx,
          exclude, limit: 16,
        });
      } catch (e) {
