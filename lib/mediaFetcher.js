...
6. "epoque" = "historique" | "recent" | "contemporain" — ton analyse du
   segment. Elle détermine les sources interrogées : ne te trompe pas.`,
    ...
    for (const [rang, shot] of listeUtile.entries()) {
      ...
      s.queries = [...new Set(qs.concat(s.queries))]
        .map(q => ancrerRequeteHumaine(q, topic))
        .slice(0, 4);
  ...
  const remplis = fillGaps(segments, topic);
  for (const s of remplis) {
    s.queries = (s.queries || []).map(q => ancrerRequeteHumaine(q, topic));
  }
  return remplis;
}
...
/* ── GARDE-FOU DES VISAGES ET DES PERSONNES ─────────────────────────
 * Une banque peut répondre à « prime minister Africa » avec un visage
 * occidental. Pour les plans humains, le pays/démonyme doit être dans la
 * requête, et le nom de la personne lorsqu'il est connu. Ce filtre ne
 * prétend pas reconnaître une nationalité dans les pixels : il évite surtout
 * de la demander de façon ambiguë et laisse le contrôle des métadonnées
 * rejeter les résultats explicitement étrangers.
 */
const MOTIFS_HUMAINS = /\b(people|person|man|woman|men|women|crowd|faces?|portrait|minister|president|official|speaker|speaking|demonstrat|protest|traders?|workers?)\b/i;
const DEMONYMES = {
  senegal: 'senegalese', mali: 'malian', guinee: 'guinean', ghana: 'ghanaian',
  nigeria: 'nigerian', kenya: 'kenyan', cameroun: 'cameroonian',
  cote: 'ivorian', ivoire: 'ivorian', rdc: 'congolese', congo: 'congolese',
  burkina: 'burkinabe', niger: 'nigerien', maroc: 'moroccan', algerie: 'algerian',
};
function ancrerRequeteHumaine(query, topic) {
  const q = String(query || '').trim();
  if (!MOTIFS_HUMAINS.test(q)) return q;
  const sujets = keywords(topic);
  const pays = sujets.find(x => DEMONYMES[x] || LIEUX.has(x));
  if (!pays) return q;
  const dem = DEMONYMES[pays] || pays;
  const ent = (() => { try { return entites.extraire(String(topic || '')); } catch (e) { return null; } })();
  const personne = ent && ent.personnes && ent.personnes[0];
  let prefix = '';
  if (personne && !new RegExp(personne.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i').test(q)) prefix = personne + ' ';
  if (!new RegExp(`\\b(?:${dem}|${pays}|${String(pays).replace(/s$/, '')})\\b`, 'i').test(q)) prefix += dem + ' ';
...
const UNIVERS = [
  ...
  /* Une dérive « football stadium crowd » ou « athletes training » a été
   * observée sur un sujet solaire saharien : la requête était filmable mais
   * totalement étrangère au mécanisme raconté. Les termes concrets ne
   * suffisent donc pas ; chaque univers doit aussi être autorisé par le
   * sujet global. */
  { nom: 'sport', re: /\b(football|soccer|athlete|athletes|stadium|basketball|match|training)\b/i,
    sujet: /\b(football|soccer|sport|athlet|stade|match|olympi|joueur)\b/i },
];
...
module.exports = {
  ...
  requeteCoherente, requeteDescriptive, ancrageSujet, ancrerRequeteHumaine,
};
