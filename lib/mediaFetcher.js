...
async function cascadeSource(seg, queries, sourceName, opts) {
  ...
  const maxQ = LENTES.has(sourceName)
    ? (Number(process.env.MAX_QUERIES_LENTES) || 2)
    : queries.length;
      ...
      switch (source) {
        ...
        case 'youtube':
          if (socialP1 && socialP1.downloadYouTubeClip) {
            ...
            let ytTo = Number(process.env.YT_TIMEOUT_MS) || 35000;
        ...
        case 'social':
          ...
          try {
            ...
            const authResp = await social.searchAll(q, {
            ...
            const openResp = await social.searchAll(q, {
              platforms: ['reddit', 'mastodon', 'archive'],
              perPlatform: 5,
            }).catch(() => null);
            const openItems = openResp && openResp.items ? openResp.items : [];
...
function cascadeOrder(seg, opts) {
  ...
  base.push('pexels', 'pixabay', 'unsplash', 'shutterstock');
...
async function fetchForSegments(segments, opts = {}) {
  ...
  const segBudgetBase = Math.max(12000, Math.min(45000,
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (budgetLeft) {
      ...
      const segBudget = Math.max(8000, Math.min(segBudgetBase,
        Math.round((budgetMs - ecoule) / Math.max(1, restant)) + 6000));
