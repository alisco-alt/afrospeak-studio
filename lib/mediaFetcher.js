      ...
      switch (source) {
        ...
        case 'unsplash':
          got = await mediaLib.acquire([q], {
            ...
            providers: [source], contexte: seg.contexte, limit: 8,
          }).catch(() => null);
          break;
...
function cascadeOrder(seg, opts) {
  const ctx = seg.contexte || {};
  const base = ['pexels', 'pixabay', 'unsplash', 'wikimedia', 'websearch'];
  // YouTube pour TOUS les sujets (les clips vidéo sont essentiels même
  // en histoire — archives documentaires, footage d'époque)
  base.push('youtube');
  // Playwright si disponible (recherche d'images plus large)
  if (webScraper && webScraper.toolStatus().available) {
    base.push('webScraper');
  }
  // Réseaux sociaux (Reddit, Mastodon, Archive) pour fraîcheur topique
  // → c'est la méthode des grands créateurs : Brut, BrutX, Konbini...
  base.push('social');
