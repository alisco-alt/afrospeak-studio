...
async function buildQueries(segments, { topic = '', useLLM = true } = {}) {
  // Base lexicale : toujours calculée, sert de filet
  for (const s of segments) {
    const fromLex = queriesFromLexique(s.text);
    s.queries = fromLex.length ? fromLex.slice(0, 3) : [];
  ...
  try {
    ...
    const res = await llm.chat([
      {
        role: 'system',
        content: `Tu es directeur artistique d'une chaîne vidéo panafricaine.
Pour chaque segment de narration, propose la requête de banque d'images qui
illustre LE PLUS PRÉCISÉMENT ce qui est dit à cet instant.

RÈGLES :
- requête en ANGLAIS, 2 à 5 mots, décrivant une scène RÉELLE et FILMABLE ;
- privilégie un contexte africain quand le propos le permet ("africa", nom du pays) ;
- jamais de concept abstrait ("economy", "growth") : préfère l'objet concret
  qui le représente ("container port crane", "banknotes counting") ;
- jamais de nom de personnalité (les banques d'images n'en ont pas) ;
- "kind" = "data" si le segment énonce un chiffre marquant, sinon "broll".`,
      },
      {
        role: 'user',
        content: `SUJET DE LA VIDÉO : ${topic}\n\nSEGMENTS :\n${list}\n\n`
...
function fillGaps(segments, topic) {
  ...
  const generic = fallback.length ? fallback : ['african city skyline aerial', 'africa business people'];
  segments.forEach((s, i) => {
    if (!s.queries || !s.queries.length) {
      // Les mots-clés du segment eux-mêmes, à défaut de mieux
      const kw = s.keywords.slice(0, 2).join(' ');
      s.queries = kw ? [kw + ' africa', generic[i % generic.length]] : [generic[i % generic.length]];
    }
  });
  return segments;
}
