...
router.get('/videos', auth.required, wrap(async (req, res) => {
  ...
  ok(res, { videos: videos.map(v => shapeVideo(v)) });
}));
...
router.get('/videos/:id', auth.required, wrap(async (req, res) => {
  ...
  ok(res, { video: shapeVideo(v, true), queue: queue.eta(v.id) });
}));
  ...
  const mirror = setInterval(async () => {
    try {
      ...
      await db.updateVideo(projectId, {
        status: p.status === 'done' ? 'done' : p.status === 'error' ? 'error' : 'running',
...
function shapeVideo(v, full = false) {
  const base = {
    ...
    videoUrl: v.video_url, thumbUrl: v.thumb_url, srtUrl: v.srt_url, metaUrl: v.meta_url,
...
router.get('/status/:id', auth.required, wrap(async (req, res) => {
  ...
  ok(res, {
    ...
    queue: q,
    result: v.status === 'done' ? {
      videoUrl: v.video_url, thumbUrl: v.thumb_url,
      srtUrl: v.srt_url, metaUrl: v.meta_url,
...
module.exports = { router, runJob, shapeVideo };
