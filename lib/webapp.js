...
const { DIRS, logger, clamp, ffmpegStatus } = require('./util');
...
router.post('/videos', auth.required, wrap(async (req, res) => {
  ...
  /* Refus immédiat et explicite si le moteur de rendu n'est pas installé.
   * Avant ce checkpoint, le téléphone pouvait lancer une file qui échouait
   * beaucoup plus tard avec « spawn ffmpeg ENOENT ». */
  const render = await ffmpegStatus();
  if (!render.ready) {
    throw Object.assign(new Error(
      `Moteur vidéo indisponible : FFmpeg/FFprobe prêt requis (${render.error || render.ffmpegPath}).`,
  ...
  const brief = {
    ...
    style: ['ecofin', 'brut', 'moneyradar', 'doc', 'bankable', 'impact', 'cinema', 'viral'].includes(b.style) ? b.style : 'ecofin',
    ...
    /* ── CHAMPS PERDUS EN ROUTE ───────────────────────────────────────
     * Cette route ne recopie PAS `req.body` : elle reconstruit le brief
     * champ par champ, en liste blanche. Tout champ absent de cette liste
     * est donc silencieusement supprimé, même s'il est correctement
     * envoyé par l'interface et correctement lu par le pipeline.
     *
     * C'est ce qui s'est produit pour le choix de voix : le sélecteur
     * envoyait bien 'F', `resolveVoiceLock` savait l'exploiter, mais la
     * valeur n'arrivait jamais — d'où « Voix verrouillée : Rémy » alors
     * que la voix féminine avait été demandée.
     * `consignes` et `scriptFourni` subissaient exactement le même sort
     * sur ce chemin. */
    voiceGender: ['M', 'F'].includes(String(b.voiceGender || '').toUpperCase())
      ? String(b.voiceGender).toUpperCase() : '',
    voiceId: String(b.voiceId || '').slice(0, 80),
    consignes: String(b.consignes || '').slice(0, 2000),
    scriptFourni: String(b.scriptFourni || '').slice(0, 40000),
  };
