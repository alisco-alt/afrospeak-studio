...
async function _executer(projectId, userId, lancer) {
  ...
  try {
    ...
    if (!p.result) {
      /* Ne masque jamais l'erreur réelle du pipeline derrière le message
       * générique « Rendu sans résultat ». Les logs utilisateur doivent
       * exposer le premier diagnostic utile (quota, script, timeout, etc.). */
      if (p.status === 'error' || p.error) {
        throw new Error(String(p.error || 'Échec du pipeline'));
      }
      throw new Error('Rendu sans résultat');
    }
