...
const FEEDS = [
  ...
  { id: 'africanarguments', name: 'African Arguments', url: 'https://africanarguments.org/feed/', lang: 'en', tags: ['analyse'] },
...
async function news({ sources = [], query = '', limit = 40, maxAgeHours = 0 } = {}) {
  /* Sans sélection explicite : les 10 premiers flux PLUS les flux de
   * ligne éditoriale — la veille doit VOIR les fronts de souveraineté,
   * d'unité et de transformation, pas seulement les généralistes. */
  const wanted = sources && sources.length
    ? FEEDS.filter(f => sources.includes(f.id))
    : FEEDS.filter(f => FEEDS.indexOf(f) < 10 || FEEDS_LIGNE.includes(f.id));
