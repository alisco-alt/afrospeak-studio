...
/* Sonde unique et réutilisable : le serveur, le diagnostic et le smoke test
 * doivent tous vérifier LE MÊME binaire que les appels de rendu. */
let _ffmpegStatusCache = null;
let _ffmpegStatusAt = 0;
async function ffmpegStatus({ force = false } = {}) {
  if (!force && _ffmpegStatusCache && Date.now() - _ffmpegStatusAt < 30000) {
    return _ffmpegStatusCache;
  }
  const out = {
    ready: false, ffmpeg: false, ffprobe: false, libass: false, libx264: false,
    ffmpegPath: FFMPEG, ffprobePath: FFPROBE, error: '', checkedAt: new Date().toISOString(),
  };
  try {
    await run(FFMPEG, ['-hide_banner', '-version'], { timeout: 15000 });
    out.ffmpeg = true;
    const filters = await run(FFMPEG, ['-hide_banner', '-filters'], { timeout: 15000 });
    out.libass = /(^|\s)ass\s/.test(filters.stdout || '');
    const encoders = await run(FFMPEG, ['-hide_banner', '-encoders'], { timeout: 15000 });
    out.libx264 = /libx264/.test(encoders.stdout || '');
  } catch (e) {
  ...
  try {
    ...
    if (!out.error) out.error = String(e.message || e).slice(0, 240);
  }
  out.ready = out.ffmpeg && out.ffprobe && out.libass && out.libx264;
  _ffmpegStatusCache = out;
  _ffmpegStatusAt = Date.now();
  return out;
}
...
/* ── COMBIEN DE CPU/RAM NOUS EST VRAIMENT ATTRIBUÉ ? ─────────────────
 * `os.cpus().length` et `os.totalmem()` renvoient la MACHINE, pas le
 * QUOTA du conteneur. Docker, Kubernetes, WSL2, CI et la plupart des
 * sandbox limitent via cgroups : un conteneur« 2 cœurs » sur une machine
 * 32 cœurs voit 32 — chaque mesure de parallélisme se retrouve alors
 * 16× trop optimiste, et chaque plan rendu « en parallèle » combat tous
 * les autres pour le même CPU (c'est exactement la signature d'une
 * génération qui traîne : 50 min pour un short).
 * Lecture : quota/période de cpu.cfs_quota_us, mémoire de
 * memory/memory.max (cgroup v2) ou memory.limit_in_bytes (v1). */
function _litCgroup(dans, fichier) {
  try {
    const p = path.join(dans, fichier);
    if (!fs.existsSync(p)) return null;
    const v = fs.readFileSync(p, 'utf8').trim();
    return v === 'max' ? null : Number(v);
  } catch (e) { return null; }
}
function coeursDisponibles() {
  if (process.env.AFROSPEAK_CORES) return Math.max(1, Number(process.env.AFROSPEAK_CORES));
  const physique = require('os').cpus().length;
  // cgroup v2 : /sys/fs/cgroup/cpu.max → "quota période" (ex. "200000 100000")
  try {
    const p = '/sys/fs/cgroup/cpu.max';
    if (fs.existsSync(p)) {
      const [q, per] = fs.readFileSync(p, 'utf8').trim().split(/\s+/);
      if (q !== 'max' && per) return Math.max(1, Math.floor(Number(q) / Number(per)));
    }
  } catch (e) {}
  // cgroup v1 : quota + période séparés
  const q = _litCgroup('/sys/fs/cgroup/cpu', 'cpu.cfs_quota_us');
  const per = _litCgroup('/sys/fs/cgroup/cpu', 'cpu.cfs_period_us');
  if (q > 0 && per > 0) return Math.max(1, Math.floor(q / per));
  return physique;
}
function memoireDisponibleGo() {
  if (process.env.AFROSPEAK_MEM_GO) return Math.max(0.2, Number(process.env.AFROSPEAK_MEM_GO));
  // cgroup v2 puis v1 : le plus petit des deux gagne (le conteneur ne peut
  // jamais dépasser son quota, même si la machine a plus).
  const lims = [];
  const v2 = _litCgroup('/sys/fs/cgroup', 'memory.max');
  if (v2 > 0) lims.push(v2);
  const v1 = _litCgroup('/sys/fs/cgroup/memory', 'memory.limit_in_bytes');
  if (v1 > 0 && v1 < Number.MAX_SAFE_INTEGER / 2) lims.push(v1);
  const machine = require('os').totalmem();
  const limite = lims.length ? Math.min(machine, ...lims) : machine;
  return limite / 1e9;
}
...
const FF_THREADS = (() => {
  ...
  if (env > 0) return env;
  const n = coeursDisponibles();
  const memGo = memoireDisponibleGo();
...
/* ── CACHE DE SONDE ──
 * `mediaInfo` est appelé des dizaines de fois par production SUR LE MÊME
 * fichier (renderShot, autoCrop, concat, gardes-fous de durée…). Chaque
 * appel lance un ffprobe : sur un montage de 30 plans, cela représente
 * des centaines de processus. On cache par (chemin, mtime, taille) — une
 * donnée invalide est impossible tant que le fichier n'est pas réécrit. */
const _probeCache = new Map();
async function probe(file) {
  let cle = file;
  try {
    const st = fs.statSync(file);
    cle = `${file}|${st.mtimeMs}|${st.size}`;
  } catch (e) { /* fichier absent : on tente quand même, l'erreur sera parlante */ }
  if (_probeCache.has(cle)) return _probeCache.get(cle);
  const { stdout } = await run(FFPROBE, [
    '-v', 'error', '-print_format', 'json', '-show_format', '-show_streams', file,
  ]);
  const j = JSON.parse(stdout);
  if (_probeCache.size > 900) _probeCache.clear();
  _probeCache.set(cle, j);
  return j;
}
...
module.exports = {
  ...
  readJSON, writeJSON, run, ffmpeg, ffmpegStatus, probe, mediaInfo, audioDuration,
