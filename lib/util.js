...
async function fetchBuf(url, opts = {}) {
  ...
  // Court-circuit : si ce domaine a échoué récemment par connexion, on ne réessaie pas
  const dom = _domaine(url);
  if (dom && _domaineEstMort(dom)) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (connFail && dom) {
        ...
        // Pas la peine de réessayer un domaine mort (pour l'instant)
        break;
      }
