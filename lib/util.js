  ...
  return new Promise((resolve, reject) => {
    ...
    const armWatchdog = () => {
      ...
      watchdog = setTimeout(() => {
        ...
        const mins = (inactivityTimeoutMs / 1000).toFixed(0);
        const e = new Error(`${label} : aucune activité depuis ${mins}s — process tué (blocage probable du graphe de filtres)`);
        e.stderr = errBuf;
        e.code = 'FFMPEG_WATCHDOG';
        reject(e);
      }, inactivityTimeoutMs);
    };
    armWatchdog();
...
/* ── DÉFAILLANCE DE DOMAINE : MÉMOIRE DE COURT-CIRCUIT ──
 ...
 * mort pour le reste de la session. */
const _domaineMort = new Set();
...
async function fetchBuf(url, opts = {}) {
  ...
  // Court-circuit : si ce domaine a déjà échoué par connexion, on ne réessaie pas
  const dom = _domaine(url);
  if (dom && _domaineMort.has(dom)) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    // Timeout dégressif : 8 s au premier essai, 5 s au retry. Inutile
    // d'attendre 20 s pour un domaine qui ne répond pas.
    const tNow = attempt === 0 ? Math.min(timeout, 8000) : Math.min(timeout, 5000);
    ...
    try {
      ...
      clearTimeout(t);
      lastErr = e;
      // Erreur de CONNEXION (pas une erreur HTTP) : le domaine est probablement
      // injoignable. On le marque et on ne réessaie pas.
      const code = e.cause && e.cause.code;
      const connFail = code === 'ECONNREFUSED' || code === 'ENOTFOUND'
        || code === 'ECONNRESET' || code === 'EAI_AGAIN'
        || code === 'CERT_HAS_EXPIRED' || code === 'UNABLE_TO_VERIFY_LEAF_SIGNATURE'
        || String(e.message).startsWith('fetch failed');
      if (connFail && dom) {
        _domaineMort.add(dom);
        // Pas la peine de réessayer un domaine mort
        break;
      }
      // AbortError = timeout : on réessaie une fois, puis on court-circuite aussi
      if (e.name === 'AbortError' && attempt >= 1 && dom) {
        _domaineMort.add(dom);
        break;
      }
