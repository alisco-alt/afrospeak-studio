...
async function analyzeTopic(topic, articles, opts = {}) {
  ...
  try {
    ...
    const res = await llm.chat([
      {
        role: 'system',
        content: `Tu es le rédacteur en chef d'une chaîne YouTube panafricaine (AfroSpeak).
Ta mission : transformer une actualité en ANGLE documentaire qui sert l'émancipation
africaine, l'éveil des consciences et la souveraineté du continent.

═══ LIGNE ÉDITORIALE — ÉMANCIPATION & SOUVERAINETÉ ═══
Chaque sujet est lu DEPUIS l'Afrique, pour les Africains et la diaspora.
L'angle n'est jamais neutre : il oriente vers QUI DÉCIDE, QUI POSSÈDE,
QUI CAPTE LA VALEUR, et surtout — QU'EST-CE QUI REND AUTONOME ?

RÈGLES D'ANALYSE :
1. Identifie le VÉRITABLE enjeu derrière l'actualité : pas ce que dit la dépêche,
   mais le RAPPORT DE FORCE — qui décide, qui possède, qui capte la valeur.
...
4. L'angle doit servir l'émancipation : montrer un levier concret d'autonomie,
   une transformation possible, une comparaison qui élève — jamais un constat
   d'impuissance.
5. L'angle doit permettre un script de 3 à 15 minutes sans redondance.
6. Évalue le potentiel viral ET la valeur conscientisante : est-ce que ça
   donne envie de cliquer ET de comprendre ?

STYLE D'ANGLES QUI MARCHENT :
- Le paradoxe souverain : « Premier producteur mondial. Et pourtant il importe tout. »
...
- Le levier d'autonomie : « Voici comment le Ghana a repris le contrôle. »`,
      },
      {
        role: 'user',
...
async function scanForTopics(opts = {}) {
  ...
  try {
    ...
    /* Ligne éditoriale : la boussole émancipation/unité/souveraineté. */
...
En tant que rédacteur en chef, propose ${limit} sujets de vidéos d'investigation à fort potentiel viral.${ligne.blocPrompt()}
...
JSON strict :
{
  "proposals": [
    ...
    /* Bonus d'alignement ligne éditoriale : les sujets souveraineté/unity/
     * éveil remontent. Les étiquettes themeLigne/motsLigne restent dans la
     * proposition pour l'audit. */
    ligne.reoriente(proposals);

    onLog(proposals.length + ' sujets proposés par le rédacteur en chef'
      + (proposals.find(p => p.themeLigne)
        ? ` — alignement dominant : ${proposals.find(p => p.themeLigne).themeLigne}` : ''));
    return { proposals };
  } catch (e) {
