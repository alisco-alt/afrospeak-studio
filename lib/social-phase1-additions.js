...
async function downloadYouTubeClip(query, opts = {}) {
  ...
  try {
    ...
    for (const line of (r.stdout || '').split('\n').filter(Boolean)) {
      try {
        ...
        if (dur < 5 || dur > maxDuration * 6) continue; // filtrer les trop courts/longs
        candidates.push({
          id: meta.id,
  ...
  try {
    ...
    // Télécharger une section si startTime/endTime fournis ou si vidéo > maxDuration
    if (startTime != null && endTime != null) {
      args.push('--download-sections', '*' + startTime + '-' + endTime);
      args.push('--force-keyframes-at-cuts');
    } else if (best.duration > maxDuration + 5) {
      // Extraire une section du début (après les 10% pour éviter les intros)
      const start = Math.min(best.duration * 0.1, Math.max(2, best.duration * 0.08));
      const end = start + maxDuration;
      args.push('--download-sections', '*' + start.toFixed(1) + '-' + end.toFixed(1));
      args.push('--force-keyframes-at-cuts');
    }
