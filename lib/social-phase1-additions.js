...
function preuveDroit(opts = {}) {
  if (opts.authorized === true || opts.licensed === true) return true;
  const l = String(opts.license || '').toLowerCase();
  const u = String(opts.licenseUrl || '').trim();
  return !!l && !!u && /creative commons|domaine public|public domain/i.test(l);
}
...
async function downloadYouTubeClip(query, opts = {}) {
  const {
    ...
    /* Le téléchargement automatique exige une licence vérifiée. Une
     * citation ou un crédit affiché ne sont pas une permission. */
    requireLicense = true,
  } = opts;
  ...
  try {
    ...
    for (const line of (r.stdout || '').split('\n').filter(Boolean)) {
      try {
        ...
        if (requireLicense && !/creative commons|public domain/i.test(String(meta.license || ''))) {
          candidates.pop();
        }
  ...
  try {
    ...
    return {
      ...
      license: best.license || (requireLicense ? 'Creative Commons' : 'Usage éditorial — crédit affiché'),
      licenseUrl: best.licenseUrl || '',
      licenceCC: requireLicense,
...
async function downloadYouTubeMulti(query, opts = {}) {
  ...
  if (!requireLicense) {
    log.warn('YouTube multi : téléchargement refusé sans preuve de licence');
    return [];
  }
  ...
  try {
    ...
    for (const line of (r.stdout || '').split('\n').filter(Boolean)) {
      try {
        ...
        if (requireLicense && !/creative commons|public domain/i.test(license)) continue;
        candidates.push({
          id: meta.id, url: 'https://www.youtube.com/watch?v=' + meta.id,
          title: meta.title || '', duration: dur,
          uploader: meta.uploader || meta.channel || '',
          license, licenseUrl: meta.license_url || '',
        });
      } catch (e) {}
    }
  } catch (e) { return []; }
    ...
    if (fs.existsSync(outFile) && fs.statSync(outFile).size > 50000) {
      ...
      if (info && info.hasVideo) {
        results.push({
          ...
          license: cand.license || 'Creative Commons', licenseUrl: cand.licenseUrl || '',
          licenceCC: true, requiresAttribution: true, title: cand.title,
...
async function downloadTikTokVideo(url, opts = {}) {
  /* TikTok ne fournit pas ici de licence exploitable : jamais de
   * téléchargement automatique, même avec cookies ou watermark conservé. */
  if (!preuveDroit(opts)) return null;
  ...
  try {
    ...
    return {
      ...
      license: opts.license || 'Licence autorisée', licenseUrl: opts.licenseUrl || '',
...
async function downloadFacebookVideo(url, opts = {}) {
  /* Aucun téléchargement Facebook sans licence/autorisation structurée. */
  if (!preuveDroit(opts)) return null;
...
async function downloadInstagramVideo(url, opts = {}) {
  /* Aucun téléchargement Instagram sans licence/autorisation structurée. */
  if (!preuveDroit(opts)) return null;
...
function _makeYouTubeAsset(file, meta, url) {
  return {
    ...
    license: meta.license || 'Creative Commons', licenseUrl: meta.licenseUrl || '',
    licenceCC: true, requiresAttribution: true, title: meta.title || '',
