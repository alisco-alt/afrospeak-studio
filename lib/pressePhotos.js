...
const FLUX = [
  ...
  /* ── FLUX AJOUTES APRES MESURE (22/08/2026) ────────────────────────
   * Demande de l'auteur : se connecter aux reseaux sociaux pour capter
   * l'actualite en images. Facebook, X, Instagram et TikTok sont fermes
   * (voir FACEBOOK-ET-EQUIVALENTS.md : six voies testees, toutes en
   * echec — ce n'est pas un defaut de configuration).
   *
   * Mais le besoin reel — des images d'actualite fraiches, sans
   * authentification — est couvert par les redactions qui PUBLIENT ces
   * memes images dans leurs flux. Mesure comparative des candidats :
   *   France24 Afrique   30 items / 30 images  <- retenu
   *   Le Monde Afrique   20 items / 20 images  <- retenu
   *   Al Jazeera         25 items /  0 image
   *   TV5 Monde, APA     HTTP 403
   *   AP News, Ecofin    HTTP 404
   * Les deux retenus portent une image par article, en `media:content`
   * ou `enclosure` — donc exploitables directement, sans og:image. */
  { url: 'https://www.france24.com/fr/afrique/rss', nom: 'France 24 Afrique', img: true },
  { url: 'https://www.lemonde.fr/afrique/rss_full.xml', nom: 'Le Monde Afrique', img: true },
];

/* Presse nationale, interrogée selon le pays détecté dans le sujet. */
/* ── PRESSE LOCALE : LA MEILLEURE SOURCE DE VISUELS PERTINENTS ────────
 * Proposition de l'auteur : « concentrez-vous sur la presse et les
 * créateurs locaux ». Mesuré, et il a raison — le stock généraliste
 * ignore l'Afrique (« protest mali » rendait Vancouver et Barcelone),
 * alors qu'une rédaction locale photographie l'événement lui-même.
 *
 * 24 flux candidats testés le 22/08/2026 ; seuls les VIVANTS sont
 * retenus. Écartés pour cause de flux mort ou inaccessible :
 *   Maliweb, Fratmat, Koaci, ActuNiger, Banouto, GhanaWeb,
 *   Togobreakingnews, Alwihda (HTTP 404 ou 0 item),
 *   Nation Kenya (403), Cameroon-Info (délai dépassé).
 * `img: true` = le flux porte lui-même la photo, donc aucun
...
const FLUX_PAYS = {
  guinee: [
    ...
    { url: 'https://mosaiqueguinee.com/feed/', nom: 'Mosaique Guinee', img: false },
  ],
  senegal: [
    { url: 'https://www.seneweb.com/news/rss.php', nom: 'Seneweb', img: false },
    // 18 items sur 20 portent leur photo dans le flux (mesuré)
    { url: 'https://www.dakaractu.com/xml/syndication.rss', nom: 'Dakaractu', img: true },
    { url: 'https://aps.sn/feed/', nom: 'APS Senegal', img: false },
    { url: 'https://www.seneplus.com/rss.xml', nom: 'SenePlus', img: false },
  ],
  mali: [
    { url: 'https://malijet.com/feed', nom: 'Malijet', img: false },
    { url: 'https://bamada.net/feed', nom: 'Bamada', img: false },
    // 10 items sur 10 avec media:content (mesuré)
    { url: 'https://www.studiotamani.org/feed', nom: 'Studio Tamani', img: true },
  ],
  nigeria: [
    { url: 'https://punchng.com/feed/', nom: 'Punch', img: false },
    { url: 'https://www.premiumtimesng.com/feed', nom: 'Premium Times', img: false },
    { url: 'https://www.vanguardngr.com/feed/', nom: 'Vanguard', img: false },
  ],
  ...
  burkina: [
    { url: 'https://lefaso.net/spip.php?page=backend', nom: 'Lefaso', img: false },
    { url: 'https://burkina24.com/feed/', nom: 'Burkina24', img: false },
  ],
  cameroun: [{ url: 'https://actucameroun.com/feed/', nom: 'Actu Cameroun', img: false }],
  rdc: [
    // 50 items sur 50 avec photo dans la description (mesuré)
    { url: 'https://www.radiookapi.net/feed', nom: 'Radio Okapi', img: true },
    { url: 'https://actualite.cd/feed', nom: 'Actualite CD', img: true },
  ],
  gabon: [{ url: 'https://www.gabonreview.com/feed/', nom: 'Gabonreview', img: false }],
};
...
function parserFlux(xml, source) {
  ...
  for (const b of blocs) {
    ...
    const descBrute = decode((/<description[^>]*>([\s\S]*?)<\/description>/i.exec(b) || [])[1] || '');
    /* ── L'IMAGE EST SOUVENT DANS LA DESCRIPTION ──────────────────────
     * La plupart des rédactions africaines n'exposent ni `media:content`
     * ni `enclosure` : elles insèrent la photo directement dans le corps
     * HTML de la description (souvent échappé en &lt;img src=…&gt;).
     * Vérifié sur Radio Okapi — la photo de l'article
     * (`photos.radiookapi.net/picture/…`) était présente dans le flux et
     * purement ignorée : le studio partait alors chercher `og:image`,
     * soit un aller-retour réseau par article, pour la MÊME photo.
     * On la lit donc directement quand le flux ne la déclare pas.
     * Logos, icônes et pixels de suivi restent écartés. */
    if (!image) {
      const imgs = descBrute.match(/<img[^>]+src="([^"]+)"/gi) || [];
      for (const balise of imgs) {
        const s = (/src="([^"]+)"/i.exec(balise) || [])[1] || '';
        if (!/^https?:/i.test(s)) continue;
        if (/logo|icon|avatar|sprite|pixel|banner|placeholder|1x1|blank|feed|rss/i.test(s)) continue;
        if (/\.svg(\?|$)/i.test(s)) continue;
        /* Certains flux (Lefaso) pointent une vignette de PIÈCE JOINTE :
         * `IMG/pdf/communique_….pdf`. Mesuré : l'extraction rendait un PDF
         * comme s'il s'agissait d'une photo. On exige une vraie image. */
        if (/\.(pdf|docx?|xlsx?|zip|mp3|mp4)(\?|$)/i.test(s)) continue;
        image = decode(s);
        break;
      }
    }
    const desc = descBrute.replace(/<[^>]+>/g, ' ').slice(0, 300);
...
async function chercher(topic, opts = {}) {
  const {
    ...
    /* Une photo publiée dans un flux de presse reste protégée par défaut.
     * Le caller doit fournir une preuve externe explicite (URL de licence
     * et régime) avant d'autoriser son téléchargement. */
    allowPressImages = false, pressLicense = '', pressLicenseUrl = '',
  } = opts;
  const preuvePresse = allowPressImages === true
    && String(pressLicense).trim().length > 0
    && /^https?:\/\//i.test(String(pressLicenseUrl).trim());
  ...
  const assets = [];
  let _jinaUtilise = 0;
  let _imagesPresseEcartees = 0;
  for (const a of articles) {
    if (assets.length >= max) break;
    if (!preuvePresse) {
      _imagesPresseEcartees++;
      continue;
    }
    let url = a.image;
    ...
    try {
      ...
      assets.push({
        ...
        pageUrl: a.lien,
        url,
        license: String(pressLicense).trim(),
        licenseUrl: String(pressLicenseUrl).trim(),
        isVideo: false,
        kind: 'image',
        _rel: a._rel,
        news: true,
  ...
  if (_imagesPresseEcartees) {
    onLog(`Presse africaine : ${_imagesPresseEcartees} image(s) non téléchargée(s) — licence absente`, 'warn');
  }
