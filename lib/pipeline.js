    ...
    if (stages.includes('render')) {
      ...
      if (style.dataCards && process.env.MOTION_SLIDES !== '0') {
        ...
        for (let i = 1; i < p.storyboard.length; i++) {
          ...
          let _retard = null;
          const _mots = (s.voice && s.voice.words) || [];
          if (_mots.length) {
      ...
      if (style.captionMode !== 'none' && allWords.length) {
        ...
        const captionModeRendu = b.format === 'landscape' ? 'phrase' : style.captionMode;
        const assText = await captions.buildASS(motsEcran, {
          ...
          activeBox: !!style.captionActiveBox,
