...
function createProject(brief) {
  ...
  const b = {
    ...
    /* Remplir le cadre par défaut ; `blur` reste disponible pour préserver
     * intégralement une archive au ratio très différent. */
    fitMode: ['auto','crop','blur'].includes(brief.fitMode) ? brief.fitMode : 'crop',
...
async function run(id, { stages = ['script', 'media', 'voice', 'render'] } = {}) {
  ...
  const state = { cancelled: false, children: new Set(), cancelReason: '' };
  ...
  const _globalTimer = setTimeout(() => {
    if (running.has(id)) {
      ...
      state.cancelReason = 'Timeout global';
      /* Marquer l'état ne suffit pas si une Promise attend un processus
       * externe. Les enfants FFmpeg doivent être tués immédiatement :
       * sinon le run peut rester suspendu bien au-delà du plafond (voire
       * produire un temps de 20 h comme observé sur une vidéo réelle). */
      for (const child of state.children) {
        try { child.kill('SIGKILL'); } catch (e) {}
      }
      say(`⚠ Timeout global (${Math.round(GLOBAL_TIMEOUT_MS / 60000)}min) — processus externes arrêtés`, 'warn');
    }
  }, GLOBAL_TIMEOUT_MS);
  _globalTimer.unref();
  ...
  try {
    ...
    const b = p.brief;
    /* La boussole ne doit pas rester une consigne abstraite dans le prompt
     * système. On calcule un angle factuel dès le brief et on le transmet à
     * la planification, à la rédaction, au storyboard et au sourcing. Ainsi
     * « raffinerie en Bourse » devient aussi « où se crée la valeur, qui la
     * contrôle, quelle capacité industrielle reste au Nigeria », sans
     * transformer le reportage en slogan. Un angle saisi par l'utilisateur
     * reste prioritaire. */
    const _verdictLigne = ligne.scoreSujet(`${b.topic} ${b.angle || ''}`);
    const _angleEditorial = b.angle || ligne.anglePour(_verdictLigne.theme)
      || 'Ce que ce fait révèle du contrôle africain : qui décide, qui capte la valeur et quelle marge de manœuvre reste au continent.';
    ...
    if (stages.includes('script')) {
      ...
      if (String(b.scriptFourni || '').trim()) {
        ...
        p.script = await scriptwriter.generate({
          topic: b.topic, angle: _angleEditorial, style: b.style, format: b.format,
      ...
      let _validation = p.script && p.script.importe
        ...
        : scriptwriter.validateScript(p.script, { targetWords: _targetWords, format: b.format });
      ...
      while (!_validation.ok && _repairAttempts < 2) {
        ...
        _repairAttempts++;
        const _motsAvantRepair = _validation.wordCount || 0;
        ...
        try {
          ...
          /* Une réponse identique est un signal fort : le fournisseur ou
           * le modèle n'a pas compris la réparation. Rejouer immédiatement
           * la même cascade brûle quota et plusieurs minutes (0/219 → 0/219
           * observé deux fois). On passe au moteur local structuré, puis au
           * rédacteur en chef, au lieu de refaire un troisième appel jumeau. */
          if (!_validation.ok && (_validation.wordCount || 0) <= _motsAvantRepair) {
            say(`Re-prompt sans amélioration (${_validation.wordCount} mots) — bascule vers le secours structuré`, 'warn');
            break;
          }
        } catch (e) {
      ...
      if (!_validation.ok) {
        ...
        if (_meilleurScript && _meilleurScript !== p.script) {
          ...
          _validation = scriptwriter.validateScript(p.script,
            { targetWords: _targetWords, format: b.format });
      ...
      /* ── GARDE-FOU AVANT DÉPENSER DU MÉDIA ───────────────────────────
       * Un brouillon hors contrat ne doit jamais atteindre la voix, le
       * sourcing ou FFmpeg. Les anciennes versions journalisaient l'alerte
       * puis continuaient, ce qui produisait une vidéo publiable avec un
       * texte trop court ou une structure cassée. La rédaction automatique
       * doit être autonome, mais jamais complaisante : si les réparations
       * ont échoué, on arrête proprement avec un diagnostic actionnable.
       *
       * Un script fourni par l'auteur est déjà exempté de cette validation
       * plus haut et conserve son chemin explicite. */
      if (!p.script.importe) {
        const _finalScriptCheck = scriptwriter.validateScript(p.script,
          { targetWords: _targetWords, format: b.format });
        if (!_finalScriptCheck.ok) {
          const _motif = (_finalScriptCheck.issues || []).join('; ').slice(0, 500)
            || 'contrat éditorial non respecté';
          const _ratioFinal = _targetWords > 0
            ? (_finalScriptCheck.wordCount || 0) / _targetWords : 1;
          /* Une accroche neutre ou une étiquette outro perdue n'est pas
           * une raison suffisante pour perdre tout le run : la copie reste
           * parlable et passe en revue requise. Seuls un body absent ou une
           * narration vide sont structurellement fatals. */
          const _structureFatal = (_finalScriptCheck.issues || []).some(i =>
            /section ['\"]body|narration vide|structure irrécupérable/i.test(String(i)));
          const _strictScript = process.env.SCRIPT_STRICT === '1';
          const _fatalScript = _strictScript || _structureFatal
            || _ratioFinal < (Number(process.env.SCRIPT_MIN_RATIO) || 0.40);
          if (_fatalScript) {
            p.editorialGate = {
              status: 'blocked', reason: _motif,
              checkedAt: new Date().toISOString(),
            };
            say(`⛔ Production bloquée avant médias : ${_motif}`, 'error');
            throw new Error(`SCRIPT_NON_CONFORME : ${_motif}`);
          }
          /* Une copie imparfaite mais exploitable vaut mieux qu'un run
           * entièrement perdu après plusieurs appels LLM et quotas. On
           * conserve la meilleure version, on marque la revue, puis on
           * poursuit vers la voix et les médias. Le mode strict reste
           * disponible avec SCRIPT_STRICT=1. */
          p.editorialGate = {
            status: 'review_required', reason: _motif,
            ratio: +_ratioFinal.toFixed(3), checkedAt: new Date().toISOString(),
          };
          say(`⚠ Script exploitable malgré ${_finalScriptCheck.issues.length} alerte(s) `
            + `(${Math.round(_ratioFinal * 100)} % du volume cible) — production poursuivie : ${_motif}`, 'warn');
        } else {
          p.editorialGate = { status: 'passed', checkedAt: new Date().toISOString() };
        }
      }
      ...
      /* ── AUDIT DES NOMBRES CONTRE LA MATIÈRE ───────────────────────
       * La conformité de forme ne prouve pas qu'un prix, une date ou un
       * pourcentage est sourcé. Chaque nombre prononcé est maintenant
       * comparé au corpus réellement fourni au rédacteur. Un nombre absent
       * des sources bloque avant TTS et médias : produire une belle vidéo
       * avec un chiffre invérifiable est pire que demander une nouvelle
       * rédaction qualitative sans chiffre.
       * Les valeurs écrites en toutes lettres feront l'objet d'un audit
       * sémantique ultérieur ; ce premier verrou couvre les chiffres qui
       * alimentent les slides et les cas observés en production.
       */
      if (!p.script.importe) {
        p.factAudit = factcheck.auditScript(p.script, docs);
        if (!p.factAudit.sourceAvailable) {
          p.factAudit.status = 'review_required';
          say('⚠ Audit factuel non conclusif : aucune matière textuelle disponible — revue éditoriale requise', 'warn');
        } else if (p.factAudit.unverified.length || p.factAudit.unitConflicts.length) {
          const _conflits = (p.factAudit.unitConflicts || []).slice(0, 3)
            .map(x => `valeur ${x.value} avec unités ${x.units.join(' / ')}`);
          const _noms = p.factAudit.unverified.slice(0, 5)
            .map(x => `« ${x.raw} » (plan ${Number(x.shot) + 1})`).concat(_conflits).join(', ');
          const _strictFacts = process.env.FACTCHECK_STRICT === '1';
          p.factAudit.status = _strictFacts ? 'blocked' : 'review_required';
          if (_strictFacts) {
            say(`⛔ Audit factuel bloqué : ${p.factAudit.unverified.length} nombre(s) absent(s) des sources ou contradictoire(s) — ${_noms}`, 'error');
            throw new Error('CHIFFRE_NON_SOURCÉ : ' + _noms);
          }
          /* Par défaut, préserver le fallback autonome : l'audit est
           * bloquant seulement en mode strict explicite. Le chiffre reste
           * signalé dans le projet pour revue, mais un quota LLM ou une
           * source partiellement lisible ne transforme plus un run entier
           * en « Rendu sans résultat ». */
          say(`⚠ Audit factuel non conclusif : ${p.factAudit.unverified.length + (p.factAudit.unitConflicts || []).length} élément(s) à vérifier — ${_noms}`, 'warn');
        } else {
          say(`✓ Audit factuel : ${p.factAudit.verified} nombre(s) traçable(s) dans la matière`);
        }
      }

      /* ── AUDIT DE RÉTENTION ────────────────────────────────────────
       * Ce score ne remplace pas le jugement éditorial et ne bloque pas un
       * sujet important. Il rend visible ce qui transforme une matière juste
       * en vidéo regardable : tension d'ouverture, relances et chute ouverte.
       * Une alerte demande une optimisation au rédacteur, jamais un clickbait
       * ou l'invention d'un fait.
       */
      p.retentionAudit = { ...retention.audit(p.script), checkedAt: new Date().toISOString() };
      if (p.retentionAudit.status === 'strong') {
        say(`✓ Rétention narrative : ${p.retentionAudit.score}/100 — accroche, relances et chute solides`);
      } else {
        say(`⚠ Rétention narrative : ${p.retentionAudit.score}/100 — ${p.retentionAudit.details.join('; ')}`, 'warn');
      }
    ...
    if (stages.includes('media')) {
      ...
      ctxSujet = contexte.analyser(`${b.topic} ${_angleEditorial}`);
      ...
      const seqFiliere = contexte.sequenceFiliere(ctxSujet, `${b.topic} ${_angleEditorial}`);
      ...
      const batchQuery = b.topic + ' ' + _angleEditorial;
      ...
      if (_batchPool.length) {
                  ...
                  try {
                    ...
                    for (const cand of (r || [])) {
                      ...
                      try {
                        ...
                        if (got && got.file) {
                          _batchPool.push({
                            file: got.file, provider: cand.provider || src,
                            title: cand.title || q, platform: 'stock',
                            isVideo: true, duration: got.info && got.info.duration,
                            source: cand.pageUrl || cand.url, requete: q,
                          });
                          _ajoutes++;
                        }
        ...
        const _sujetHistorique = ctxSujet && ctxSujet.epoque === 'historique';
        const _archiveHistorique = a => {
          const hay = [a && a.title, a && a.source, a && a.pageUrl, a && a.url,
            a && a.requete, a && a.provider].filter(Boolean).join(' ').toLowerCase();
          return /histor|archiv|ancient|colonial|vintage|old photograph|19th century|18\d{2}|19\d{2}|dahomey|amazone|amazon warriors|abomey|benin kingdom|royal palace/.test(hay);
        };
        /* Sur un sujet historique, le pool global contient souvent des
         * plans contemporains parfaitement géolocalisés mais faux pour la
         * narration (ex. étudiants modernes pour les Amazones de 1892).
         * On ne les distribue plus par défaut : seuls les assets portant une
         * trace archivistique explicite peuvent servir de premier choix.
         * Les plans restants repassent par la cascade ciblée historique. */
        const _poolVisuel = _sujetHistorique
          ? _batchPool.filter(_archiveHistorique)
          : _batchPool;
        if (_sujetHistorique && _poolVisuel.length < _batchPool.length) {
          say(`Sujet historique : ${_batchPool.length - _poolVisuel.length} asset(s) contemporain(s) écarté(s) du pool global`, 'warn');
        }
        const clips = _poolVisuel.filter(a => a.isVideo);
        const imgs = _poolVisuel.filter(a => !a.isVideo);
        ...
        /* ── PERTINENCE PAR PLAN (mots-clés de la phrase) ─────────────────
         * Le batch ramène des clips du SUJET GLOBAL. Les coller au hasard sur
         * les plans donnait des images qui « ne reflètent pas le texte »
         * (retour utilisateur). On n'assigne donc un clip du batch à un plan
         * que s'il partage au moins un mot-clé avec CE plan (sa requête, sa
         * requête alternative, ses requêtes LLM). Sinon on laisse le plan à la
         * cascade par-plan, qui cherche spécifiquement pour lui — c'est là que
         * la pertinence fine se gagne. */
        const STOPW = new Set(['avec', 'pour', 'dans', 'sur', 'les', 'des', 'une', 'plus',
          'sans', 'leur', 'cette', 'entre', 'afrique', 'africa', 'african', 'africain',
          'africaine', 'video', 'vue', 'image', 'stock', 'footage', 'the', 'and', 'for',
          'with', 'from', 'over', 'into', 'this', 'that']);
        const motsPlan = (shot) => {
          const src = [shot.query, shot.queryAlt].concat(shot.queries || []).filter(Boolean).join(' ');
          return [...new Set(norm(src).split(/[^a-z0-9]+/).filter(w => w.length > 3 && !STOPW.has(w)))];
        };
        // Nombre de mots-clés du plan retrouvés dans le titre/URL de l'asset.
        const overlap = (planWords, asset) => {
          if (!planWords.length) return 0;
          const hay = norm([asset.title, asset.url, asset.source, asset.requete]
            .filter(Boolean).join(' '));
          if (!hay) return 0;
          let hit = 0;
          for (const w of planWords) {
            if (hay.includes(w)) hit += 1;
            else if (w.length > 5 && hay.includes(w.slice(0, w.length - 2))) hit += 0.5;
          }
          return hit;
        };
        /* Un seul mot commun (« Mali », « armes ») ne prouve pas qu'un
         * asset illustre le plan. Deux mots-clés distinctifs minimum
         * évitent les images génériques du batch global ; le plan revient
         * alors à la recherche ciblée quand le match est faible. */
        const SEUIL_BATCH = Number(process.env.BATCH_MATCH_MIN) || 2;

        const _poserAsset = (s, a, ov) => {
          s.asset = {
            file: a.file, provider: a.provider,
            author: a.source || '', pageUrl: a.source || '',
            license: a.license || 'Licence à vérifier', licenseUrl: a.licenseUrl || '',
            title: a.title || '', url: a.url || a.source || '',
            platform: a.platform, social: true,
            _source: 'batch', _reemploye: false, _rel: ov,
          };
          s.credit = a.provider + (a.title ? ' — ' + a.title.slice(0, 50) : '');
        };
        ...
        for (let i = 0; i < total; i++) {
          ...
          for (const a of pool) {
            ...
            const ov = overlap(pw, a);
            if (ov > bestOv) { best = a; bestOv = ov; }
          }
          if (best && bestOv >= SEUIL_BATCH) {
            usedAssets.add(best._gi);
            _poserAsset(s, best, bestOv);
          }
        }

        // 2e passage : combler un plan encore vide avec le meilleur reste
        // (clip OU image), mais toujours sous condition de mot-clé partagé.
        // Un plan sans aucun clip pertinent reste vide : la cascade par-plan
      ...
      for (let i = 0; i < total; i++) {
         ...
         * par recouvrement de mots-clés ≥ SEUIL_BATCH) sur le plan.
      ...
      /* Audit de provenance : une couverture à 100 % ne signifie pas que
       * chaque fichier possède une licence vérifiable. On conserve la
       * production possible (la revue média reste le filet humain), mais
       * on expose clairement les éléments à licence ou URL manquante dans
       * le projet et dans les logs — jamais de faux sentiment de sécurité. */
      const _sansLicence = (p.credits || []).filter(c =>
        !c.license || (!c.licenseUrl && !c.social && !/Illustration générée/i.test(String(c.license))));
      p.mediaAudit = {
        checkedAt: new Date().toISOString(),
        credits: (p.credits || []).length,
        unresolvedLicenses: _sansLicence.length,
        status: _sansLicence.length ? 'review_required' : 'complete',
      };
      if (_sansLicence.length) {
        say(`⚠ Provenance média : ${_sansLicence.length} source(s) sans licence ou URL de licence vérifiable — revue requise`, 'warn');
      } else {
        say(`✓ Provenance média complète : ${p.credits.length} source(s) documentée(s)`);
      }
      saveProject(p);
    }
    ...
    if (stages.includes('render')) {
      ...
      /* Plafond 4 (au lieu de 3) sur les machines à ≥8 cœurs : chaque
       * plan garde ses 2 threads (4×2 = 8, aucune sur-souscription) et
       * le budget mémoire (1,5 Go/plan) reste la contrainte réelle. */
      const _plafond = _coeurs >= 8 ? 4 : 3;
      ...
      if (style.dataCards && process.env.MOTION_SLIDES !== '0') {
        ...
        for (let i = 1; i < p.storyboard.length; i++) {
           ...
           * désynchronisé. */
          /* renderMotionShot remplace encore le plan entier : il ne sait pas
           * superposer la carte à un média déjà en cours. Une avance de
           * 0,2 s suffit donc à faire apparaître le chiffre avant sa
           * prononciation. Mieux vaut conserver le visuel réel que publier
           * une slide désynchronisée. */
          if (_retard > 0.15) continue;
          s.motionStart = 0;
      ...
      for (let i = 0; i < p.storyboard.length; i += front) {
        ...
        for (let k = 0; k < lot.length; k++) {
          ...
          try {
            ...
            try {
              ...
              /* Dernier filet : fond de studio animé, mais plan conservé.
               * Omettre un plan supprimait une phrase de la timeline et
               * créait l'écart image/voix observé dans les rendus longs.
               * Un habillage honnête vaut mieux qu'un trou silencieux. */
              say(`Plan ${lot[k].index + 1} : fallback studio obligatoire — aucun plan ne sera omis`, 'warn');
              const assetAvant = lot[k].asset;
              const motionAvant = lot[k].motion;
              lot[k].asset = null;
              lot[k].motion = null;
              try {
                fichiers[k] = await renderer.renderShot(lot[k], {
                  ...ctx, ffThreads: undefined, kenburns: false,
                  quality: 'draft', _planMaxMs: 120000,
                }, () => {});
                if (fichiers[k]) say(`Plan ${lot[k].index + 1} conservé avec habillage de secours`, 'warn');
              } catch (e4) {
                lot[k].asset = assetAvant;
                lot[k].motion = motionAvant;
                throw new Error(`Plan ${lot[k].index + 1} impossible, y compris fallback studio : ${e4.message}`);
              }
            }
          }
        }
        ...
        _montes += fichiers.filter(Boolean).length;
        ...
        if (rssMB > memLimitMB) {
          ...
          /* Ne jamais purger les exports depuis le chemin mémoire : le
           * renderer peut encore en avoir besoin et un export local est un
           * livrable, pas un cache. La purge est réservée au stockage après
           * téléversement, où elle connaît l'état de conservation. */
          if (global.gc) global.gc();
        }
    ...
    const timedOut = state.cancelReason === 'Timeout global';
    p.status = cancelled ? 'cancelled' : 'error';
    p.error = timedOut ? 'Timeout global — processus externes arrêtés'
      : (cancelled ? 'Annulé' : (e.message || String(e)));
    p.step = timedOut ? 'Arrêté après timeout' : (cancelled ? 'Annulé' : 'Erreur');
    say((timedOut ? '⛔ Timeout global : ' : cancelled ? 'Annulé : ' : 'ERREUR : ')
      + (e.message || e), timedOut || cancelled ? 'warn' : 'error');
    saveProject(p);
...
function getStoryboard(id) {
  ...
  return {
    ...
    timeline: p.timeline,
    mediaAudit: p.mediaAudit || null,
    editorialGate: p.editorialGate || null,
    shots: (p.storyboard || []).map((s, i) => ({
      ...
      asset: s.asset ? {
        ...
        author: s.asset.author || '',
        license: s.asset.license || '',
        licenseUrl: s.asset.licenseUrl || '',
        pageUrl: s.asset.pageUrl || '',
        query: s.asset.query,
