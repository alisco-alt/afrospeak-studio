...
async function run(id, { stages = ['script', 'media', 'voice', 'render'] } = {}) {
  ...
  try {
    ...
    if (stages.includes('render')) {
      ...
      // Sur une petite instance, les plans sont montés en définition réduite
      // puis remis à l'échelle au master : c'est le seul moyen de garder
      // FFmpeg sous les 512 Mo d'un conteneur gratuit.
      const lowMem = renderer.LOW_MEM;
      const workScale = lowMem ? (Number(process.env.WORK_SCALE) || 0.67) : 1;
