  ...
  try {
    ...
    if (stages.includes('script')) {
      ...
      if (!p.script.importe) {
        ...
        if (!p.factAudit.sourceAvailable) {
        ...
        } else if (p.factAudit.unverified.length) {
          const _noms = p.factAudit.unverified.slice(0, 5)
            .map(x => `« ${x.raw} » (plan ${Number(x.shot) + 1})`).join(', ');
          const _strictFacts = process.env.FACTCHECK_STRICT === '1';
          p.factAudit.status = _strictFacts ? 'blocked' : 'review_required';
          if (_strictFacts) {
            say(`⛔ Audit factuel bloqué : ${p.factAudit.unverified.length} nombre(s) absent(s) des sources — ${_noms}`, 'error');
          ...
          say(`⚠ Audit factuel non conclusif : ${p.factAudit.unverified.length} nombre(s) à vérifier — ${_noms}`, 'warn');
        } else {
