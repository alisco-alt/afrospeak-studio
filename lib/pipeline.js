  ...
  try {
    ...
    if (stages.includes('script')) {
      ...
      let _validation = p.script && p.script.importe
        ...
        : scriptwriter.validateScript(p.script, { targetWords: _targetWords, format: b.format, topic: b.topic, angle: b.angle });
      ...
      while (!_validation.ok && _repairAttempts < 2 && !_scriptVide) {
        ...
        try {
          const repairInstructions = scriptwriter.repairPrompt(p.script, _validation.issues, { targetWords: _targetWords, format: b.format, topic: b.topic, angle: b.angle });
          ...
          scriptwriter.updateScriptStats(p.script);
          _validation = scriptwriter.validateScript(p.script, { targetWords: _targetWords, format: b.format, topic: b.topic, angle: b.angle });
      ...
      if (!_validation.ok) {
        ...
        if (_meilleurScript && _meilleurScript !== p.script) {
          ...
          _validation = scriptwriter.validateScript(p.script,
            { targetWords: _targetWords, format: b.format, topic: b.topic, angle: b.angle });
        ...
        if (_ratio < _seuilCroupion) {
          say(`Script inutilisable (${_validation.wordCount} mots pour ${_targetWords} `
            + `attendus, soit ${Math.round(_ratio * 100)} %) — bascule sur le moteur local`, 'warn');
          try {
            ...
            const _vLocal = scriptwriter.validateScript(_local,
              { targetWords: _targetWords, format: b.format, topic: b.topic, angle: b.angle });
            /* On ne remplace que si le repli fait RÉELLEMENT mieux. */
            if (_vLocal.wordCount > _validation.wordCount) {
              p.script = _local;
              scriptwriter.updateScriptStats(p.script);
              _validation = _vLocal;
              say(`Moteur local : ${_vLocal.wordCount} mots — script exploitable`);
            }
          } catch (e) {
    ...
    if (stages.includes('media')) {
      ...
      if (total > 0) {
        ...
        for (let i = 0; i < total; i++) {
          ...
          for (const a of pool) {
            if (usedAssets.has(a._gi)) continue;
      ...
      for (let i = 0; i < total; i++) {
        ...
        if (_mediaStageDead || _budgetLeft < _minParPlanMs) {
          if (!_mediaStageDead) {
            ...
            say(`⚠ Phase visuelle court-circuitée (${raison}) — un seul plan IA isolé au maximum`, 'warn');
        ...
        if (!got && b.social !== false && _citationTiersAutorisee
            && (nYouTube || 0) < _ytMaxClips && !_mediaStageDead) {
          try {
    ...
    if (stages.includes('render')) {
      ...
      p.quality.master = qualityGate.auditMaster(finfo, { format: b.format });
