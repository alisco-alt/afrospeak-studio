...
async function run(id, { stages = ['script', 'media', 'voice', 'render'] } = {}) {
  ...
  try {
    ...
    if (stages.includes('media')) {
      ...
      const _webAssetsAutorises = process.env.ALLOW_UNLICENSED_WEB_ASSETS === '1';
      if (_uncoveredShots.length && !_webAssetsAutorises) {
        say(`Pre-pass Bing ignoré : droits non établis (${_uncoveredShots.length} plans → banques sous licence)`, 'warn');
      } else if (_uncoveredShots.length && _allSourcesDead) {
        ...
        for (let bi = 0; bi < _uncoveredShots.length; bi += _bingBatchSize) {
          ...
          const results = await Promise.allSettled(batch.map(async (shot) => {
            try {
              ...
              const ranked = shotAssets
                .map(asset => {
                  /* Le titre Bing est la requête exacte du plan. Il sert ici
                   * de preuve de recherche ciblée ; ne pas le marquer comme
                   * requête globale, sinon le score l'ignorerait comme
                   * métadonnée technique de batch. */
                  const relevance = visualRelevance.scoreAsset(
                    { ...asset, requete: '' }, shot.shot,
                  );
              ...
              return ranked.length
                ? { idx: shot.idx, query: shot.query, asset: ranked[0].asset,
          ...
          for (const r of results) {
            if (r.status === 'fulfilled' && r.value) {
              ...
              s.asset = {
                ...
                author: r.value.asset.author || '',
                pageUrl: r.value.asset.pageUrl || r.value.asset.source || '',
                license: r.value.asset.license || 'Usage éditorial — droits à vérifier',
                licenseUrl: r.value.asset.licenseUrl || r.value.asset.pageUrl || '',
                requiresAttribution: true, rights: r.value.asset.rights || 'verify',
                title: r.value.asset.title || r.value.query,
                url: r.value.asset.source || '', platform: 'web', social: false,
                _source: 'bing-per-shot', _reemploye: false,
                _visualSubject: b.topic,
                _visualQuery: r.value.query,
                _visualScore: r.value.relevance.score,
                _visualMinimum: r.value.relevance.minimum,
                _visualHits: r.value.relevance.hits,
              };
      ...
      for (let i = 0; i < total; i++) {
        ...
        const wantSocialHere = process.env.ALLOW_THIRD_PARTY_CITATION === '1'
          && b.social && !got && socialFails < 4 && budgetLeft
        ...
        const _citationTiersAutorisee = process.env.ALLOW_THIRD_PARTY_CITATION === '1';
    ...
    if (stages.includes('render')) {
      ...
      if (style.dataCards && process.env.MOTION_SLIDES !== '0') {
        const maxSlides = Math.max(1, Math.floor(p.storyboard.length / 6));
        let poses = 0;
        let dernier = -99;
        for (let i = 1; i < p.storyboard.length; i++) {
          if (poses >= maxSlides) break;
          const s = p.storyboard[i];
          const f = s && s.figure;
          if (!f || !f.value) continue;
          if (i - dernier < 4) continue;            // jamais deux de suite
          if (s.duration && s.duration < 1.8) continue;  // trop bref pour être lu
          ...
          /* Le montage remplace le plan ENTIER par la slide : on ne peut
           * donc pas l'insérer en cours de plan sans réécrire la
           * timeline. Plutôt que d'afficher un carton avant la parole —
           * le défaut signalé — on n'habille QUE les plans où le chiffre
           * tombe tôt (première moitié). Les autres gardent leur visuel
           * réel, ce qui est de toute façon préférable à un carton
           * désynchronisé. */
          // Même une donnée prononcée plus tard reste synchronisable : la
          // slide est préfixée par la durée exacte calculée ci-dessus.
          if (_retard >= _dur - 0.25) continue;
