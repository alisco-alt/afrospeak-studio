...
const {
  DIRS, uid, slug, writeJSON, readJSON, clamp, logger, mediaInfo, fetchBuf,
} = require('./util');
      ...
      for (let i = 0; i < total; i++) {
        ...
        if (_mediaStageDead || Date.now() - _mediaStartMs > _MEDIA_BUDGET_MS) {
          if (!_mediaStageDead) {
            ...
            say(`⚠ Phase visuelle court-circuitée (${raison}) — réemploi pour les ${total - i} plans restants`, 'warn');
          }
          /* tombe directement dans la logique de réemploi ci-dessous */
          let got = null;
          /* saute les fetchs, va au reemploi */
          {
        ...
        if (!got && b.aiAssets !== false && aiassets.disponible() && !_allSourcesDead) {
          try {
            ...
            const gen = anime
              ? await aiassets.genererSequence(req, {
                format: b.format, style: b.style, sujet: b.topic,
