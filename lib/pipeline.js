  ...
  try {
    ...
    if (stages.includes('media')) {
      ...
      if (_batchPool.length) {
        ...
        let clipIdx = 0, imgIdx = 0;
        for (let i = 0; i < total && (clipIdx < clips.length || imgIdx < imgs.length); i++) {
          const s = p.storyboard[i];
          if (s.assetLocked && s.asset) continue;
          if (s.asset) continue; // déjà couvert

          const wantVideo = (s.kind === 'broll') && (s.duration > 2.5);
          let chosen = null;
          if (wantVideo && clipIdx < clips.length) {
            chosen = clips[clipIdx++];
          } else if (imgIdx < imgs.length) {
            chosen = imgs[imgIdx++];
          } else if (clipIdx < clips.length) {
            chosen = clips[clipIdx++];
          }
          ...
          if (chosen) {
            s.asset = {
              ...
              author: chosen.source || '', pageUrl: chosen.source || '',
              license: 'Usage éditorial', licenseUrl: '',
              title: chosen.title || '', url: chosen.source || '',
              platform: chosen.platform, social: true,
              _source: 'batch', _reemploye: false,
            };
            s.credit = chosen.provider + (chosen.title ? ' — ' + chosen.title.slice(0, 50) : '');
          }
        }
