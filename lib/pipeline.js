...
async function run(id, { stages = ['script', 'media', 'voice', 'render'] } = {}) {
  ...
  try {
      ...
      for (let i = 0; i < total; i++) {
        ...
        try {
          const v = await tts.speak(s.narration, {
            provider: voiceProvider, lang: b.language, voiceId: b.voiceId, wpm: style.wpm,
          });
    ...
    const gap = style.captionMode === 'karaoke' ? 0.18 : 0.28;
    let t = 0;
    for (const s of p.storyboard) {
      const vd = (s.voice && s.voice.duration) || 2.5;
      const minShot = style.shotSeconds[0];
      s.audioStart = +(t + gap * 0.5).toFixed(3);
      s.duration = +Math.max(minShot * 0.6, vd + gap).toFixed(3);
      t = +(t + s.duration).toFixed(3);
    }
    // Découpe des plans trop longs : un plan long = plusieurs visuels
    if (b.broll) splitLongShots(p, style);
    ...
    if (stages.includes('media')) {
      ...
      for (let i = 0; i < total; i++) {
        ...
        if (!got) {
          const queries = [s.query, s.queryAlt, `${b.topic} africa`, 'africa business city'].filter(Boolean);
          try {
    ...
    if (stages.includes('render')) {
      ...
      if (style.captionMode !== 'none' && allWords.length) {
        ...
        fs.writeFileSync(assFile, captions.buildASS(allWords, {
          ...
          upper: style.captionUpper, primary: '#FFFFFF',
          highlight: ch.primary, outline: '#000000',
        }));
      }
      ...
      const outFile = await renderer.mux(videoFile, audioFile, assFile, ctx, totalDuration, frac => {
