...
async function run(id, { stages = ['script', 'media', 'voice', 'render'] } = {}) {
  ...
  // Timeout global : 10 minutes max. Si depasse, on sauvegarde et on rend
  // avec ce qui existe deja (le catch en bas gere l'erreur proprement).
  const GLOBAL_TIMEOUT_MS = Number(process.env.PIPELINE_TIMEOUT_MS) || 600000;
  ...
  try {
    ...
    for (let si = 0; si < p.storyboard.length; si++) {
      ...
      // Règle des 3 secondes : minimum 3s pour les styles documentaires
      // (doc, cinema, default) pour maintenir l'attention visuelle.
      // Les styles rapides (brut, impact, moneyradar) gardent leur floor original.
      const minFloor = ['doc', 'cinema', 'default'].includes(style.id)
        ? Math.max(minShot * 0.6, Number(process.env.MIN_SHOT_SECONDS) || 3.0)
        : minShot * 0.6;
    ...
    if (stages.includes('media')) {
      ...
      const _MEDIA_BUDGET_MS = Number(process.env.MEDIA_BUDGET_MS) || 300000;
      const _PLAN_TIMEOUT_MS = Number(process.env.PLAN_TIMEOUT_MS) || 25000;
      ...
      const _perShotBudget = Math.min(_MEDIA_BUDGET_MS / total, 30000);
      ...
      /* Reset du timer budget : le batch sourcing a déjà consommé du temps,
       * mais le budget per-segment doit couvrir les plans RESTANTS, pas
       * pénaliser le batch qui vient de couvrir la moitié des plans. */
      ...
      for (let i = 0; i < total; i++) {
        ...
        /* ── RECHERCHE CIBLÉE PAR PLAN (Bing images) ──
         * Si le batch n'a pas couvert ce plan, on fait une recherche Bing
         * ciblée avec la query LLM du plan AVANT de tomber sur l'IA.
         * C'est la différence entre une VRAIE photo du sujet et une
         * illustration IA abstraite. On ne le fait que si le plan n'a
         * pas déjà un asset (du batch) et si on n'est pas en mode
         * court-circuit (budget épuisé). */
        if (!s.asset && !_mediaStageDead && s.queries && s.queries.length) {
          try {
            const shotQuery = s.queries[0] || s.query || b.topic;
            const shotAssets = await batchSource.newsImageBatch(shotQuery, {
              onLog: () => {}, maxImages: 3,
            });
            if (shotAssets.length) {
              const chosen = shotAssets[0];
              s.asset = {
                file: chosen.file, provider: chosen.provider,
                author: '', pageUrl: '', license: 'Usage éditorial',
                licenseUrl: '', title: chosen.title || shotQuery,
                url: '', platform: 'web', social: false,
                _source: 'bing-per-shot', _reemploye: false,
              };
              s.credit = 'Source: Bing — ' + shotQuery.slice(0, 40);
              say(`Plan ${i + 1} : photo réelle Bing « ${shotQuery.slice(0, 30)} »`);
              continue; // ce plan est couvert, passer au suivant
            }
          } catch (e) { /* Bing échoue → fallback IA/réemploi */ }
        }
...
async function resegmentByMeaning(p, style, say) {
  ...
  const maxPlans = mode2 === 'reel' ? 25 : Math.min(60, Math.max(before, before * 3));
  if (out.length > maxPlans) {
    ...
    while (out.length > maxPlans) {
      ...
      // Fusionner : garder la narration du premier, sommer les durées
      out[bestIdx] = {
        ...out[bestIdx],
        duration: +(out[bestIdx].duration + out[bestIdx + 1].duration).toFixed(3),
        narration: (out[bestIdx].narration || '') + ' ' + (out[bestIdx + 1].narration || ''),
      };
