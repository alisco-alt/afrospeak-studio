...
async function run(id, { stages = ['script', 'media', 'voice', 'render'] } = {}) {
  ...
  const GLOBAL_TIMEOUT_MS = Number(process.env.PIPELINE_TIMEOUT_MS) || 2700000;
    ...
    if (stages.includes('render')) {
      ...
      const front = Number(process.env.MONTAGE_PARALLELE)
        || (renderer.LOW_MEM || _memGo < 3 ? 1
          : Math.max(1, Math.min(4, Math.floor(Math.min(_memGo / 2.5, _coeurs / 2)))));

      if (front > 1) {
        say(`Montage en parallèle : ${front} plans à la fois (${_coeurs} cœurs, ${_memGo.toFixed(1)} Go)`);
      }
      ...
      for (let i = 0; i < p.storyboard.length; i += front) {
        ...
        /* Les plans d'un même lot sont indépendants : chacun écrit son
         * propre fichier `shot_XXX.mp4`. En cas d'échec de l'un, on ne
         * perd pas les autres — `renderShot` lève, la tâche s'arrête,
         * mais les fichiers déjà produits restent en cache disque et
         * seront réutilisés au prochain essai. */
        const fichiers = await Promise.all(lot.map((s, k) => renderer.renderShot(s, ctx, frac => {
          const base = 0.63 + 0.20 * ((i + k) / p.storyboard.length);
          p.progress = clamp(base + (0.20 / p.storyboard.length) * frac, 0, 1);
        })));

        lot.forEach((s, k) => clips.push({ file: fichiers[k], duration: s.duration }));
        _montes += lot.length;
