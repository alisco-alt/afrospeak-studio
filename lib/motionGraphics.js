...
async function _renderAssClip({
  ...
  accentBar = true, force = false, grid = false,
}) {
...
async function dataSlide({
  ...
  const portrait = H >= W;
  const key = sha1(['data', value, label, source, kicker, part, duration, W, H, accent]).slice(0, 12);
  ...
  if (_p > 0.004) {
    ...
    ev.push(`Dialogue: 1,${assTime(0.45)},${assTime(duration)},Fill,,0,0,0,,{\\pos(${gx},${gy})\\fad(300,300)\\fscx0\\t(0,${d1},\\fscx70)\\t(${d1},${d2},\\fscx100)}${assRect(plein, gh)}`);
  }

  return _renderAssClip({ W, H, duration, bg, accent, styles, events: ev, workDir, cacheKey: key, force });
}
...
async function animatedBarChart({
  ...
  duration = 5, title = null, source = '', force = false,
}) {
  ...
  const portrait = H >= W;
  const S = Math.min(W, H);
  const key = sha1(['bar', JSON.stringify(rows), title, source, duration, W, H, accent]).slice(0, 12);
  ...
  for (let i = 0; i < n; i++) {
    ...
    // Barre animée
    ev.push(`Dialogue: 1,${assTime(delay)},${assTime(duration)},Bar${ci},,0,0,0,,{\\pos(${marginX},${barY})\\fad(200,200)\\fscx0\\t(${d1},${d2},\\fscx100)}${assRect(plein, barH)}`);
...
async function comparisonSlide({
  ...
  W = 1080, H = 1920, workDir, accent = DEFAULTS.accent, bg = DEFAULTS.bg, force = false,
}) {
  if (!left || !right) throw new Error('comparisonSlide: left et right requis');
  const portrait = H >= W;
  const S = Math.min(W, H);
  const key = sha1(['cmp', JSON.stringify(left), JSON.stringify(right), title, source, duration, W, H]).slice(0, 12);
...
async function animatedLine({
  ...
  duration = 4, title = null, source = '', force = false,
}) {
  if (!points || points.length < 2) throw new Error('animatedLine: au moins 2 points');
  const S = Math.min(W, H);
  const key = sha1(['line', JSON.stringify(points), title, source, duration, W, H, accent]).slice(0, 12);
  ...
  for (let i = 0; i < segCount; i++) {
    ...
    ev.push(`Dialogue: 1,${assTime(t0)},${assTime(duration)},Ln,,0,0,0,,{\\an4\\pos(${a.x},${a.y})\\frz${ang.toFixed(2)}\\fscx0\\t(${t1a},${t1b},\\fscx100)}${assRect(len, thick)}`);
  }
  // Points
  for (let i = 0; i < px.length; i++) {
    const r = Math.max(5, Math.round(S * 0.010));
    const dt = Math.round((i * per) * 1000);
    ev.push(`Dialogue: 2,${assTime(i * per)},${assTime(duration)},Ln,,0,0,0,,{\\an5\\pos(${px[i].x},${px[i].y})\\fad(150,0)\\fscx0\\fscy0\\t(${dt},${dt + 200},\\fscx100\\fscy100)}${assRect(r * 2, r * 2)}`);
  }
...
async function quoteCard({
  ...
  const S = Math.min(W, H);
  const key = sha1(['quote', quote, author, duration, W, H]).slice(0, 12);
...
async function chapterMarker({
  ...
  const S = Math.min(W, H);
  const key = sha1(['chap', number, title, duration, W, H]).slice(0, 12);
  ...
  const ev = [];
  ev.push(`Dialogue: 0,${assTime(0)},${assTime(duration)},Bar,,0,0,0,,{\\pos(${Math.round((W - barW) / 2)},${Math.round(H * 0.30)})\\fad(200,200)\\fscx0\\t(0,500,\\fscx100)}${assRect(barW, barH)}`);
...
async function statTile({
  ...
  side = 'center', force = false,
}) {
  const S = Math.min(W, H);
  const key = sha1(['stat', value, label, duration, W, H, side]).slice(0, 12);
