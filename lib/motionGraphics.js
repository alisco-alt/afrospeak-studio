...
async function dataSlide({
  value, label = '', source = '', duration = 3,
  W = 1080, H = 1920, workDir, accent = DEFAULTS.accent, bg = DEFAULTS.bg,
  force = false,
}) {
  const out = path.join(workDir || DIRS.cache, 'motion',
    `slide_${sha1([value, label, source, duration, W, H, accent]).slice(0, 12)}.mp4`);
  fs.mkdirSync(path.dirname(out), { recursive: true });
  if (!force && fs.existsSync(out)) return out;
  ...
  const tailleVal = Math.round(Math.min(W, H) * 0.20);
  const tailleLab = Math.round(Math.min(W, H) * 0.045);
  const tailleSrc = Math.round(Math.min(W, H) * 0.026);
...
Dialogue: 0,${t(0)},${t(duration)},Val,,0,0,0,,{\\an5\\pos(${Math.round(W / 2)},${Math.round(H * 0.38)})\\fad(350,350)}${esc(value)}
${label ? `Dialogue: 0,${t(0.15)},${t(duration)},Lab,,0,0,0,,{\\an5\\pos(${Math.round(W / 2)},${Math.round(H * 0.525)})\\fad(400,350)}${esc(label)}` : ''}
${source ? `Dialogue: 0,${t(0.3)},${t(duration)},Src,,0,0,0,,{\\an5\\pos(${Math.round(W / 2)},${Math.round(H * 0.60)})\\fad(450,350)}${esc(source)}` : ''}
`;
  ...
  await ffmpeg([
    ...
    '-vf', `drawbox=x=0:y=0:w=${W}:h=${barre}:color=${accent}@1:t=fill,`
      + `ass='${assPath}':fontsdir='${fontsDir}',format=yuv420p`,
