...
async function dataSlide({
  value, label = '', unit = '', period = '', source = '', duration = 3, kicker = '', part = 0,
  W = 1080, H = 1920, workDir, accent = DEFAULTS.accent, bg = DEFAULTS.bg,
  force = false, fps = 25,
}) {
  /* Défense supplémentaire pour les scripts importés ou les remplacements
   * manuels : les cartes affichent toujours les nombres en chiffres. */
  value = nombresEnChiffres(value);
  /* Les métadonnées ne sont jamais déduites : si le script les fournit,
   * elles restent visibles sur la carte avec le libellé. */
  const labelComplet = [label, unit, period].filter(Boolean).join(' · ');
  label = labelComplet;
  const portrait = H >= W;
  /* Version du cache : le passage à un seul calque de valeur doit invalider
   * les anciens clips qui contenaient encore le chiffre fantôme. */
  const key = sha1(['data-v3-single-value', value, label, source, kicker, part, duration, W, H, accent, fps]).slice(0, 12);

  const S = Math.min(W, H);
  const baseVal = Math.round(S * (portrait ? 0.24 : 0.30));
  /* Un grand nombre ne doit jamais sortir du cadre. Montserrat Black est
   * plus net et plus institutionnel qu'Anton pour les données économiques ;
   * on réduit seulement les valeurs longues, jamais les petits pourcentages. */
  const approxChars = Math.max(3, String(value || '').length);
  const maxVal = Math.round((W * 0.82) / (approxChars * 0.72));
  const tVal = Math.max(Math.round(S * 0.105), Math.min(baseVal, maxVal));
  ...
  const styles = [
    `Style: Val,Montserrat Black,${tVal},${assCol(DEFAULTS.text)},${assCol(accent)},${assCol(bg)},&H00000000,0,0,0,0,100,100,0,0,1,1,1,5,60,60,0,1`,
  ...
  const ev = [];
  /* Un seul calque porte la valeur : aucun fantôme, aucune seconde glyphée
   * ne peut vibrer ou être interprétée comme un autre chiffre à la lecture. */
  if (kicker) {
...
async function animatedBarChart({
  ...
  const titleComplet = [title, source].filter(Boolean).join(' · ');
  const rows = data.slice(0, 6).map(d => ({
    label: String(d.label || '').slice(0, 40),
    value: Number(d.value) || 0,
    display: nombresEnChiffres(d.display != null ? String(d.display) : fmtNum(Number(d.value) || 0)),
  }));
  ...
  const ev = [];
  if (title) {
    ev.push(`Dialogue: 3,${assTime(0)},${assTime(duration)},Ttl,,0,0,0,,{\\pos(${marginX},${Math.round(H * 0.09)})\\fad(300,250)}${assEsc(String(titleComplet).toUpperCase())}`);
  }
  for (let i = 0; i < n; i++) {
    const r = rows[i];
    const y = topY + i * rowH;
...
async function comparisonSlide({
  left, right, title = null, unit = '', period = '', source = '', duration = 4,
  W = 1080, H = 1920, workDir, accent = DEFAULTS.accent, bg = DEFAULTS.bg, force = false, fps = 25,
}) {
  const titleComplet = [title, unit, period].filter(Boolean).join(' · ');
  if (!left || !right) throw new Error('comparisonSlide: left et right requis');
  /* Les comparaisons peuvent provenir directement de la narration et donc
   * contenir « deux milliards » : normaliser les deux côtés ensemble évite
   * une hiérarchie incohérente (« 2 Md$ VS 220 M$ »). */
  const gauche = { ...left, value: nombresEnChiffres(left.value) };
  const droite = { ...right, value: nombresEnChiffres(right.value) };
  const portrait = H >= W;
  const S = Math.min(W, H);
  const key = sha1(['cmp', JSON.stringify(gauche), JSON.stringify(droite), titleComplet, source, duration, W, H, accent, bg, fps]).slice(0, 12);

  const cL = gauche.color || DEFAULTS.accent2;
  const cR = droite.color || accent;
  const tTitle = Math.round(S * 0.038);
  const longest = Math.max(String(gauche.value || '').length, String(droite.value || '').length);
  const baseVal = Math.round(S * (portrait ? 0.15 : 0.16));
  const maxVal = Math.round((W * (portrait ? 0.72 : 0.34)) / (Math.max(3, longest) * 0.72));
  const tVal = Math.max(Math.round(S * 0.075), Math.min(baseVal, maxVal));
  ...
  const styles = [
    ...
    `Style: ValL,Montserrat Black,${tVal},${assCol(cL)},${assCol(cL)},${assCol(bg)},&H00000000,0,0,0,0,100,100,0,0,1,1,1,5,0,0,0,1`,
    `Style: ValR,Montserrat Black,${tVal},${assCol(cR)},${assCol(cR)},${assCol(bg)},&H00000000,0,0,0,0,100,100,0,0,1,1,1,5,0,0,0,1`,
  ...
  const ev = [];
  if (titleComplet) {
    ev.push(`Dialogue: 3,${assTime(0)},${assTime(duration)},Ttl,,0,0,0,,{\\an5\\pos(${Math.round(W / 2)},${Math.round(H * 0.16)})\\fad(300,250)}${assEsc(String(titleComplet).toUpperCase())}`);
  }

  if (portrait) {
    const yL = Math.round(H * 0.36), yR = Math.round(H * 0.64);
    ev.push(`Dialogue: 2,${assTime(0.15)},${assTime(duration)},ValL,,0,0,0,,{\\an5\\pos(${Math.round(W / 2)},${yL})\\fad(300,250)\\move(${Math.round(W * 0.35)},${yL},${Math.round(W / 2)},${yL},0,280)}${assEsc(gauche.value)}`);
    ev.push(`Dialogue: 2,${assTime(0.30)},${assTime(duration)},Lab,,0,0,0,,{\\an5\\pos(${Math.round(W / 2)},${yL + Math.round(tVal * 0.62)})\\fad(320,250)}${assEsc(gauche.label || '')}`);
    ev.push(`Dialogue: 3,${assTime(0.4)},${assTime(duration)},Vs,,0,0,0,,{\\an5\\pos(${Math.round(W / 2)},${Math.round(H * 0.50)})\\fad(360,250)}VS`);
    ev.push(`Dialogue: 2,${assTime(0.5)},${assTime(duration)},ValR,,0,0,0,,{\\an5\\pos(${Math.round(W / 2)},${yR})\\fad(300,250)\\move(${Math.round(W * 0.65)},${yR},${Math.round(W / 2)},${yR},0,280)}${assEsc(droite.value)}`);
    ev.push(`Dialogue: 2,${assTime(0.65)},${assTime(duration)},Lab,,0,0,0,,{\\an5\\pos(${Math.round(W / 2)},${yR + Math.round(tVal * 0.62)})\\fad(320,250)}${assEsc(droite.label || '')}`);
  } else {
    ...
    ev.push(`Dialogue: 2,${assTime(0.15)},${assTime(duration)},ValL,,0,0,0,,{\\an5\\pos(${xL},${yc})\\fad(300,250)\\t(0,300,\\fscx110\\fscy110)\\t(300,520,\\fscx100\\fscy100)}${assEsc(gauche.value)}`);
    ev.push(`Dialogue: 2,${assTime(0.30)},${assTime(duration)},Lab,,0,0,0,,{\\an5\\pos(${xL},${yc + Math.round(tVal * 0.62)})\\fad(320,250)}${assEsc(gauche.label || '')}`);
    ev.push(`Dialogue: 3,${assTime(0.35)},${assTime(duration)},Vs,,0,0,0,,{\\an5\\pos(${Math.round(W / 2)},${yc})\\fad(360,250)}VS`);
    ev.push(`Dialogue: 2,${assTime(0.5)},${assTime(duration)},ValR,,0,0,0,,{\\an5\\pos(${xR},${yc})\\fad(300,250)\\t(0,300,\\fscx110\\fscy110)\\t(300,520,\\fscx100\\fscy100)}${assEsc(droite.value)}`);
    ev.push(`Dialogue: 2,${assTime(0.65)},${assTime(duration)},Lab,,0,0,0,,{\\an5\\pos(${xR},${yc + Math.round(tVal * 0.62)})\\fad(320,250)}${assEsc(droite.label || '')}`);
  }
  if (source) {
...
async function animatedLine({
  ...
  const ev = [];
  if (title) ev.push(`Dialogue: 3,${assTime(0)},${assTime(duration)},Ttl,,0,0,0,,{\\pos(${padL},${Math.round(H * 0.10)})\\fad(300,250)}${assEsc(String(titleComplet).toUpperCase())}`);
...
async function chapterMarker({
  ...
  const S = Math.min(W, H);
  const titleComplet = String(title || '');
  ...
  ev.push(`Dialogue: 1,${assTime(0.4)},${assTime(duration)},Ttl,,0,0,0,,{\\an5\\pos(${Math.round(W / 2)},${cy + Math.round(H * 0.10)})\\fad(320,300)}${assEsc(String(titleComplet).toUpperCase())}`);
  return _renderAssClip({ W, H, duration, bg, accent, styles, events: ev, workDir, cacheKey: key, force, fps });
}
