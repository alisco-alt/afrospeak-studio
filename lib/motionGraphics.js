 ...
 */
/* ── SLIDE DE DONNÉES ÉCRITE EN ASS (sans drawtext) ───────────────────
 * Tout ce module reposait sur le filtre `drawtext` — 17 usages. Or le
 * binaire fourni par `ffmpeg-static` est compilé SANS ce filtre :
 *   « No such filter: 'drawtext' »
 * Le repli interne échouait pour la même raison. Résultat : aucune slide
 * n'a jamais pu être produite, ce qui explique leur absence totale à
 * l'écran malgré un module complet.
 *
 * Vérifié sur le binaire du projet : `drawtext` ABSENT, mais `ass`,
 * `subtitles`, `drawbox` et `geq` PRÉSENTS. C'est déjà la parade retenue
 * pour les sous-titres (voir captions.js) : on passe donc le texte par
 * un fichier ASS, rendu par libass.
 *
 * La slide reste sobre, dans l'esprit d'une chaîne d'analyse : fond
 * sombre, filet d'accent, le CHIFFRE en grand, son libellé dessous, la
 * source en petit. Apparition et sortie en fondu.
 */
async function dataSlide({
  ...
  const out = path.join(workDir || DIRS.cache, 'motion',
    `slide_${sha1([value, label, source, kicker, part, duration, W, H, accent]).slice(0, 12)}.mp4`);
  fs.mkdirSync(path.dirname(out), { recursive: true });
  if (!force && fs.existsSync(out)) return out;

  /* ASS attend &HAABBGGRR : alpha, puis BLEU-VERT-ROUGE (ordre inversé). */
   ...
   * Codes relevés chez les chaînes de référence (Kurzgesagt, Vox,
   * Bloomberg Originals) et confirmés par l'analyse de 30 explainers :
   *   · 86 % isolent le chiffre dans une large zone vide — au moins 60 %
   *     de l'image reste sombre, l'œil n'a nulle part ailleurs où aller ;
   *   · 71 % emploient une typographie surdimensionnée pour la donnée ;
   *   · 85 % font CROÎTRE l'élément plutôt que de l'afficher d'un bloc ;
   *   · un « kicker » court situe la donnée avant qu'elle n'apparaisse.
   * On applique ces quatre principes. */
  const tailleVal = Math.round(Math.min(W, H) * 0.22);
  const tailleLab = Math.round(Math.min(W, H) * 0.042);
  const tailleSrc = Math.round(Math.min(W, H) * 0.024);
  const tailleKick = Math.round(Math.min(W, H) * 0.030);
  const cx = Math.round(W / 2);
  /* Bloc centré verticalement : le chiffre au centre optique, le reste
   * dessous. Un bloc trop bas laissait un vide en haut de cadre. */
  const yVal = Math.round(H * (kicker ? 0.44 : 0.42));
  const yLab = Math.round(H * (kicker ? 0.60 : 0.58));
  const ySrc = Math.round(H * (kicker ? 0.685 : 0.665));
  const fin = duration.toFixed(2);
  const t = (s) => {
    const m = Math.floor(s / 60);
    return `0:${String(m).padStart(2, '0')}:${(s - m * 60).toFixed(2).padStart(5, '0')}`;
  };

  /* Proportion à représenter : fournie, ou déduite d'un pourcentage
   * présent dans la valeur (« moins de 5 % » -> 0,05). Sans proportion
   * fiable, aucune jauge : une barre fausse est pire que pas de barre. */
  let _p = Number(part);
  ...
  let _jauge = '';
  if (_p > 0.004) {
    const gw = Math.round(W * 0.34);          // largeur du rail
    ...
    const rect = (w) => `{\\p1}m 0 0 l ${w} 0 l ${w} ${gh} l 0 ${gh}{\\p0}`;
    /* Croissance amortie : 70 % de la course en 45 % du temps, puis
     * ralentissement — la décélération recommandée pour laisser lire. */
    const d1 = Math.round(Math.min(900, duration * 450));
    const d2 = Math.round(Math.min(1500, duration * 800));
    _jauge = `Dialogue: 0,${t(0.35)},${t(duration)},Rail,,0,0,0,,{\\pos(${gx},${gy})\\fad(300,300)\\alpha&H70&}${rect(gw)}\n`
      + `Dialogue: 1,${t(0.45)},${t(duration)},Fill,,0,0,0,,{\\pos(${gx},${gy})\\fad(300,300)\\fscx0\\t(0,${d1},\\fscx${Math.round(70)})\\t(${d1},${d2},\\fscx100)}${rect(plein)}`;
  }

  const contenu = `[Script Info]
ScriptType: v4.00+
PlayResX: ${W}
PlayResY: ${H}
WrapStyle: 0
...
Style: Val,Anton,${tailleVal},${ass(DEFAULTS.text)},${ass(accent)},${ass(bg)},&H00000000,0,0,0,0,100,100,2,0,1,0,0,5,60,60,0,1
Style: Lab,Montserrat SemiBold,${tailleLab},${ass(DEFAULTS.text)},${ass(accent)},${ass(bg)},&H00000000,0,0,0,0,100,100,0,0,1,0,0,5,80,80,0,1
Style: Src,Montserrat Regular,${tailleSrc},${ass(DEFAULTS.muted)},${ass(accent)},${ass(bg)},&H00000000,0,0,0,0,100,100,0,0,1,0,0,5,80,80,0,1
Style: Kick,Montserrat Black,${tailleKick},${ass(accent)},${ass(accent)},${ass(bg)},&H00000000,0,0,0,0,100,100,4,0,1,0,0,5,80,80,0,1
Style: Rail,Arial,20,${ass('#26303C')},${ass('#26303C')},${ass('#26303C')},&H00000000,0,0,0,0,100,100,0,0,1,0,0,7,0,0,0,1
Style: Fill,Arial,20,${ass(accent)},${ass(accent)},${ass(accent)},&H00000000,0,0,0,0,100,100,0,0,1,0,0,7,0,0,0,1

[Events]
Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text
${kicker ? `Dialogue: 0,${t(0)},${t(duration)},Kick,,0,0,0,,{\\an5\\pos(${cx},${Math.round(H * 0.30)})\\fad(250,300)}${esc(kicker).toUpperCase()}` : ''}
Dialogue: 0,${t(0.12)},${t(duration)},Val,,0,0,0,,{\\an5\\pos(${cx},${yVal})\\fad(260,320)\\t(0,320,\\fscx108\\fscy108)\\t(320,520,\\fscx100\\fscy100)}${esc(value)}
${label ? `Dialogue: 0,${t(0.30)},${t(duration)},Lab,,0,0,0,,{\\an5\\pos(${cx},${yLab})\\fad(320,320)}${esc(label)}` : ''}
${source ? `Dialogue: 0,${t(0.55)},${t(duration)},Src,,0,0,0,,{\\an5\\pos(${cx},${ySrc})\\fad(380,320)}${esc(source)}` : ''}
${_jauge}`;

  const assFile = out.replace(/\.mp4$/, '.ass');
  fs.writeFileSync(assFile, contenu, 'utf8');

  /* Filet d'accent en haut : `drawbox` est disponible, lui. */
  ...
  /* ── JAUGE QUI CROÎT ──────────────────────────────────────────────
   * 85 % des explainers analysés animent la CROISSANCE de la donnée
   * plutôt que de l'afficher figée : « montrer l'acte », pas le
   * résultat. `drawbox` accepte des expressions dépendant du temps, ce
   * qui permet une barre qui se remplit sans recourir à `drawtext`
   * (absent du binaire).
   * La largeur suit une progression amortie (rapide puis ralentie) :
   * c'est la décélération que recommandent les mêmes analyses, elle
   * laisse le temps de lire au moment où la valeur se fige.
   * `part` (0 à 1) donne le remplissage cible ; à défaut, on le déduit
   * d'un pourcentage présent dans la valeur. */
  /* ── JAUGE RETIRÉE APRÈS CONTRÔLE À L'IMAGE ───────────────────────
   * Une barre de proportion avait été ajoutée (85 % des explainers
   * analysés animent la croissance de la donnée). Vérification faite en
   * extrayant une frame : l'expression de largeur dépendant du temps
   * n'était pas honorée par ce build de ffmpeg, et la barre s'affichait
   * PLEINE sous « moins de 5 % ». Une jauge qui contredit le chiffre est
   * pire que pas de jauge : elle ment au spectateur.
   * On conserve donc ce qui a été vérifié efficace à l'image — kicker,
   * chiffre surdimensionné, libellé, source, et surtout la large zone
   * vide qui isole la donnée (86 % des références font ce choix). */
  /* ── JAUGE RÉINTRODUITE, MAIS DESSINÉE PAR LIBASS ─────────────────
   * La tentative précédente passait par `drawbox` avec une largeur
   * dépendant du temps : ce build de ffmpeg n'honore pas l'expression,
   * la barre sortait pleine et contredisait le chiffre.
   * libass, lui, sait animer : `\t()` interpole une transformation dans
   * le temps, et `\p1` trace un rectangle vectoriel. On dessine donc la
   * jauge comme un événement ASS dont l'échelle horizontale croît de 0 à
   * la proportion voulue — la croissance animée que 85 % des explainers
   * analysés utilisent, avec la décélération recommandée.
   * La part est déduite du pourcentage présent dans la valeur si elle
   * n'est pas fournie. Aucune jauge si l'on ne sait pas quoi montrer :
   * mieux vaut rien qu'une donnée fausse. */
  const filtres = [
    `drawbox=x=0:y=0:w=${W}:h=${barre}:color=${accent}@1:t=fill`,
  ];
  filtres.push(`ass='${assPath}':fontsdir='${fontsDir}'`, 'format=yuv420p');

  await ffmpeg([
    '-f', 'lavfi', '-t', fin,
    '-i', `color=c=${bg}:s=${W}x${H}:r=25`,
    '-vf', filtres.join(','),
    '-c:v', 'libx264', '-preset', 'medium', '-crf', '18',
    '-pix_fmt', 'yuv420p', '-an', out,
  ], { label: 'motion:dataSlide' });
...
async function generateMotionClip(type, params, ctx = {}) {
  ...
  switch (type) {
    /* `counter` visait un compteur qui s'incrémente — impossible sans
     * `drawtext`, absent du binaire. La slide ASS rend la même
     * information (chiffre, libellé, source) avec un fondu, et elle
     * fonctionne réellement. On y redirige plutôt que de laisser
     * l'ancien chemin échouer silencieusement. */
    case 'counter':
    case 'dataSlide':
