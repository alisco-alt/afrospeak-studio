...
/* ── NOMS DE FICHIERS PRODUITS PAR LES EXTENSIONS ───────────────────────
 * Cookie-Editor et consorts nomment l'export d'après le DOMAINE visité,
 * pas d'après la plateforme : « www.youtube.com_cookies.txt » et non
 * « youtube_cookies.txt ».
 *
 * Constaté chez l'utilisateur : le fichier était bien présent dans
 * cookies/, correctement exporté, mais `hasCookies('youtube')` renvoyait
 * false — le studio cherchait un nom que l'extension ne produit jamais.
 * Le cookie était donc ignoré en silence, et YouTube continuait de
 * renvoyer 403.
 *
 * On accepte donc les variantes de domaine les plus courantes. Le nom
 * canonique reste `<plateforme>_cookies.txt` pour l'écriture. */
const ALIAS_COOKIES = {
  ...
  facebook: ['www.facebook.com', 'facebook.com', 'web.facebook.com'],
  x: ['x.com', 'www.x.com', 'twitter.com', 'www.twitter.com'],
};

/** Chemin canonique (utilisé pour ÉCRIRE un cookie). */
function cookiePathCanonique(platform) {
  return path.join(COOKIE_DIR, `${platform}_cookies.txt`);
}

/**
 * Chemin RÉEL du cookie : le nom canonique s'il existe, sinon la première
 * variante de domaine trouvée sur le disque.
 */
function cookiePath(platform) {
  const canon = cookiePathCanonique(platform);
  try { if (fs.statSync(canon).size > 40) return canon; } catch (e) { /* on cherche */ }
  for (const alias of (ALIAS_COOKIES[platform] || [])) {
    const p = path.join(COOKIE_DIR, `${alias}_cookies.txt`);
    try { if (fs.statSync(p).size > 40) return p; } catch (e) { /* suivant */ }
  }
  return canon;   // absent : on renvoie le nom attendu, pour les messages
}
...
function saveCookies(platform, content) {
  ...
  /* On ECRIT toujours sous le nom canonique : `cookiePath` accepte les
   * variantes de domaine en LECTURE, mais un enregistrement doit produire
   * un nom previsible, sans quoi deux fichiers coexisteraient. */
  const p = cookiePathCanonique(platform);
...
/* Cookies qui portent réellement la SESSION. Un export de navigateur
 * contient des dizaines de lignes : traceurs publicitaires, préférences
 * d'affichage, identifiants invités… La plupart expirent vite et n'ont
...
const COOKIES_SESSION = {
  ...
  facebook: ['c_user', 'xs'],
  youtube: ['SID', '__Secure-1PSID', '__Secure-3PSID', 'LOGIN_INFO'],
};
...
function cookieStatus(platform) {
  ...
  /* ── NE JUGER QUE LES COOKIES DE SESSION ─────────────────────────────
   * L'ancienne règle déclarait le fichier expiré dès QU'UNE ligne l'était.
   * Or un export contient toujours des traceurs à durée courte : mesuré
   * sur un export X réaliste, `auth_token` et `ct0` valides 60 jours
   * étaient masqués par un `guest_id_marketing` périmé de la veille, et
   * la plateforme était écartée alors que la session fonctionnait.
   *
   * On ne regarde donc que les cookies qui portent l'authentification.
   * Si aucun n'est identifiable (plateforme inconnue), on retombe sur
   * l'ancien comportement plutôt que de déclarer un fichier valide à
   * tort. */
  const cles = COOKIES_SESSION[platform] || null;
  let expired = false, soonest = null, count = 0;
  let sessionVue = false, sessionExpiree = false, sessionSoonest = null;
  try {
    for (const line of fs.readFileSync(p, 'utf8').split('\n')) {
      ...
      count++;
      const nom = String(c[5] || '').trim();
      const exp = Number(c[4]);
      const estSession = cles && cles.includes(nom);
      if (estSession) sessionVue = true;
      if (exp > 0) {
        if (!soonest || exp < soonest) soonest = exp;
        if (exp * 1000 < Date.now()) expired = true;
        if (estSession) {
          if (!sessionSoonest || exp < sessionSoonest) sessionSoonest = exp;
          if (exp * 1000 < Date.now()) sessionExpiree = true;
        }
      }
      // exp === 0 : cookie de session pure, valide tant que le navigateur vit.
    }
  } catch (e) { return { present: false }; }

  const utile = sessionVue ? sessionExpiree : expired;
  return {
    present: true, count,
    expired: utile,
    /* Signale le cas « le fichier a des lignes périmées mais la session
     * tient » : utile pour ne pas inquiéter inutilement dans l'interface. */
    sessionValide: sessionVue ? !sessionExpiree : null,
    expiresAt: (sessionVue ? sessionSoonest : soonest)
      ? new Date((sessionVue ? sessionSoonest : soonest) * 1000).toISOString() : null,
...
async function collectArchive(keyword, { limit = 12, mediatype = 'movies', resolve = true, maxFileBytes = 45e6 } = {}) {
  try {
    ...
    for (let di = 0; di < Math.min(docs.length, limit * 2); di++) {
      ...
      /* Archive.org expose aussi des dépôts sans droit exploitable. Une
       * fiche sans URL de licence n'est pas assimilée au domaine public. */
      if (!/creativecommons|publicdomain|public-domain|wikimedia/i.test(licenseUrl)) continue;
      out.push({
        ...
        license: /publicdomain|public-domain/i.test(licenseUrl)
...
function preuveDroitSocial(item) {
  if (!item) return false;
  if (item.authorized === true || item.licensed === true || item.licenceCC === true) return true;
  const license = String(item.license || '').toLowerCase();
  const proofUrl = String(item.licenseUrl || '').trim();
  if (!license || !proofUrl || /usage éditorial|crédit affiché|droits à vérifier/.test(license)) return false;
  return /creative commons|domaine public|public domain|archive\.org/.test(license + ' ' + proofUrl);
}
 ...
 * Un crédit seul n'est pas une autorisation de téléchargement.
 */
async function download(item, { browser, maxHeight = 1920, timeout = 120000 } = {}) {
  if (!preuveDroitSocial(item)) {
    log.warn('téléchargement social refusé : licence/autorisation non établie');
    return null;
  }
