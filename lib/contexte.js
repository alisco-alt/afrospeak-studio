...
/* Actions de repli : quand aucune photo ne prouve l'identité exacte d'une
 * personne ou d'un événement, on ne remplace pas le sujet par un visage de
 * stock. On demande une scène filmable, ancrée dans le lieu et directement
 * liée au propos. Ces requêtes sont également utilisées par ensureCoverage :
 * elles ne doivent donc jamais contenir « African people » ou « city skyline »
 * sans pays/ville. */
const ACTIONS_VISUELLES = [
  { re: /(?:fabri|transform|atelier|artisan|savon|saponif|production|usine|manufactur)/i, q: 'local workshop artisans manufacturing' },
  { re: /(?:march[ée]|commerce|vendeur|prix|vente|boutique|consomm)/i, q: 'local market traders' },
  { re: /(?:agricul|ferme|champ|r[ée]colte|plantation|[ée]levage|rural)/i, q: 'local farmers working in fields' },
  { re: /(?:[ée]tudiant|universit|acad[ée]mie|[ée]cole|formation|apprenant)/i, q: 'local students learning classroom or workshop' },
  { re: /(?:port|export|import|logistique|camion|fret|conteneur)/i, q: 'local port workers cargo logistics' },
  { re: /(?:manifest|protest|rassemblement|meeting|sommet|[ée]lection|gouvernement)/i, q: 'local civic gathering official event' },
  { re: /(?:musique|danse|culture|festival|artiste|tradition|cuisine)/i, q: 'local artists cultural performance' },
];

function requetesAction(ctx = {}, texte = '') {
  const t = String(texte || '');
  const anchor = ctx.lieuEn || ctx.pays || ctx.aire || '';
  if (!anchor) return [];
  const action = ACTIONS_VISUELLES.find(x => x.re.test(t));
  const natureFallback = ctx.natures && ctx.natures.includes('economie')
    ? 'local workers market or workshop'
    : ctx.natures && ctx.natures.includes('societe')
      ? 'local community activity'
      : 'local people working in context';
  const base = action ? action.q : natureFallback;
  const out = [];
  /* Le lieu exact est toujours le premier terme. Les requêtes de repli ne
   * prétendent pas représenter l'identité nommée ; elles documentent l'action
   * et le contexte, ce qui est le seul repli honnête sans preuve. */
  out.push(`${anchor} ${base}`);
  if (ctx.lieuEn && ctx.pays && ctx.pays !== ctx.lieuEn) {
    out.push(`${ctx.lieuEn} ${ctx.pays} ${base}`);
  }
  if (ctx.entites && ctx.entites[0]) {
    out.unshift(`${ctx.entites[0]} ${anchor} official press photo`);
  }
  return [...new Set(out)].slice(0, 3);
}

/* ════════════════════════════════════════════════════════════════
   TAGGING SÉMANTIQUE D'UNE PHRASE (étape 1.3)
   ════════════════════════════════════════════════════════════════
 * Un monteur ne lit pas une phrase comme un bloc : il en extrait ce qui se
 * FILME. Cette fonction rassemble en une seule structure les trois familles
 * d'information dont dépend le choix du plan — jusqu'ici dispersées entre
 * mediaFetcher (mots-clés, chiffres) et contexte (lieux, entités).
 *
 * @returns {{visuels:string[], lieux:string[], donnees:object[], contexte:object}}
 */
function taguerPhrase(phrase, { heritage = null } = {}) {
  const mf = require('./mediaFetcher');
  const ctx = analyser(phrase, { heritage });

  // Mots-clés VISUELS : ce qu'une caméra peut cadrer, hors chiffres et
  // hors mots creux (le tri fin est déjà fait par mediaFetcher).
  const visuels = mf.keywords(phrase)
    .filter(w => !/^\d+$/.test(w))
    .filter(w => !mf.MOTS_NON_VISUELS.has(w))
    .slice(0, 8);

  // Lieux et institutions nommément cités
  const lieux = [...(ctx.entites || [])];
  if (ctx.lieuEn && !lieux.some(l => new RegExp(ctx.lieuEn, 'i').test(l))) {
    lieux.push(ctx.lieuEn);
  }

  // Données chiffrées, avec leur unité quand elle est exprimée
  const donnees = (mf.extractFigures(phrase) || []).map(f => {
    const m = /^([\d\s.,]+|\S+)\s*(.*)$/.exec(String(f.value).trim()) || [];
    return {
      valeur: String(f.value).trim(),
      nombre: (m[1] || '').trim(),
      unite: (m[2] || '').trim(),
      position: f.index,
    };
  ...
  return { visuels, lieux, donnees, contexte: ctx };
}

/* ════════════════════════════════════════════════════════════════
   SÉQUENÇAGE « VALLEY TO PORT »
   ════════════════════════════════════════════════════════════════
 * Sur un sujet de filière — cacao, or, coton, pétrole — un reportage
 * d'agence remonte la chaîne de valeur dans un ordre invariable, parce que
 * c'est l'ordre dans lequel la valeur se crée :
 *
 *   1. le produit brut       (gros plan, la matière)
 *   2. les acteurs officiels (visages, décideurs)
 *   3. la production         (terrain, vues aériennes)
 *   4. la transformation     (usines, laboratoires, qualité)
 *   5. la logistique         (ports, fret, export)
 *
 * Sans cette grammaire, le montage saute du port à la plantation puis
 * revient au ministère : le spectateur perd le fil de la filière.
 */
const FILIERES = {
  cacao: { en: 'cocoa', brut: 'cocoa pods beans', terrain: 'cocoa farm plantation', usine: 'chocolate processing factory' },
  cafe: { en: 'coffee', brut: 'coffee cherries beans', terrain: 'coffee plantation hillside', usine: 'coffee roasting plant' },
  coton: { en: 'cotton', brut: 'raw cotton bolls', terrain: 'cotton field harvest', usine: 'textile spinning mill' },
  or: { en: 'gold', brut: 'gold ore nuggets', terrain: 'open pit gold mine', usine: 'gold refinery smelting' },
  petrole: { en: 'oil', brut: 'crude oil barrels', terrain: 'offshore oil platform', usine: 'oil refinery towers' },
  gaz: { en: 'gas', brut: 'natural gas pipeline', terrain: 'gas field wellhead', usine: 'LNG processing plant' },
  bauxite: { en: 'bauxite', brut: 'bauxite ore red', terrain: 'bauxite open pit mine', usine: 'alumina refinery' },
  lithium: { en: 'lithium', brut: 'lithium ore spodumene', terrain: 'lithium mine site', usine: 'lithium processing plant' },
  cobalt: { en: 'cobalt', brut: 'cobalt ore', terrain: 'cobalt mine site', usine: 'cobalt refinery' },
  anacarde: { en: 'cashew', brut: 'raw cashew nuts', terrain: 'cashew orchard farm', usine: 'cashew processing plant' },
  karite: { en: 'shea', brut: 'shea nuts', terrain: 'shea tree savanna women', usine: 'shea butter factory' },
  hevea: { en: 'rubber', brut: 'rubber tree latex', terrain: 'rubber tree plantation', usine: 'rubber processing plant' },
  manioc: { en: 'cassava', brut: 'cassava roots', terrain: 'cassava field farmers', usine: 'cassava flour mill' },
  riz: { en: 'rice', brut: 'rice paddy grains', terrain: 'rice paddy field aerial', usine: 'rice milling plant' },
  poisson: { en: 'fish', brut: 'fresh fish catch', terrain: 'fishing boats harbour', usine: 'fish processing plant' },
};

/** Repère la filière dont parle le sujet, s'il y en a une. */
function detecterFiliere(texte) {
  const t = normaliser(texte);
  for (const [cle, f] of Object.entries(FILIERES)) {
    if (new RegExp(`\\b${cle}`).test(t) || new RegExp(`\\b${f.en}\\b`).test(t)) {
      return { cle, ...f };
    }
  }
  return null;
...
/**
 * Plan de séquence « Valley to Port ».
 * @returns {Array<{etape:number, role:string, requete:string}>} vide si le
 *   sujet ne relève pas d'une filière d'exportation
 */
function sequenceFiliere(ctx, texte = '') {
  const f = detecterFiliere(`${texte} ${(ctx.entites || []).join(' ')}`);
  if (!f) return [];
  // La séquence n'a de sens que sur un sujet d'industrie ou d'export
  if (!ctx.natures.includes('economie') && !/export|fili[èe]re|production|transform/i.test(texte)) {
    return [];
  }
  const lieu = ctx.lieuEn || 'Africa';
  return [
    { etape: 1, role: 'produit brut', requete: `${f.brut} close up ${lieu}` },
    { etape: 2, role: 'acteurs officiels', requete: `${lieu} ${f.en} industry officials meeting` },
    { etape: 3, role: 'production terrain', requete: `${f.terrain} aerial view ${lieu}` },
    { etape: 4, role: 'transformation', requete: `${f.usine} ${lieu}` },
    { etape: 5, role: 'logistique', requete: `${lieu} port container terminal export` },
  ];
}
...
module.exports = {
  ...
  sourcesPour, vocabulairePour, requetesDocumentaires, requetesAction, coherenceVisuelle, resumer,
  sequenceFiliere, detecterFiliere, FILIERES, taguerPhrase,
